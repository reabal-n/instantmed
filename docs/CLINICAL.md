...
The **AI-draft `requiresReview` signal blocks issuance** (operator decision 2026-08-07, promoted from soft): an uncertain clinical draft routes the request to `needs_doctor` — deterministically, never the retry queue — so a doctor decides **before any certificate exists**. Ninety days of production drove the promotion: 8 of 109 auto-approvals carried the flag, all of them from the draft lane, including a presentation the draft itself called potential red-flag symptoms; the keyword gates fired on none of them. Cost: roughly one extra manual review per 11 days. The AI still decides nothing — uncertainty escalates to a human.

The engine's remaining **soft flags** — co-symptom mental-health / injury / chronic keyword mentions — still do not block issuance (they fired zero times in the same 90 days). They are persisted onto `intakes.risk_flags` as `info`-severity flags at approval time, render through the normal `IntakeFlagsBadge`/`IntakeFlagsPanel` path, and mark their row **Flagged**, sorted first in the daily approved list. Prioritisation, not obligation — nothing expires and nothing alerts. Promoting any further soft signal to a pre-issuance block remains an operator clinical-policy decision. That stream is admin-scoped because an auto-issued certificate has no reviewing doctor and therefore falls outside the per-doctor patient-access boundary. **Revocation remains the standing correction path** and is always individual, requires a recorded clinical reason, invalidates the certificate, and returns the intake to `in_review`. Both the revoke and the 30s approval-undo return the intake to `in_review` via a DB-trigger-guarded reversal that requires the certificate to be revoked first (migration `20260711193000`). The `batch_reviewed_at` / `batch_reviewed_by` columns are retained as a historical audit record of reviews performed while the obligation existed; nothing writes them now.
