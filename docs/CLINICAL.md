...
| **Med cert language** | Conservative but doctor-owned consultation statement only. PDF body has three locked paragraphs: (1) certification — "I certify that [patient] consulted me on X. Based on my assessment, they were unable to attend [work/study] duties" or, for carer's leave, "required to provide care or support"; (2) absence scope — "This certificate relates to the absence date(s) stated above."; (3) support — "Please get in touch with us if you have any questions about this certificate." (warmth only; no support email in the body — the footer carries the contact channel). No "medically unfit", no fitness-for-X, no exam-deferral support, no workplace-restriction/capacity-assessment disclaimer, no diagnosis, and no modality disclosure on the cert body. All three sentences (`getBodyText`/`getReturnText`/`getSupportText`) are locked in `lib/pdf/template-renderer.ts` and pinned by `lib/__tests__/med-cert-medicolegal-scope.test.ts` |
