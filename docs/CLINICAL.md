...
The **AI-draft `requiresReview` signal blocks protocol issuance** (operator decision 2026-08-07, promoted from soft): an uncertain clinical draft routes the request to `needs_doctor` deterministically, never the retry queue. Ninety days of production drove the promotion: 8 of 109 historical auto-approvals carried the flag, all of them from the draft lane, including a presentation the draft itself called potential red-flag symptoms; the keyword gates fired on none of them. Those eight historical cases form the bounded Medical Director retrospective-review set. AI uncertainty can only narrow the automated lane; it cannot approve a request or widen the protocol.
