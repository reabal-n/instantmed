...
The dormant engine remains in `lib/clinical/auto-approval-pipeline.ts` for controlled evaluation and possible future reactivation. If governance approval is later recorded through a reviewed code change, it remains feature-flagged, rate-limited, logged to `ai_audit_log`, and limited to medical certificates. Prescriptions and specialty requests never use this pathway. See ARCHITECTURE.md -> Auto-Approval Pipeline.
...
The dormant engine's remaining **soft flags** — co-symptom mental-health / injury / chronic keyword mentions — fired zero times in the same 90-day period. If protocol issuance is reactivated, they remain persisted onto `intakes.risk_flags` as `info`-severity flags at approval time and render through the normal `IntakeFlagsBadge`/`IntakeFlagsPanel` path. Promoting any further signal to a pre-issuance block remains an operator clinical-policy decision. **Revocation remains the standing correction path** for historical auto-issued certificates and is always individual, requires a recorded clinical reason, invalidates the certificate, and returns the intake to `in_review`. Both the revoke and the 30s approval-undo return the intake to `in_review` via a DB-trigger-guarded reversal that requires the certificate to be revoked first (migration `20260711193000`). The `batch_reviewed_at` / `batch_reviewed_by` columns are retained as a historical audit record; nothing writes them now.
