...
| Weight management | LIVE 2026-08-10. Doctor-reviewed only — never auto-approved. Phentermine is within prescribing scope (operator correction 2026-09-24); medicine-specific suitability remains a doctor decision. One-off review: up to 4 weeks of medicine with no repeats; continuation requires a new consult (operator supply decision 2026-09-24); no ongoing-monitoring promise. Eating-disorder or cardiac history requires a doctor call before any decision. `can_review_weight_loss` capability + Medical Director sign-off required for non-admin doctors. |
...
- **Weight-management routing — operator decision 2026-09-14.** GLP-1 requests, including Mounjaro (and the `monjaro` spelling), Wegovy and Ozempic, and phentermine requests, including Duromine and Metermine, hard-route to the $89.95 weight assessment and cannot proceed through repeat-Rx. A `type_2_diabetes` answer does not exempt a GLP-1 request; the former structured weight-vs-diabetes escape is retired. Routing does not guarantee a prescription. Phentermine is within prescribing scope (operator correction 2026-09-24), and orlistat stays flag-only for decline-to-GP. The original August launch plan records the earlier routing decision; this September decision supersedes its repeat-lane exemptions. Application release evidence remains separate in `docs/OPERATIONS.md`.

The September 24 operator correction supersedes the documented phentermine exclusion; it corrects the recorded scope rather than describing a new prescribing launch. The repeat classifier also recognises Trulicity/dulaglutide and explicit `GLP-1`/`GLP1` medicine requests. Brand and generic detection covers Mounjaro/Zepbound/tirzepatide, Wegovy/Ozempic/Rybelsus/semaglutide, Saxenda/Victoza/liraglutide, Trulicity/dulaglutide, and Duromine/Metermine/phentermine, including combined brand-and-generic labels. Exact whole-word aliases `duramine`, `ozemptic` and `triztepide` route like Duromine, Ozempic and tirzepatide; aliases determine the assessment route only and are not verified prescribing identities. Within the assessment, the optional `weightLossMedPreference` answer records daily oral treatment, weekly injection, or unsure. It is visible in patient review and the clinician summary, does not select a medicine, and never relaxes existing safety checks. The doctor must assess medicine-specific contraindications, interactions and monitoring before prescribing. Public marketing remains medicine-neutral. The same-day safety decision keeps MEN2/medullary thyroid cancer and pancreatitis histories eligible for assessment with high-risk triage and a persisted `weight_medicine_review` attention flag, rather than declining the whole service. The clinician summary calls for medicine-specific assessment, with further information/contact when needed; neither preference nor checkout approval establishes medicine suitability. Required answers, age, pregnancy/breastfeeding, BMI and existing cardiac/eating-disorder call rules remain enforced. No blood-pressure input or mandatory reading upload is added; this does not waive clinician responsibility for medicine-specific assessment and monitoring.

Weight-management supply: each approved consultation covers **up to 4 weeks with no repeats**; continuing requires a new consultation. Patient wording is medicine-neutral and explains that medicine costs are separate. Clinician guidance specifies one four-dose weekly pen or one oral pack only where the chosen product, dose and quantity fit the four-week limit; a pen or pack is not universally a four-week quantity. This is a service prescribing limit, not automated control of quantities entered in the external prescribing portal.
...
For the authenticated doctor handoff only, `lib/clinical/generic-medication-resolver.ts` first exact-matches patient wording against InstantMed's curated first-party `medications` table. Only generic names and brand aliases participate. The clinician-only reference also includes the versioned PBS names snapshot in `lib/clinical/pbs-medication-names.json` and the TGA-referenced semaglutide and tirzepatide supplements. The PBS extract covers ATC medicinal groups A-S; it imports no doses, eligibility rules, prices, or patient-facing search. Reference presence does not establish availability, therapeutic equivalence or prescribing suitability. If that lookup is unresolved, `lib/clinical/prior-medication-match.ts` may compare the entry with up to 20 non-cancelled prescriptions belonging to the same patient, but only after the doctor is verified as the current or prior reviewer of that intake (admins retain their full clinical role). A unique, narrowly bounded typo candidate must then exact-resolve through the curated table before it becomes copyable; this prior-history path remains independent of the catalogue spelling check. For unresolved entries, the clinician-only spelling check in `lib/clinical/medication-spelling-match.ts` may consider one unique, narrowly similar catalogue name. Brand aliases for the same generic count as one identity when checking ambiguity. The direct clinical AI provider requires `ANTHROPIC_API_KEY`; gateway-only configuration leaves spelling unresolved. The existing clinical AI provider may only accept or reject that candidate; it cannot return a new medicine name. Only the dose/form-stripped name fragment and catalogue labels are sent, with no patient identifier, raw narrative, dose or directions. An eight-second timeout, no retries, disabled prompt telemetry, strict boolean output validation and per-doctor AI rate limit bound the request. Ambiguous, unrelated, unconfigured or failed results remain unresolved. Catalogue coverage is limited; no match does not mean a medicine is invalid. The offline snapshot is regenerated from a reviewed public monthly PBS ZIP with `python3 scripts/generate-pbs-medication-names.py ARCHIVE YYYY-MM-DD lib/clinical/pbs-medication-names.json`; its effective date, schedule revision and archive hash are recorded in the snapshot. Run `python3 scripts/test-generate-pbs-medication-names.py` and the medication-reference/spelling tests after refresh. No patient entry is sent to PBS. The panel labels the source and requires confirmation in Parchment. The narrow result is a clipboard action containing the verified generic medicine name alone; it never returns strength, form, dose, directions, PBS eligibility, substitution advice, or therapeutic equivalence. Unknown, ambiguous, unsafe-looking, unrelated, or unauthorised entries expose no verified-generic copy action; the existing sanitised patient-entered search aid remains explicitly unverified. The reference cannot affect intake eligibility, safety routing, request status, prescribing, or the medicine selected in Parchment/MIMS.
...
- **Codeine combination repeats are prescribed at most once every 7 days (operator decision 2026-09-19).** The doctor may prescribe Panadeine Forte and other paracetamol/ibuprofen + codeine products case by case, but a repeat request inside 7 days of a prior codeine-combination script is stopped before payment by `lib/stripe/checkout/codeine-repeat-gate.ts` on every payable path (authenticated checkout, guest checkout, guest recovery, signed guest resume, retry payment). Signed-in patients see the issue date and the day they can request again; a guest whose typed email or name + date of birth matches a patient with a recent codeine script is asked to sign in, so prescription history is never disclosed to an unauthenticated form. The gate keys on issued `prescriptions` rows (active or completed) matched by the codeine-combination detector in `lib/clinical/controlled-substances.ts`; it fails open on a lookup error and never replaces the doctor's decision on requests that reach the queue. Each refusal writes a sanitized `safety_audit_log` receipt (`codeine_repeat_within_7d`). Signed-in patients are also warned at the medication step through the patient-scoped `checkCodeineRepeatWindowAction` (same gate, same dates, nothing persisted) so they do not fill three more screens first; the warning is advisory UX and the checkout gate remains the enforcement.

Prescription issue dates are normalized to the Australia/Sydney calendar day at Parchment sync. The repeat window uses the recorded `issued_date`; database insertion time is not evidence of issue time and must not move that date. Historical date corrections require the authoritative provider issue date, not an inference from `created_at`.
...
- The doctor medication-search copy control contains only a locally verified generic name or a clearly labelled, safely stripped patient-entered name fallback; it never includes strength, form or regimen. The compact Parchment header shows complete medicine/strength, full source directions, actual separately captured frequency when available, and the source indication or specialty request label. A separate directions control copies the original patient answer without converting units or dropping qualifiers. The current intake does not independently capture frequency: retain that distinction in the request packet, omit the empty frequency row from the prescribing header, and never copy a shortened extraction as a separate answer. No clipboard action selects or enters a prescription. Specialty medicine/directions retain a visible template label; the full source assessment is available under Clinical details, with genuine blockers remaining visible outside the disclosure. Request context is not a diagnosis.
- The ED review groups each named, explicitly negative screening answer into a compact wrapping row. Positive, missing and conflicting answers stay distinct; absence never means no. The retired separate blood-pressure-medication field appears only when an older request records a value; the current form's general medication history remains authoritative for what the patient was asked. Prior-treatment confirmation and its detail may share one row when consistent, while contradictory or missing source answers remain visible separately.
...
- Clinical safety classification and durable safety holds run before a consent-reconfirmation response, including for historical requests without receipts. Authenticated retries and signed guest resume require a current receipt before any payable checkout URL. Missing evidence prevents a new payment session. Existing owned open sessions are expired, paid/in-flight sessions retain their completion handoff, and uncertain payment state stays unresolved. This does not revoke a previously issued Stripe URL after a later identity change. Cookie preferences remain independent of clinical attestations.
