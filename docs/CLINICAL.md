...
- A medical certificate or prescription is issued only after an AHPRA-registered doctor reviews the request
...
- The doctor remains accountable for individual review, prescribing, and clinical outcomes

**Canonical public clinical-model claim:** "AI never prescribes or makes clinical decisions. A certificate or prescription is issued only after an AHPRA-registered doctor reviews the request." Public surfaces must read this from `clinical_decision_model` in `lib/marketing/approved-claims.ts`, which also owns its risk classification, allowed contexts, and evidence receipts.
...
- Medical-certificate, prescribing, and specialty requests require a doctor-selected outcome before fulfilment
...
| **Med cert duration** | Hard cap 3 days. Constant: `MAX_MED_CERT_DURATION_DAYS` in `lib/clinical/intake-validation.ts`. The dormant protocol evaluator also flags `duration_too_long` for anything above. No override |
...
Deterministic safety rules may stop or redirect a clearly unsuitable pathway without AI or clinician inference. Medical-certificate protocol issuance is **not active**. The code-owned gate in `lib/clinical/auto-approval-governance.ts` is fail-closed while Medical Director and legal reconciliation remains pending. The database feature flag (`ai_auto_approve_enabled`) is only an operational kill switch and cannot authorise issuance. Every paid medical-certificate request therefore remains in the doctor queue for an individual outcome before any certificate exists.

The dormant engine remains in `lib/clinical/auto-approval-pipeline.ts` for controlled evaluation and possible future reactivation. The prepared initial rollout is code-clamped to clean, unflagged one-day requests after a minimum 15-minute delay, at no more than 3 approvals per five minutes or 10 per day. Database settings may narrow those limits but cannot widen them. This preparation does not authorise issuance while the governance gate remains closed. If governance approval is later recorded through a reviewed code change, the pathway remains feature-flagged, logged to `ai_audit_log`, and limited to medical certificates. Prescriptions and specialty requests never use this pathway. See ARCHITECTURE.md -> Auto-Approval Pipeline.

If the dormant protocol is ever reactivated, medical-certificate risk remains gated **before** issuance, not after. Any deterministic risk signal — red flags, emergency presentation, mental health, injury, chronic disease, pregnancy, high-stakes use case (exam deferral, fitness to drive or operate, court, workers comp), under-18 or unusable DOB, over-length duration, overlapping certificate dates, or any doctor-attention intake flag — short-circuits to `needs_doctor` via `DETERMINISTIC_FAILURE_PREFIXES` (`lib/clinical/auto-approval-state.ts`). Widening that prefix list is the correct response to a new risk class.

There is **no post-approval attestation control relied upon for current issuance**. The former 24-hour batch/cohort review window was retired on 2026-08-04. A retrospective sign-off is not a substitute for a pre-issuance gate, so the unresolved governance question is now handled conservatively: protocol issuance is hard-paused and doctor review happens before issue.

Medical Board telehealth guidance emphasises real-time consultation and individual practitioner judgment for medical certificates, and AHPRA guidance requires human oversight of AI-supported care. The Medical Director / legal reconciliation remains open. Reactivation requires an explicit code review of `AUTO_APPROVAL_GOVERNANCE.approved`; changing the database flag alone is insufficient.
...
The **AI-draft `requiresReview` signal blocks dormant protocol issuance** (operator decision 2026-08-07, promoted from soft): an uncertain clinical draft routes the request to `needs_doctor` — deterministically, never the retry queue. Ninety days of production drove the promotion: 8 of 109 auto-approvals carried the flag, all of them from the draft lane, including a presentation the draft itself called potential red-flag symptoms; the keyword gates fired on none of them. Those eight historical cases form the bounded Medical Director retrospective-review set. The AI still decides nothing — uncertainty escalates to a human.

The dormant engine's remaining **soft flags** — co-symptom mental-health / injury / chronic keyword mentions — fired zero times in the same 90-day period. The prepared initial rollout treats every one as a pre-issuance manual-review route while still persisting it onto `intakes.risk_flags` as an `info`-severity signal for the doctor through the normal `IntakeFlagsBadge`/`IntakeFlagsPanel` path. Allowing any soft-flagged request to issue would require another reviewed code-policy change; the database cannot widen this boundary. **Revocation remains the standing correction path** for historical auto-issued certificates and is always individual, requires a recorded clinical reason, invalidates the certificate, and returns the intake to `in_review`. Both the revoke and the 30s approval-undo return the intake to `in_review` via a DB-trigger-guarded reversal that requires the certificate to be revoked first (migration `20260711193000`). The `batch_reviewed_at` / `batch_reviewed_by` columns are retained as a historical audit record; nothing writes them now.
