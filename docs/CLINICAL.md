...
Medical-certificate risk is gated **before** issuance, not after. Any deterministic risk signal — red flags, emergency presentation, mental health, injury, chronic disease, pregnancy, high-stakes use case (exam deferral, fitness to drive or operate, court, workers comp), under-18 or unusable DOB, over-length duration, overlapping certificate dates, or any doctor-attention intake flag — short-circuits to `needs_doctor` via `DETERMINISTIC_FAILURE_PREFIXES` (`lib/clinical/auto-approval-state.ts`) and is **manually approved or declined by a doctor before any certificate exists**. Widening that prefix list is the correct response to a new risk class.

There is **no post-approval attestation obligation** (operator decision 2026-08-04). The former 24-hour batch/cohort review window was retired: it applied only to the certificates that had already cleared every deterministic gate, so it added no clinical signal, while a lapsed window paged the operator every 30 minutes. A retrospective sign-off is not a substitute for a pre-issuance gate — do not reintroduce one; strengthen the pre-approval gate instead. The 24-hour window was an **InstantMed governance control, not a statutory AHPRA requirement**: no rule mandating this specific attestation was identified.

**This is not a finding that removal leaves professional obligations unchanged.** Medical Board telehealth guidance emphasises real-time consultation and individual practitioner judgment for medical certificates, and AHPRA guidance requires human oversight of AI-supported care. Removal is an operator product decision that is **still pending Medical Director / legal reconciliation** against those obligations; it supersedes the "confirmed invariant" recorded in `docs/superpowers/plans/2026-07-10-audit-remediation-program.md`, which carries the full reconciliation note. If that review concludes an individual outcome is required, reinstate it at the **pre-issuance** gate, not as a retrospective attestation.

Oversight after issuance is **visibility plus correction, not sign-off**. Auto-issued certificates appear in the staff dashboard's daily approved list, labelled `auto_issued` so a protocol issuance is never presented as a clinician's own decision, and never counted in a doctor's own review totals.

The engine's **soft flags** — co-symptom mental-health / injury / chronic mentions, and an AI-draft `requiresReview` hint — do not block issuance, but `lib/clinical/auto-approval.ts` produces them on the explicit assumption that a human sees them afterwards ("Batch review catches any concerns post-approval"). They are therefore persisted onto `intakes.risk_flags` as `info`-severity flags at approval time, render through the normal `IntakeFlagsBadge`/`IntakeFlagsPanel` path, and mark their row **Flagged** and sort it first in the daily approved list. This is prioritisation, not obligation — nothing expires and nothing alerts. **Whether any soft signal should instead become a pre-issuance block is an open clinical-policy decision for the operator**; promoting one would reduce the auto-approval rate and must not be done without that decision. That stream is admin-scoped because an auto-issued certificate has no reviewing doctor and therefore falls outside the per-doctor patient-access boundary. **Revocation remains the standing correction path** and is always individual, requires a recorded clinical reason, invalidates the certificate, and returns the intake to `in_review`. Both the revoke and the 30s approval-undo return the intake to `in_review` via a DB-trigger-guarded reversal that requires the certificate to be revoked first (migration `20260711193000`). The `batch_reviewed_at` / `batch_reviewed_by` columns are retained as a historical audit record of reviews performed while the obligation existed; nothing writes them now.
...
**Brand coverage is how this list actually fails (2026-08-04).** Five paid repeat-prescription requests reached a doctor and were declined+refunded because the blocklist held the generic name but not the brand a patient writes: `Oxynorm` (immediate-release oxycodone — every *other* oxycodone brand was listed) and a bare `CBD` entry (only `cbd oil` was listed). Also added on the same evidence: `norspan`, `flunitrazepam`/`hypnodorm` (an S8 benzodiazepine that was missing outright), `euhypnos`, `ducene`, `alepam`, and the AU spelling `dexamfetamine`. **Refund evidence alone is not grounds to block.** `Lomotil` was added in the same pass and reverted the same day: it is Schedule 3/4 in Australia (not S8), it is a legitimate ongoing repeat for chronic diarrhoea/IBS/ostomy patients, and its "decline" was recorded as *"Patient will resubmit request"* — operational, not clinical. Before blocking a medicine, confirm both that the decline reason was clinical and that the AU schedule actually warrants it. When adding a controlled medicine, add **every AU brand and spelling a patient might type**, not just the generic — a generic-only entry is an open gate. Adding widens every layer at once and is the safe direction; over-blocking an ordinary repeat is not, so `lib/__tests__/controlled-substances-parity.test.ts` pins both directions.

**Predictably-declined medicines are warned about, never blocked.** The codeine-combination carve-out stands: `Panadeine Forte`, `Mersyndol Forte` and similar stay out of `CONTROLLED_SUBSTANCE_TERMS` so the reviewing doctor keeps the decision. But two of them were paid for and declined in 2026-08, so the intake now shows an advisory **before payment** (`isLikelyDeclinedOnline()` in `lib/clinical/controlled-substances.ts`, rendered on the medication step): it names the medicine, says these are usually declined online and referred to the patient's GP, states plainly that a decline is refunded in full, and requires one explicit "Continue anyway" acknowledgement. It never blocks and never overlaps a hard block — a controlled substance returns `false` from the advisory so the two signals cannot both render. Adding a brand to that advisory list costs a patient nothing and can only prevent a wasted payment.
