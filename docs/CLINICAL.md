...
- The doctor medication-search copy control contains only a locally verified generic name or a clearly labelled, safely stripped patient-entered name fallback; it never includes strength, form or regimen. The Parchment panel separately shows the same complete medicine/strength, current dose/directions, frequency state and indication as the current request packet. A separate directions control copies the original patient answer without converting units or dropping qualifiers. Frequency is not independently captured by the current intake, so show that state and do not copy a shortened extraction as a separate answer. No clipboard action selects or enters a prescription. Specialty panels label medicine/directions templates separately from the existing request assessment, show that assessment as request context rather than a diagnosis, and explicitly state when separate frequency or assessment context is unavailable.
