...
| **Med cert language** | Conservative but doctor-owned consultation statement only. PDF body says "I certify that [patient] consulted me on X. Based on my assessment, they were unable to attend [work/study] duties" or, for carer's leave, "required to provide care or support". The only scope line is "This certificate relates to the absence date(s) stated above." No "medically unfit", no fitness-for-X, no exam-deferral support, no workplace-restriction/capacity-assessment disclaimer, no diagnosis, and no modality disclosure on the cert body. Locked in `lib/pdf/template-renderer.ts` |
