...
| E2E tests | Playwright | `e2e/**/*.spec.ts` | 72 specs — blocking CI currently runs ops/navigation/clinical-input smoke plus focused paid critical flows |
...
- `monitoring-observations`, `browser-observer` and `monitoring-source-contract` exercise synthetic source timestamps, 359/360/365-minute silence, durable grace, running after failure, skipped execution, late completion/reruns, malformed/future/HTTP/timeout evidence, cache/request limits, finite coverage-gap recovery, capped multi-poll failure/recovery, failed-preflight purchase-state preservation with independent category recovery, setup-over-imports and imports-over-primary masking transitions, explicit failure/recovery and persistence fail-open. The contract preserves seven production cases and local `/api/draft` + `/ingest` isolation; these tests make no live provider/browser calls.
- `bash scripts/test-monitor-observation-state-db.sh` uses disposable PostgreSQL 15 and no repository environment/production URL. It verifies additive migration reapplication, table/RPC ACLs, bounded payloads, source ordering, immutable cache, a held transaction competing with a second connection for the same version, retention of current state and missing-state failure. `--baseline` deliberately omits the new migration and must fail on the absent RPC. CI runs the normal fixture as a blocking build step. SQL/shell fixtures need no Knip TypeScript entry; CI invokes them directly.
- `lib/__tests__/decline-intake.test.ts` — coverage for `app/actions/decline-intake.ts` (actor gate, idempotency, full-refund entitlement by category, durable Stripe-mode snapshot, and the Vercel-safe E2E short-circuit; there is no caller-controlled refund bypass)
...
- `lib/__tests__/profile-encryption-backfill.test.ts` runs the actual profiles-only CLI with synthetic authenticated ciphertext. It covers read-only defaults/errors, strict flags, wrong/mixed/missing key evidence, preserved parity exceptions, null/empty fields, canonical machine exclusions, frozen candidate scopes across between-scan insert/eligibility races, strict expectation-file parsing before all data access, aggregate/HMAC approval mismatches before every status/profile write (including equal-count substitutions and zero-candidate reruns), deterministic opaque fingerprints, full-snapshot CAS source/target/identity/timestamp/deletion races, affected-row receipts, partial-batch continuation, safe status errors, resume and zero-change reruns. The test-only `apply` helper obtains a separate synthetic dry-run expectation before injecting apply faults; raw `run` calls own missing/malformed/stale expectation tests. CLI children use a fresh temporary working directory and an allowlisted environment; they never load repository `.env` files. `bash scripts/test-profile-encryption-backfill-db.sh` repeats the contract suite through disposable PostgreSQL/PostgREST, using `scripts/fixtures/profile-encryption-backfill.sql` and a loopback-only `PROFILE_BACKFILL_FIXTURE_URL`. The normal unit run explicitly skips this database variant; the dedicated blocking CI step runs it. This fixture proves conditional-update behavior on a minimal synthetic schema, not production schema, authorization, key compatibility, or execution.
...
**CI pipeline:** `pnpm ci` runs `install → lint → typecheck → test → build` in sequence. Main requires a PR plus strict, up-to-date GitHub Actions `build` **and** `e2e` checks; zero required approvals supports the solo operator without reopening direct pushes. Under the September 24 owner-approved cadence, PRs changing payment, authentication, clinical, database, shared runtime, tooling or unclassified paths run full checks immediately. Only the explicit presentation/content allowlist in `scripts/ci-e2e-required.sh` uses lint, typecheck, related Vitest tests and the production build without the shared E2E queue. Documentation-only PRs run the agent-doc sync and documentation audit without an application build. The blocking E2E gate for sensitive changes runs the current ops/navigation/clinical-input smoke (`e2e/admin.ops-index.spec.ts`, `e2e/marketing-dashboard-nav.spec.ts`, `e2e/dashboard.keyboard-safety.spec.ts`, `e2e/landing-pages.spec.ts`) plus focused paid-flow smoke coverage (`pnpm medcert:readiness:e2e`, `e2e/unified-request-flow.spec.ts`, `e2e/consult-subtypes.spec.ts`, `e2e/intake-terminal-blocks.spec.ts`, `e2e/parchment-webhook.spec.ts`) after a fail-fast required-secret check, then runs `e2e/checkout-resume.spec.ts` as a separate encrypted-answer safety gate with isolated Playwright artifacts. Known low-risk and documentation-only changes resolve the shared-fixture job without running those tests; file enumeration and classifier failures keep E2E enabled. Full coverage, database checks, the maintained browser regression matrix and Lighthouse run Monday/Thursday at 19:17 UTC (Tuesday/Friday morning in Sydney), or by manual workflow dispatch. Live-production Lighthouse also runs Tuesday/Friday at 02:00 UTC; the existing request-availability monitor keeps its separate operational cadence. There is no duplicate CI run on a merge push; protected PR checks precede Vercel deployment. This schedule uses the maintained regression matrix, not every historical Playwright spec. The older broad Playwright suite is not a reliable blocking signal right now; it contains stale routes and product-state assumptions and should be repaired as a dedicated E2E cleanup pass before being restored as a merge gate. Preview deployments run `e2e/preview-smoke.spec.ts`.
...
The real-session queue Realtime regression is fail-closed: it runs only with `E2E_ISOLATED_SUPABASE=1` against an isolated, non-production Supabase project, and its runtime guard requires the server and browser Supabase URLs to resolve to the same non-production project ref. It creates one ephemeral doctor auth account and one synthetic patient profile, verifies draft-to-paid queue arrival while another review stays selected, then deletes both. Never enable that flag against the shared production project; an authenticated staff Realtime subscription can receive live queue frames even when the page itself renders seeded fixtures.
...
E2E tests auto-seed and teardown test data. Seed/teardown scripts in `scripts/e2e/`, invoked by `e2e/global-setup.ts` and `e2e/global-teardown.ts`. Paid-flow fixtures use deterministic patient IDs registered in `lib/data/seeded-e2e-data.ts`, and every direct E2E intake insert must set `exclude_from_reporting: true`. The admin-only seeded dashboard mode scopes both the active queue and the completed-review history to the fixed seed patient; never add an adjacent dashboard read that can blend live staff data into `onlyTestData`. The canonical auth/profile/intake fixtures are preserved by default during teardown; only transient child records are cleared unless `E2E_TEARDOWN_RESET_FIXTURES=1` is set for a deliberate full fixture reset. CI serializes the blocking E2E job across branches because those shared fixtures point at the same Supabase project. `scripts/ci-e2e-required.sh` keeps documentation and explicitly allowlisted presentation/content changes out of that shared queue while failing closed for sensitive/unclassified paths, workflow changes, empty file sets, and unknown events. Schedules and manual dispatch always run the full maintained matrix. Helpers in `e2e/helpers/`:
...
**Checkout consent telemetry:** `pnpm exec playwright test --config=playwright.consent-tracking.config.ts` starts isolated local services on 3060/3079 with a fake PostHog key and denied external server I/O. The dedicated `e2e/checkout-consent.browser.ts` filename keeps this analytics-enabled test out of the normal analytics-disabled E2E suite. Browser interception captures SDK event payloads locally and blocks draft/payment/provider writes. It verifies restored consent, edits, confirmation taps and visible consent on Android/iPhone in light/dark modes, including absence of synthetic identity/clinical text in the new events. Bot signals are overridden only inside these intercepted test browsers so the real SDK emits events. This proves local capture and privacy filtering, not ingestion by production PostHog or a completed Stripe payment.
...
**Staff test reads must be scoped before decryption.** Navigation counts, prescribing-identity reports, doctor queue rows/counts/oldest selection and recent review-history streams use `lib/data/staff-query-scope.ts`: non-production E2E reads require both an `E2E-` reference and `exclude_from_reporting=true` in the database query. This also applies to an ordinary doctor opening the bare dashboard; capability and actor filters remain in force. The shared navigation cache separates synthetic and ordinary scopes. Vercel production/preview cannot activate this test scope, and explicit ordinary admin seed opt-ins remain available. `filterSeededE2EIntakes` alone is an inclusion toggle, not synthetic-only isolation; a selected synthetic request does not bound an unrelated layout query. These markers isolate synthetic data across runs, so old synthetic ciphertext still needs its own exact-record diagnosis.

**Server-action acceptance uses durable state.** Clinical note and clarification tests match the exact action request and successful response, then assert the saved note or exact message/status and visible result. RSC revalidation may keep the response stream open after the mutation is saved; stream EOF is not a persistence boundary. Keep the durable assertions when changing response synchronization.
...
jobs:
  ...
  e2e:
    ...
    steps:
      ...
      - playwright test --project=chromium e2e/admin.ops-index.spec.ts e2e/marketing-dashboard-nav.spec.ts e2e/dashboard.keyboard-safety.spec.ts e2e/landing-pages.spec.ts
      ...
      - pnpm build                        # PLAYWRIGHT=1 and NEXT_PUBLIC_PLAYWRIGHT=1 at build time
      - PLAYWRIGHT_SERVER_MODE=production playwright test --project=chromium --retries=0 --grep="$PLAYWRIGHT_PRODUCTION_CASES"
...
**Browser evidence retention:** each app Playwright invocation writes separate report/result directories (`ops`, `medcert`, `paid-clinical`, `paid-clinical-mobile`, `checkout-resume`, `production-regressions`). Both artifact parents upload even when the job succeeds, retaining failed-attempt traces from tests that pass on retry. Desktop and mobile paid-clinical runs both execute, and either failure fails their step.

**Compiled browser regressions:** CI runs seven selected scenarios from a production bundle with `--retries=0`. Three established retry-dependent UI scenarios cover multi-section clinical-note persistence, explicit request/profile comparison, and mobile Certificate details → Symptoms navigation; the existing ED FAQ case provides no-JavaScript first-paint proof. Three newly demonstrated retry-dependent cases cover merged duplicate-profile handling after med-cert payment, explicit empty saved-profile state, and rejection of a Parchment webhook with no signature. `PLAYWRIGHT_PRODUCTION_CASES` owns the shared selection and exclusion from the development runs. Build after all development suites, immediately before this group, because `next dev` also writes `.next`. Build and runtime both receive `NEXT_PUBLIC_PLAYWRIGHT=1` and matching public Supabase/app settings; the compiled runtime also receives the required Parchment signing secret and `INTERNAL_API_SECRET` mapped to the existing `E2E_SECRET`, matching the med-cert development lane. The latter signs patient-access links after certificate approval; the test route's own E2E authorization does not replace that downstream dependency. `PLAYWRIGHT_SERVER_MODE=production` selects `next start`; the default stays `next dev`. Assertion deadlines remain unchanged. All other Stripe/Parchment webhook cases and the development-chunk failure test retain their existing server mode and provider guards. These owned synthetic scenarios do not provide hosted-payment or real-prescription evidence.

The CI E2E job records a shared 75-minute browser deadline inside a 78-minute job limit, reserving three minutes for diagnostics. `playwright.config.ts` applies the remaining budget to each invocation, allowing interrupted suites to write reports and traces before artifact upload. The compiled-regression build uses that same remaining deadline with GNU `timeout`, terminating its process group on expiry and escalating after ten seconds. An exhausted or missing deadline fails before starting the build. The full matrix passed in 45.5 minutes in run `34374443028`, but run `34384679478` exhausted the former 47-minute browser budget with 30 desktop cases, the mobile case and guest recovery still outstanding. All completed cases passed, including five recovered retries; the unfinished stages are not passing evidence. The matching remaining workload took about six minutes in the passing reference run. The larger bounded allowance accommodates observed CI duration variance without changing individual test timeouts, retries, assertions or shared-fixture serialization. Completion on the adjusted head still requires CI. Exhausted later invocations fail before clearing output; they are never treated as passing or skipped coverage. Local runs without that deadline retain their existing behavior.
...
**Lighthouse gates** (cadence updated 2026-09-24): scheduled/manual CI fails its Lighthouse job on accessibility, SEO, FCP, and CLS; Lighthouse no longer runs on each PR. LCP and TBT are warning-only in the general localhost Lighthouse config because simulated throttling on GitHub runners is too noisy for untouched marketing pages. The dedicated mobile `/request` Lighthouse gate hard-gates stable paid-intake metrics (FCP ≤2s, TBT ≤300ms, CLS ≤0.05) while keeping composite performance score and simulated LCP warning-only.

**Opt-in money-page performance profile:** `lighthouserc.money-pages.json` covers the 10 public decision pages plus `/request` at 390x844/DPR3, runs each route three times with applied DevTools throttling, and asserts the median LCP ≤2.5s plus median CLS ≤0.1. Applied throttling measures the paint produced by the throttled browser. Keep default simulated/Lantern runs as a separate diagnostic when investigating dependency graphs; on a loopback production server, resources that finish before the very early observed paint can be over-included in Lantern's synthetic critical tail and must not be presented as the browser's measured render delay. The profile is intentionally not wired into the general CI job: build the production app, ensure `lhci` is installed, then run `lhci autorun --config=./lighthouserc.money-pages.json`. Results stay local under `.lighthouseci/money-pages/`. Treat this as production-build performance proof; do not use `next dev` timing as a release claim.
