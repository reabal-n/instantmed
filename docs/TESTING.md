...
The CI E2E job records a shared 47-minute browser deadline inside its unchanged 50-minute job limit. `playwright.config.ts` applies the remaining budget to each invocation, allowing interrupted suites to write reports and traces before artifact upload. The initial 45-minute budget cut off the final guest recovery case after the earlier stages passed; the retained report established the capacity gap, and both artifact uploads completed in five seconds. The revised budget allows two additional minutes for the remaining workload while reserving three minutes for diagnostics; completion on the adjusted head remains subject to CI. Exhausted later invocations fail before clearing output; they are never treated as passing or skipped coverage. Local runs without that deadline retain their existing behavior. Per-test timeouts and retries are unchanged.
