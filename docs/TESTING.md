...
The CI E2E job records a shared 45-minute browser deadline inside its unchanged 50-minute job limit. `playwright.config.ts` applies the remaining budget to each invocation, allowing interrupted suites to write reports and traces before artifact upload. Exhausted later invocations fail before clearing output; they are never treated as passing or skipped coverage. Local runs without that deadline retain their existing behavior. Per-test timeouts and retries are unchanged.
