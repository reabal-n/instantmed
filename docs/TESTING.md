...
**Compiled browser regressions:** CI moves three previously retry-dependent scenarios into a production-bundle run with `--retries=0`: multi-section clinical-note persistence, explicit request/profile comparison, and mobile Certificate details → Symptoms navigation. The same run checks ED FAQ visibility with JavaScript disabled. `PLAYWRIGHT_PRODUCTION_CASES` owns the shared selection and exclusion from the development run. Build after all development suites, immediately before this group, because `next dev` also writes `.next`. Build and runtime both receive `NEXT_PUBLIC_PLAYWRIGHT=1` and matching public Supabase/app settings. `PLAYWRIGHT_SERVER_MODE=production` selects `next start`; the default stays `next dev`. Assertion deadlines remain unchanged. The Stripe/Parchment webhook suites and development-chunk failure test retain their existing server mode and provider guards; they are not compatible with this production subset. These owned synthetic scenarios do not provide hosted-payment or real-prescription evidence.
