...
**Staff test reads must be scoped before decryption.** Navigation counts and prescribing-identity reports use `lib/data/staff-query-scope.ts`: non-production E2E reads require both an `E2E-` reference and `exclude_from_reporting=true` in the database query. The shared navigation cache separates synthetic and ordinary scopes. Vercel production/preview cannot activate this test scope. `filterSeededE2EIntakes` alone is an inclusion toggle, not synthetic-only isolation; a selected synthetic request does not bound an unrelated layout query. These markers isolate synthetic data across runs, so old synthetic ciphertext still needs its own exact-record diagnosis.
