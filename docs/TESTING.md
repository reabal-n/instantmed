...
- `monitoring-observations`, `browser-observer` and `monitoring-source-contract` exercise synthetic source timestamps, 359/360/365-minute silence, durable grace, running after failure, skipped execution, late completion/reruns, malformed/future/HTTP/timeout evidence, cache/request limits, finite coverage-gap recovery, explicit failure/recovery and persistence fail-open. The contract preserves seven production cases and local `/api/draft` + `/ingest` isolation; these tests make no live provider/browser calls.
