...
Scope is deliberately narrow: a one-off, form-first doctor assessment (operator decisions D-A..D-E, `docs/plans/2026-08-07-weight-loss-launch-plan.md`). Phentermine is within prescribing scope (operator correction 2026-09-24); routing does not guarantee a prescription. Continuation is a new consult, never a subscription. **No paid advertising** — `docs/ADVERTISING_COMPLIANCE.md` requires its own approval for any weight campaign.
