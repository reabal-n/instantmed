...
|-----|----------|----------|---------|
| Health Check | `/api/cron/health-check` | Every 5 min | Cron heartbeat watchdog only; queue delay, delivery failures, and business alerts stay owned by their dedicated jobs. A database-backed deployment marker gives never-run jobs 30 minutes after first watchdog observation, then missing, stale, and explicit non-`ok` outcomes are unhealthy. Each job/outage generation is claimed atomically in `operational_metrics` before one Sentry alert; concurrent watchdogs emit only newly claimed jobs, and a later successful heartbeat rearms the next outage. |
