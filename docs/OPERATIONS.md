...
**Release receipt — September 15 Sydney (September 14 UTC):** required CI [34848385130](https://github.com/reabal-n/instantmed/actions/runs/34848385130) passed on `dc4f4d5b857bde5ec2aec78ca76b463d66382426`; #564 merged normally at 14:55:18 UTC. Build, Lighthouse and Supabase Preview passed. E2E groups passed 8 ops, 78 medical-certificate, 118 paid-clinical, 1 mobile handoff, 5 signed-resume and 7 production-regression cases, plus one retry-dependent prescription-handoff case. That case initially retained the patient-entered medicine label beyond its expected inferred-match wait and passed on retry; cause is not established. This is not a zero-retry full run.

Vercel production deployment `dpl_HuKZQJaaY8HJvGyAqMvsAFVX9LKH` (`instantmed-qkllfljig-rey-project.vercel.app`) is **READY**, with exact source `f97eebdb833bf82009def30ec73f9be3ea4e3b8c` and `instantmed.com.au` plus production aliases attached. At 15:00 UTC, an isolated anonymous Chrome context checked Mounjaro, Monjaro, Wegovy, Ozempic and Duromine against the live repeat form. With ordinary fields filled synthetically, including a diabetes indication, each refused Continue and exposed the dedicated assessment action, with no keep-as-repeat escape. Duromine stated it was not offered; its action opened the actual weight assessment. Actual viewport was 500×900, light mode; no horizontal overflow or console error was observed. This does not replace the prior local 390×900/light and 1280×900/dark evidence or physical-phone acceptance. No identity, payment, account or prescribing submission occurred. Recovered Medicare/IHI editing has local/CI evidence, not authenticated production recovery proof.

Receipts are retained in the main checkout's ignored `output/weight-routing-proof/` and `output/checkout-identity-proof/`; the merged implementation branch/worktree was removed only after content-verified copying. Rollback is a governed application revert, preserving the already-applied catalog row and patient/payment state. No environment change accompanied release. Original patient causation, hosted payment, provider delivery, owner visual acceptance, physical-phone input, the existing fixed monitoring window and recipient-inbox delivery remain separate gates. Plan 5 navigation and the broader security/checkout audit remain separate workstreams.
...
- `docs/CLINICAL.md` owns clinical governance. Medical-certificate protocol issuance is active and bounded by a reviewed code-owned pre-issuance policy. The database feature flag can stop issuance but cannot widen the protocol; concerning or uncertain requests still require a doctor outcome before issue.
...
3. Doctor review builds a doctor-facing prescription context from the intake answers. An authenticated server-only resolver first exact-matches patient wording against InstantMed's curated `medications` table. If unresolved, a doctor who owns or previously reviewed that intake (or an admin) may receive one unique narrow typo match from the same patient's non-cancelled prescription history, but the prior name must still exact-resolve through the curated table before it is copyable. A further authorised clinician-only spelling check considers one unique narrow catalogue candidate; AI may only veto that candidate, never invent a name. It uses the existing clinical provider with a stripped name fragment only, an eight-second timeout and the AI rate-limit bucket. The panel labels the AI spelling match and keeps the original wording visible. No patient text is sent to an external medication reference service. Unknown or ambiguous names remain for manual review.
...
- The Parchment panel may copy only a locally verified generic medicine name. It must not copy strength, form, dose, directions, or the full request paragraph. A prior-prescription match is labelled and still requires confirmation in Parchment. Unknown, ambiguous, unauthorised, or non-unique names have no copy action; the doctor searches the patient entry manually.
...
- Patient-profile prescribing deliberately has no request. Its SSO URL sends the fixed non-PHI `IM-PATIENT-PROFILE` marker, and a successful webhook stores the prescription on the patient with `intake_id = NULL`; it must never claim an awaiting intake or manufacture `script_sent`.
...
- `/admin/ops` counts only unresolved Parchment sync work. The overview and Parchment recovery page read the full seven-day failure/retry window in stable 500-row pages (5,000-row safety cap per source) before reconciliation. A capped, failed or changing scan reports unavailable coverage rather than an all-clear. Exact synthetic webhook fixture identities from the local E2E suite are excluded at read time; a test-looking SCID alone is insufficient and audit rows are preserved. Category ages are calculated before the shared recent-feed cap. A retry with a linked request must match its SCID, current patient's Parchment link, and reviewing doctor. A callback or retry without request correlation requires one active patient profile for the external patient; duplicate links require identity review and must not fall back to the historical partner profile. A successful `parchment_webhook_retry` audit receipt clears that failure. A legacy `intake_correlation_invalid` row is displayed as **Direct prescription synced** and removed from recovery counts only when an intake-less prescription proves the exact SCID and same patient; the immutable failure audit row remains intact. Unlinked `patient_not_found` events remain historical evidence rather than retry work. Only retries of recorded script-completion failures may restore `script_sent`; history sync alone cannot unlock completion.
- An in-flight callback returns HTTP 503 with `Retry-After`; a Redis lock is not a success receipt. Each attempt releases only its own lease, including after an error. Subsequent deliveries use durable database evidence for idempotency.
- Prescription history refresh preserves the recorded patient, request and prescriber. The doctor viewing history is not inferred to be its prescriber. Conflicting ownership stops the sync for that prescription; other history items may still refresh. Verified callbacks can fill missing links, with conditional updates protecting simultaneous refreshes.
- **Record sent script** is the audited fallback for a prescription already issued in Parchment or another channel. It requires a reference or delivery channel, records sent evidence and attempts the patient notification immediately. A notification failure is shown to the doctor. **Complete request** remains a separate explicit action. A failed portal-open connection offers **Try Again** and must leave the current review available.
- Doctor notes continue saving in `awaiting_script`. Draft and decision saves are serialized; changing queue cases or closing the review sheet flushes edits first and keeps the review open if saving fails. Browser unload warns while edits remain unsaved.
- A database error while checking an existing callback reference returns retryable HTTP 503. Script evidence updates compare the stored reference and reject conflicting or concurrently replaced references. Exact-reference history recovery follows up to ten pages of fifty prescriptions, rejecting repeated/missing cursors; it never substitutes another prescription.
- Script notification status comes from `ensureScriptSentNotification`: sent, queued and failed are distinct. Failed attempts retry the same claimed outbox row and provider key; pending/sending attempts are left to their delivery owner. Bounced/complained/suppressed attempts remain terminal. New sends use an intake-scoped idempotency key. Notification failure never erases script evidence, and a successful history retry never claims the doctor completed the request.
...
| Doctor queue P95 | Below 2 hours across the 24/7 operating window |
...
| `consult_subtype` | Active: `ed`, `hair_loss`, `womens_health`, `weight_loss` | Consult category |
...
| Business Alerts | `/api/cron/business-alerts` | Every 30 min | Aggregates business metrics: failed payments, no-purchase revenue safety, Google Ads purchase-import health, **Google Ads upload-stream stall** (`google_ads_conversion_uploads_stalled`: paid orders exist but zero successful server-side uploads reached Google in 3d — the "absence of success" detector that complements the failure counter and catches a Data-Manager-only stall the click-id-gated `purchase_imports_zero` alert cannot see), certificate/email-outbox failures, auth email send failures (`auth_email_delivery_failed`), SLA breaches, prescribing-fulfilment stage breaches. Paid repeat-Rx, ED, hair-loss, and women's-health requests page as **critical aggregate alerts** when they remain approved without prescribing evidence for more than 120 minutes, remain in an opened Parchment handoff for more than 30 minutes, or have script evidence without patient-notification evidence for more than 10 minutes. Fulfilment payloads contain only stage, count, SLA, and oldest age — never intake IDs, reference numbers, patient data, medication data, or clinical answers. Auth-email alert payloads contain only the aggregate failed count and one-hour window; recipient hashes, domains, and provider error text stay out of Sentry, PostHog, Telegram, and the cron response. The Google Ads upload-audit reconciliation that feeds the `google_ads_upload_audit_source_anomaly` signal is **production-scoped** (`runtime_source !== "node"`) — local-dev / CI rows are never counted as orphans (see runbook below). Stale, low-volume audit-source residue is info-only; fresh or high-volume orphaning remains critical in Sentry and admin surfaces. Every check runs **per-section fail-soft** (`lib/monitoring/alert-sections.ts`): a broken query in one section becomes its own critical `business_alert_section_failed_<section>` alert instead of throwing to the outer catch and silencing every later section plus the Sentry dispatch — and section query errors are surfaced, not swallowed as "no alert". |
| Stale Queue | `/api/cron/stale-queue` | Hourly | Alerts on paid intakes waiting > 4h (warning) or > 8h (critical), and sends delay updates. Overdue priority requests do not trigger refunds (operator decision 2026-09-13). Historical priority refunds already submitted to Stripe remain reconcilable; no new priority reservation, replay or successor is allowed. |
| Refund Reconciliation | `/api/cron/refund-reconciliation` | Every 5 min | Claims bounded durable refund attempts. It retrieves a known Refund, discovers an unbound same-attempt Refund, or replays only the exact original idempotency generation inside the safe window; then persists exact balance evidence, reconciles cash, reserves/cancels the exact patient notice, and finalizes downstream ownership before optional Ads work. A single evidence-backed successor may repair an unmet cumulative decline/priority obligation. Ambiguous identity, mode, or exhausted-generation cases are never guessed: they remain money-blocking, service-role-only manual-review issues and make the heartbeat unhealthy. |
...
| PostHog Reconciliation | `/api/cron/posthog-reconciliation` | Hourly (:15) | Compare the last 24h reportable ever-paid intake population (`paid_at` in-window and payment status in the shared revenue-purchase set) with distinct valid UUIDv4 `flow_instance_id` values on PostHog `purchase_completed_server` events (`is_e2e=false`). Retry events therefore do not inflate coverage, and malformed/legacy flow identifiers remain outside canonical truth. Sentry alerts on >10% drift; "critical" past 30%. Requires `POSTHOG_PROJECT_API_KEY` (PostHog personal API key with `query:read` scope) + `POSTHOG_PROJECT_ID` (numeric, `277439`). `normalizePostHogApiHost()` maps ingestion hosts such as `us.i.posthog.com` / `eu.i.posthog.com` to the authenticated query API host; using an ingestion host for `/api/projects/*` returns 403 and blinds this monitor unless normalized. Missing credentials or query API 401/403/404 now return 503 and persist `configuration_error`; success is recorded only after both sources reconcile, so the heartbeat watchdog emits one durable alert for the continuous outage. |
| Google Ads Conversions | `/api/cron/google-ads-conversions` | Hourly (:45) | Backfills paid conversion uploads from Supabase truth using captured click IDs and/or hashed first-party identifiers plus stored `paid_at`. Data Manager is the primary uploader when enabled; the legacy Ads API remains the fallback. Refund and durably terminal-lost dispute cash create monotonic desired-state **RESTATEMENT** claims for aggregate retained AUD value. Revenue preserves exact zero; Ads receives a reversible A$0.01 floor so later refund reversals or dispute reinstatements remain representable. Expired identifiers are dropped when enhanced data remains; click-window expiry is terminal. `CONVERSION_NOT_FOUND` is retryable inside the 72-hour match grace and becomes `resolved_not_counted` only after the grace proves Google did not count the order/action. Local development writes no conversion upload/adjustment audit rows. |
| Google Ads Upload Stream Alert | `/api/cron/business-alerts` | Every 30 min | Alerts when paid orders exist but zero successful server-side Google Ads uploads reached Google in the lookback window, and when successful uploads continue but partial upload failures reach the configured failure-rate threshold. Uses production audit rows only; local `runtime_source=node` rows are ignored. |
| Google Ads Diagnostics Watch | `/api/cron/google-ads-diagnostics-watch` | Hourly (:50) | Watches a configured fresh upload after its processing window. For Data Manager uploads, set `GOOGLE_ADS_DIAGNOSTICS_WATCH_REQUEST_ID` and `GOOGLE_ADS_DIAGNOSTICS_WATCH_UPLOADED_AT` from the latest Data Manager `requestId`; for legacy uploads, use `GOOGLE_ADS_DIAGNOSTICS_WATCH_JOB_ID`. Compares Data Manager request status, Google Ads offline diagnostics, and production audit reconciliation, and fails loudly if the configured upload is rejected, pending, accepted while primary purchase conversions remain zero, or invisible without a clean production audit success. A success-only production audit with no expired-click or skipped rows is classified as stale Google diagnostics instead of a conversion-pipeline failure. |
| Google Ads Daily Brief | `/api/cron/google-ads-daily-brief` | `22:00` + `23:00` UTC; Sydney-hour guard accepts the full 09:00 local hour and sends once | Claims the unique closed Sydney `report_date`, reconciles fee-aware campaign economics and fail-closed tracking health, stores the aggregate snapshot, and sends the essential PHI-free Telegram brief. The full-hour guard tolerates platform scheduling and cold-start delay; the unique report-date claim prevents duplicate delivery. Production-only; no-ops unless `GOOGLE_ADS_AGENT_DAILY_BRIEF_ENABLED=true`. A failed send remains retryable; a delivered Telegram `message_id` is stored as the durable receipt. If Telegram accepts a send but that receipt cannot be persisted, the run is marked ambiguous and automatic retry is blocked to avoid a duplicate brief. |
| Paid Acquisition Contribution Alert | `/api/cron/business-alerts` | Every 30 min | Pages only when the latest delivered Ads evidence is no older than 36 hours, tracking is `GREEN`, every enabled/paused/other portfolio has complete economics, rolling-30-day spend is at least $200, and account-wide first-order contribution is negative. Query or record-integrity failures become `business_alert_section_failed_ads_contribution`; absent, stale, non-GREEN, or incomplete evidence is reported as unavailable and never called healthy. Aggregate cents and order counts only; no campaign, keyword, click, intake, patient, or medicine data and no Ads mutation. |
...
| Review Request | `/api/cron/review-request` | `00:00` + `23:00` UTC; Sydney-hour guard sends once around 10 AM local | One neutral ProductReview ask, 48h after confirmed document/eScript fulfilment. Daily finder catches up for 120d, excludes every durable outbox owner before limits, applies a 30d patient cooldown, and re-checks authoritative state, recipient, bounce/suppression, preference, and timing immediately before provider delivery. Confirmed send and terminal suppression have separate markers. The actual Sydney send run records an outcome heartbeat: successfully persisted `patient_cooldown` deferrals are expected, while all other transient blocks, failed deferral writes, provider failures and reconciliation failures remain unhealthy. `requestExpectedDeferrals` is a subset of the unchanged `requestTransientlyBlocked` count and is not counted twice in processed items. The job is watchdog-monitored at a ~25h threshold; the guard-only daylight-saving slot does not record a heartbeat, so it cannot hide a missed send run. |
...
| Retry Auto-Approval | `/api/cron/retry-auto-approval` | Every 3 min | Sends delayed-review reassurance independently. Retries active bounded protocol processing for eligible clean one-to-three-day work, study, and carer requests after the effective code-owned delay; recovers stale `attempting` intakes and draft generation after 10 minutes. Concerning, uncertain, unsupported-purpose, soft-flagged, and attention-flagged requests route to a doctor before issue. The database feature flag can stop or narrow issuance but cannot widen the code-owned ceilings of 3 per five minutes / 10 per day. |
...
## Payment reconciliation alert ownership

September 24 repair: `daily-reconciliation` owns aggregate payment/fulfilment integrity alerts. The cron opts out of per-request warnings to avoid duplicate pages; admin recovery reads retain warnings for genuine older integrity failures. Its 26-hour window overlaps the daily cadence and two-hour critical-age threshold so young records are reconsidered at the next run. Seeded profiles and explicit E2E intake markers are excluded; a real request is not hidden merely because `exclude_from_reporting` is set. A paid request awaiting review is pending queue work, not a payment mismatch. Consults in review or awaiting information are in progress, not delivered. Missing Stripe traceability, failed refunds and failed post-approval delivery remain actionable.

The separate `business-alerts` stale-human-queue monitor retains the existing 24-hour maximum and now includes `consult` alongside legacy `consultation`; explicit E2E references are excluded there too. No clinical decision, refund, payment or delivery state is rewritten by this repair. The September 23 event contained one genuine request at about three hours awaiting review and one ten-hour E2E fixture without Stripe references. Read-only September 24 evidence found the real requests completed and the fixture declined. CI/deployment and the next natural cron run remain the release proof; do not resolve an issue merely to hide recurrence.
...
- **Bidding value is retained revenue:** the server purchase import is Primary, browser purchase tags and funnel milestones are Secondary/non-bidding, and refund/dispute adjustments keep Google Ads aligned to **Net Retained Purchase Value** through PHI-safe `google_ads_conversion_adjustment` audit rows.

### Google Ads diagnostics audit join failure

The upload-audit reader retains its 1,000-row window and production-source filter, but validates distinct intake IDs in sequential batches of 100. A single PostgREST `in(id, ...)` request grows with the ID population and can exceed the gateway request-line limit, returning a generic `400 Bad Request`. On 2026-09-22, a read-only production reproduction failed with 664 IDs and succeeded with 100; the patched reader completed the same audit window. Every batch must succeed before reconciliation returns, and missing joins remain anomalies. Do not hide a failed batch as an empty result or disable the diagnostics heartbeat: the consequent `Critical cron job overdue` alert correctly reports failed work until a successful deployed run records recovery. Regression coverage lives in `lib/__tests__/google-ads-report.test.ts`.
...
Google Ads API access is assigned to the Cloud project owning the OAuth client, following Google's September 9, 2026 migration. The shared client and integration preflight omit developer-token headers and do not require `GOOGLE_ADS_DEVELOPER_TOKEN`. Before removing the obsolete deployment secret, deploy and verify tokenless reporting, conversion-action preflight and the conversion/adjustment paths. Confirm the OAuth project's access level in Google Cloud and keep its Owner/Editor contact addresses current. A quota-project header does not replace approval of the OAuth client's project. See [Google's migration guide](https://developers.google.com/google-ads/api/docs/api-policy/developer-token).

Internal automation remains permitted under [Google's developer policies](https://support.google.com/adspolicy/answer/6169371?hl=en). Verify that the approved use case covers campaign management as well as reporting; submit the tool-change form if it does not. Retain the operator-approval controls below.

For secrets masked by Vercel exports, run `pnpm check:integrations --runtime-credentials` after deploying `/api/internal/integration-readiness`. The CLI authenticates with `CRON_SECRET` over HTTPS; the endpoint returns only OpenAI model-access and Twilio voice-configuration results, with caching disabled. It never exports keys, places calls or sends messages. A missing endpoint, authentication failure or incomplete response remains a failure. A disabled voice switch is reported as disabled, not proof of active voice readiness. The OpenAI probe confirms model lookup access, not inference or realtime-call success. Ordinary `check:integrations` checks the local environment; it is not deployed-runtime evidence.
...
**2026-08-08/09 missed-run evidence:** no `google_ads_agent_runs` rows exist for either closed report date; delivered runs resume on 2026-08-10. The route and schedule did not change during the gap, the Vercel audit trail shows no InstantMed environment-variable edit, and the expired runtime logs cannot distinguish a scheduler miss from the early-return paths that previously acknowledged invocation before terminal work. Record the cause as unrecoverable rather than inventing one. The route now records `ok` only after the delivered Telegram receipt is durable; disabled, outside-window, already-delivered, in-progress, ambiguous-delivery, and thrown outcomes are recorded explicitly under the watchdog policy above.

Tracking health contains only evidence that can affect conversion measurement or an operating decision. Failures from audit-only account queries remain visible under deep-audit completeness evidence but do not degrade the scale gate. Campaign-level cross-service attribution remains visible for investigation; the September 9 owner decision removes service-purity graduation as a profitability veto. Actual attribution faults remain blocking. Diagnostics/conversion lag alone may be AMBER with scaleAllowed=true when every critical financial and attribution input is trusted.
...
The scale authorization reader enriches the latest delivered run in memory with fresh canonical cash-ledger and first-customer-order evidence for that run's exact rolling window, plus the matured first-order cohort's 60-day repeat cash for the Scripts ceiling (`docs/REVENUE_MODEL.md` §4); cohort evidence is null when unavailable and the ceiling then uses window cash. It records source run, read timestamp and window; it never edits the historical run. All campaign spend is charged to first orders for the first-order diagnostic; the scaling qualification and the cash break-even ceiling use campaign contribution, which includes repeat orders from campaign-attributed patients. Missing exact cash or actual fee evidence cannot qualify profitable scale; missing first-order identity evidence is reported as unavailable and stays advisory. Fresh campaign cash, contribution and fees replace the stored totals before authorization, with actual fees validated for repeat purchases as well as first purchases independently of identity/history classification. A failed or incomplete fresh cash-ledger, purchase-row, actual-fee or spend read for the campaign under authorization aborts the read with `scripts_scale_financial_evidence_unavailable` (recorded on the validation or apply receipt) instead of sizing the step on the stored run's older cash; identity and purchase-history gaps only leave the first-order diagnostic null. The fixed margin, order-count, purity and post-change delays are superseded by `docs/REVENUE_MODEL.md`; actual resource drift, unresolved ambiguous writes, integrity failures and exact spend bounds remain blocking.
...
The restricted operation union supports creation of responsive search ads, positive exact/phrase keywords, a bounded shared-exclusion repair, and a complete specialty Search campaign in one atomic Google Ads request. A campaign packet contains one private budget, one enabled Manual CPC Search campaign, Australia-presence and English criteria, the required enabled `IM | Never Serve` shared list, 1-5 enabled ad groups, one responsive search ad per group, and 1-20 enabled exact/phrase keywords per group. Campaign creation fails closed when that shared list is absent or missing any code-owned paid medicine exclusion. The `shared_negative_list` repair can attach only that exact list and atomically add only the exact code-owned exclusions missing from the fresh baseline. Search Partners, the Display Network, positive broad match, enhanced CPC, medicine-name targeting, and advertiser-curated health audiences are excluded. The complete campaign and every shared-list repair must fresh-read back exactly before the proposal is verified; rollback pauses the affected campaign through a new approval-gated proposal.

The Women's Health campaign launched as Google campaign `24144825264` at A$20/day with a A$3 maximum CPC. Its launch ad groups were UTI Assessment and Contraception Assessment, both using `/womens-health`; subsequent allocation and bid decisions are recorded in the dated September sections below. The copy retains possible-call and doctor-decision wording. Proposal `ADS-20260818-17` aborted before mutation on baseline drift; the byte-identical packet was rebound to the fresh baseline as `ADS-20260818-18`, validated, applied in one atomic Google request, and fresh-read verified on 2026-08-18. RSA packets require 3-15 unique headlines, 2-4 unique descriptions, an allowlisted InstantMed paid destination, service-matched pricing, and possible-call wording for prescribing pathways. Positive keyword packets prohibit broad match, medicine names, more than 10 words, and duplicate live criteria. Negative keyword packets may contain medicine names because they prevent serving rather than target a query; they remain exact-approval mutations. Enabled creations are scaling changes, so they also require a fresh GREEN tracking gate. All creations are verified from fresh account content rather than response ordering.

**2026-08-24 paid-search hardening receipts:** proposal `ADS-20260824-01` attached the canonical `IM | Never Serve` list to Women's Health and added the 20 code-owned exclusions missing from that list; apply and fresh read both verified. Hair Loss stayed enabled at A$10/day while proposal `ADS-20260824-04` paused only its losing phrase keyword; original proposal `ADS-20260824-02` had aborted on baseline drift before mutation. Scripts stayed on Maximise Conversion Value with tROAS 1.5 while proposal `ADS-20260824-05` raised only its daily budget from A$60 to A$79; original proposal `ADS-20260824-03` was terminally aborted on baseline drift after the same operation was rebound. A separate final account read confirmed all three campaigns enabled, the requested Hair Loss keyword paused, the Scripts budget at A$79/day, the Women's Health list attached, and zero missing code-owned exclusions. Sequential rebinding was required because each verified account mutation intentionally invalidated proposals signed against the prior whole-account baseline.

Keyword policy exemptions are never guessed or hidden. A campaign packet may carry only an exact exemptible key returned by Google's first failed validate-only request, with the same violating text, and the trusted approval summary displays every requested exemption. Current specialty campaign packets allow only `HEALTH_IN_PERSONALIZED_ADS`, `BIRTH_CONTROL`, and `PRESCRIPTION_DRUG_SALE`; a non-exemptible or different policy result aborts the launch.

Campaign sitelinks and callouts use the restricted `campaign_text_asset_create` packet. Exact text and allowlisted public destinations are stored durably in the immutable proposal and bound by its SHA-256 operation hash. Each asset and enabled campaign association are created in one atomic Google request with partial failure disabled; an unsuccessful request cannot leave a newly created unlinked asset. Fresh verification matches the exact content, destination and campaign association, independently of response order. A mismatch remains failed for investigation; do not invent an inverse target. Rollback pauses the one verified association through a new approved packet, retaining the asset for audit. The packet is a material asset change, binds to the actual campaign/service and requires the tracking gate. Existing campaign asset links may still be paused, enabled, or removed through their restricted family. The same text-asset packet can associate an existing `TEXT` asset as `BUSINESS_NAME`, restricted to the exact `InstantMed` identity, verified live content, the same customer, and no existing non-removed business-name association. This reuses the asset rather than creating a duplicate; readback and rollback bind to the exact campaign association.

Account call links support only reversible `ENABLED`/`PAUSED` transitions through `asset_link_status`. Their mandatory account-level read participates in the baseline hash. An account-scoped rationale, exact `CALL` link and asset, and each enabled campaign’s experiment lock are checked. Restoration additionally requires an approved asset, GREEN tracking and audience checks across every enabled campaign; a rejected call asset cannot be restored as a generic rollback. Account asset creation, removal, other field types and contact-number edits remain unsupported.

Image assets, account/ad-group asset creation, raw image bytes and transient local paths remain unsupported. Do not use a text-asset packet to bypass image storage, rights, policy or approval requirements.

**2026-09-25 asset repair:** `ADS-20260925-01` paused the two disapproved certificate sitelinks and two Scripts sitelinks with a missing destination section. `ADS-20260925-05` created the exact approved Work Absence Review and Certificate Eligibility sitelinks; `ADS-20260925-06` created How eScripts Work pointing to `/prescriptions#prescription-lifecycle-title`. Apply and fresh content/association verification succeeded. This proves saved configuration, not Google policy approval or serving. The verified carer certificate destination is also accepted for certificate RSA creation; cross-service routing remains blocked.

The owner authorised closing observations `EXP-20260922-01`, `EXP-20260919-01` and `EXP-20260919-02` as inconclusive to prioritise account repairs. Their records are `stopped` with a retained-configuration closure receipt and the original end dates preserved. Bids and budgets were not rolled back or increased. Subsequent results cannot be attributed solely to those original bid/budget changes. Replacement packets `ADS-20260925-02` and `ADS-20260925-03` failed the active experiment lock without a Google mutation; byte-identical replacements were rebound as `05` and `06` after closure.

**2026-09-25 approved copy pack:** source packets `ADS-20260925-07` through `14` received exact owner approval. Fresh-baseline, byte-identical packets `15` through `19` applied and verified the unsupported return-to-work keyword pause, five Women's Health sitelinks plus eight callouts, one UTI RSA, three Scripts RSAs, and four certificate RSAs. Each replacement RSA has 15 headlines and four descriptions. At the fresh 04:59 UTC read, all eight replacement RSAs and the eScript sitelink were `APPROVED_LIMITED`; Certificate Eligibility was `APPROVED` and Work Absence Review was `APPROVED_LIMITED`. After checking each exact replacement again, byte-identical retirement packets `20` through `22` paused the old UTI ad, three old Scripts ads and four old certificate ads. Fresh status readback verified all eight retirements. This establishes Google eligibility and saved configuration, not exposure or improved revenue. Subsequent asset cleanup and the contraception hold are recorded below. These changes did not alter bids, budgets, schedules or conversion values.


**2026-09-25 follow-on cleanup:** the owner approved both scopes, source packets `ADS-20260925-23` through `30`, in the same Codex task. Byte-identical fresh-baseline packets `31` through `38` applied and verified all 34 operations: two replacement certificate sitelinks, six callouts, the existing InstantMed business-name association for Women’s Health, 16 obsolete/duplicate sitelink pauses, two unsuitable snippet pauses, retirement of the certificate no-call callout and rejected account call association, and the contraception ad plus four pill-specific keyword pauses. Certificate retirements applied last, after both exact new links were policy-eligible. UTI and the two generic contraception keywords remain enabled; no clinical service was removed. The contraception ad must remain paused until its paid destination and replacement copy are reviewed and verified through a new approved packet. This approval did not cover restoring the old ad, publishing a new website route, increasing spend or changing bids. The subsequent owner instruction to repair the destination is implemented on PR #606: `/contraception-assessment` replaces the old acquisition URL with a permanent redirect, consultation-level page/metadata/FAQ copy and unchanged `ocp_new` clinical routing. This implementation is not production or Google eligibility evidence; the new ad still requires its own exact approved packet and live destination checks.
...
4. Google Ads stays blocked until production-scoped runtime proof shows `GOOGLE_ADS_CONVERSION_ACTION_PURCHASE` is an enabled offline click-import `UPLOAD_CLICKS` purchase action and purchase imports/diagnostics agree with Supabase/PostHog/Stripe truth. If Data Manager rollout is enabled, this proof must include a fresh Data Manager `requestId`, `requestStatus:retrieve` result, and matching production `google_ads_conversion_upload` audit row.
...
2. Create webhook endpoint `https://<domain>/api/stripe/webhook` subscribing to `checkout.session.completed`, `checkout.session.expired`, `checkout.session.async_payment_succeeded`, `checkout.session.async_payment_failed`, `payment_intent.payment_failed`, `charge.refunded`, `refund.created`, `refund.updated`, `refund.failed`, `charge.dispute.created`, `charge.dispute.updated`, `charge.dispute.closed`, `charge.dispute.funds_withdrawn`, and `charge.dispute.funds_reinstated`. Do not enable invoice or `customer.subscription.*` events while the one-off model is active; those runtime handlers are retired. Do not enable the deprecated `charge.refund.updated` event; the canonical Refund lifecycle events supersede it.
...
| Approved/completed medical certificate but no current valid certificate | Any reportable paid or partially refunded med-cert intake, regardless of age, with no current `issued_certificates.status = 'valid'` row | `ops_approved_certificate_missing_record` |

**Escalation:** The `stale-queue` cron monitors paid requests still waiting for review through PostHog and Sentry, and sends an hourly aggregate-only Telegram reminder while one or more paid requests older than 30 minutes remain queued. Essential CRITICAL-severity business alerts also reach Telegram; warning-severity and per-case SLA detail remain in Sentry/admin surfaces.
...
**Manual "Mark Sent Manually" now notifies the patient (2026-07-19).** The manual external-prescribing fallback (`markScriptSentAction`) recorded script evidence and emailed the patient nothing — three patients (26 May, 28 May, 31 May 2026) paid, had a real script written, and heard nothing from InstantMed; one received no email at all. It now calls the same idempotent `sendScriptSentEmailIfNeeded` the approval path uses. **The Parchment webhook deliberately still does not email**: it fires on a machine event, so notification waits for the doctor's explicit approval in `approvePrescribedScriptAction` — that two-step workflow is intact. The manual path differs because the doctor's own click, after prescribing externally, *is* the explicit confirmation and no second automated event follows. The outbox lookup inside the helper prevents a later approval from double-sending. Email failure is non-fatal (Sentry-captured) so an outage cannot discard durable script evidence. Pinned by `lib/__tests__/script-sent-contract.test.ts`.

**Owner-written scripts are out of scope for reorder marketing (2026-07-19).** The owner-doctor writes scripts directly in Parchment for people they know personally. Those `prescriptions` rows carry a `patient_id` but **no `intake_id`** — 22 of 52 rows as of 2026-07-19. The refill-reminder candidate query filtered only on `status = 'active'`, so it treated them as reorder prospects and sent four commercial nudges (2026-06-28, 2026-07-13) to recipients who never bought anything. `findRefillReminderCandidates` now requires `intake_id IS NOT NULL`; pinned by `lib/__tests__/refill-reminder.test.ts`.

Scope note: the exclusion belongs to **marketing** only. Clinical and patient-facing surfaces — patient script history, the doctor's patient record, `renewal-detection`, the Parchment ops recovery list — must keep showing every script regardless of origin, because hiding a prescribing history from the treating doctor is the worse failure. No business/revenue metric needs a matching change: revenue and conversion derive from `intakes`, which an owner-written script never creates, so those counts already exclude them by construction.

Direct patient-profile prescriptions use the `IM-PATIENT-PROFILE` webhook marker and remain intake-less. Older direct prescriptions that arrived with invalid correlation metadata are classified as recovered only after the exact SCID is present on the same patient's intake-less prescription row; audit history is not edited or deleted.

**Telegram critical-alert delivery (operator decision 2026-09-08).** `lib/monitoring/critical-alert-dispatch.ts` pages each critical aggregate incident once after successful delivery; `critical-alert-cooldown.ts` owns receipts and the outage fallback. It pages again for a new category, increased severity, any count rise relative to the last accepted observation (including 5→4→5), or recurrence after verified recovery. Unchanged conditions, improving counts, changing age/copy and elapsed time do not re-page. The dashboard, periodic metrics and Sentry incident classification remain active; paid-request and queue-reminder notifications retain their separate policies. Aggregate severity/count define an incident here: replacing one affected case with another at the same count between observations is not distinguishable by this monitor.

The persisted incident `at` is its last alertable transition; top-level `checkedAt` still advances on every accepted poll and owns stale-poll ordering. Successful Telegram sends create `audit_logs.action = critical_business_alert_telegram` with exact numeric `incident_metric` / `incident_at` and a content hash, never alert prose or patient data. Matching this originating token prevents a delayed old delivery from acknowledging a recurrence. Failed sends have no receipt and retry on the next normal observation; failed receipt writes mark the cron partially failed and may duplicate delivery. A superseded poll does not send. Receipt lookup errors fail open. If incident state is unavailable, explicit state errors remain visible and the legacy text cooldown is retained as a bounded fallback: four hours normally, seven days for terminal Ads adjustment risk, including historical copy aliases. This is not an exactly-once sender; overlapping sends or failed receipt persistence can duplicate a page.

An explicit operator acknowledgement uses the distinct `critical_business_alert_acknowledged` audit action for the exact accepted numeric metric/transition token. It must compare the current snapshot and known aggregate condition before insert, must not fabricate a successful send, and does not change the clinical incident or establish recovery. Legacy text-only receipts cannot safely establish an incident token and are not automatically migrated; a pre-existing condition can send once after rollout unless its exact current token is acknowledged. Application rollback restores the prior timed policy while preserving all monitor and audit evidence; no schema or environment change is required. Patient delay emails keep `patient_delay_email_hours` (default 2h), and Sentry severity becomes critical when 5+ intakes breach that threshold.

**Certificate obligation monitor:** `/admin/ops` surfaces a critical **Cert missing record** invariant for reportable paid or partially refunded medical-certificate intakes across all history that are approved/completed but have no current `issued_certificates.status = 'valid'` row. Superseded- or revoked-only certificate histories remain missing because they do not fulfil the current delivery obligation. Use the certificate delivery rescue panel first; do not resend a link until a current valid certificate exists. The rescue detail is intentionally bounded: it combines the recent 14-day cohort with up to 5,000 historical terminal candidates and raises an incomplete-coverage alert if that cap is reached. The exact invariant count remains the fail-closed signal beyond that detail cap.
...
**Kill switches:** `DISABLE_INTAKE_EVENTS=true` (disable event logging), `DISABLE_STUCK_INTAKE_SENTRY=true` (disable stuck intake Sentry warnings). `DISABLE_RECONCILIATION_SENTRY=true` disables per-record recovery-page mismatch warnings only; aggregate cron integrity alerts remain enabled.
...
- **Synthetic**: GitHub Actions runs `e2e/prod-request-flow-synthetic.spec.ts` against `https://instantmed.com.au` when the existing Vercel cron dispatches it on `17 */2 * * *` (every two hours UTC). This is not observed five-minute coverage. It clicks the med-cert certificate type, duration, and start-date controls and smoke-checks repeat script, prescription, ED, hair-loss, and women's-health request paths. The browser helper `e2e/helpers/production-synthetic-isolation.ts` fulfills `/api/draft` and `/ingest` locally; the production probe must not create `partial_intakes`, recovery emails, or PostHog funnel events.
...
### Trustworthy browser and incident monitoring

Vercel requests twelve browser runs daily at minute 17 of every even UTC hour through `/api/cron/browser-check`; the existing GitHub workflow retains its eight-minute timeout and exact seven entry-flow cases. GitHub native scheduling remains as an independent best-effort fallback at minute 47 of every even UTC hour; both the former five-minute request and subsequent two-hour request produced multi-hour gaps, so Vercel is the primary scheduler. This retains a browser execution path when the production application cannot run its own dispatcher or watchdog; GitHub Actions status remains independent, while timely Sentry paging during a total Vercel outage is not established. The dispatcher records GitHub API version `2026-03-10`'s returned `workflow_run_id`, not an inferred run title or a caller-supplied ID. No new provider, database migration or browser runner is introduced. Deploy only after configuring `GITHUB_BROWSER_MONITOR_TOKEN` in production with access limited to this repository and Actions read/write. The cron requires the existing secret, production environment and Vercel cron user-agent; the user-agent is an additional context check, not authentication. Manual workflow dispatch remains available for diagnostics. A dispatch response is acceptance evidence only; the browser step still owns execution success. It does not exercise hosted Stripe payment. The controller's 24-hour sample ending 2026-09-06 06:30 UTC found nine successful completions under the former five-minute request, with 98.18 / 118.15 / 264.45-minute minimum/median/maximum gaps. Run 34013038436 had seven passing tests; its named browser step completed at 05:05:13 UTC. This is dated pre-change evidence; new cadence needs post-deployment observation. GitHub documents that scheduled work can be [delayed or dropped under load](https://docs.github.com/en/actions/how-tos/troubleshoot-workflows); scheduling off the hour does not guarantee delivery.

The existing Vercel health-check polls every five minutes independently of GitHub. Workflow and job reads reuse `GITHUB_BROWSER_MONITOR_TOKEN` when configured, keeping production observation off the shared anonymous GitHub quota. The token stays on the fixed GitHub repository API origin and redirects are rejected; a failed authenticated request never retries anonymously. Unconfigured local readers may still inspect the public repository. HTTP 403 with exhausted-quota or retry-after headers is classified as rate limiting; other HTTP refusals remain `http_error`, with the existing bounded backoff retained. Collected browser-evidence failures attach the bounded `observer_reason` code to the existing active `observer_unavailable` Sentry transition. Raw provider responses and error text are excluded. Outer state/receipt/persistence failures can still emit that incident without a reason; this instrumentation does not establish the cause of historical unavailable polls. `lib/monitoring/browser-observer.ts` uses the bounded provider reader `lib/monitoring/browser-evidence.ts` to read only the public `reabal-n/instantmed` exact workflow on `main`, then the exact run/attempt job `request-flow-synthetic` and step `Run production request-flow synthetic`. Invocation, queued/running work, latest completed browser evidence, latest success, latest failure, observer checked-at and observer availability remain separate. Timestamps are provider-sourced epoch milliseconds. Invocation event codes preserve GitHub native scheduled runs (0) and workflow dispatch (1). Only dispatch IDs found in the ten most recent durable `browser_scheduled_dispatch` receipts can also establish scheduled coverage. Receipt reads are bounded; missing or unreadable provenance stays unknown. Unreceipted manual completions prove execution, not cadence. Browser outcome codes are 0 (no verified execution), 1 (success), 2 (failure); workflow status codes are 0 (queued/waiting), 1 (running), 2 (completed). Skipped execution never becomes success, and later cleanup outcome cannot rewrite the browser step. Seven-case source coverage is separate from provider step success. A healthy observer with ten contiguous, completed run proofs ending at its known invocation establishes the current lookback floor. A strictly older API list does not reopen completed first attempts below that floor. Unknown IDs/attempts inside the window and all older reruns remain eligible. Incomplete windows, coverage gaps and unavailable observers cannot use this exclusion. Scheduled freshness continues aging from real completion evidence; this bounded monitor does not establish absence of arbitrary historical failures.

`browser_failed`, `browser_stale` and `observer_unavailable` have independent stable Sentry fingerprints. A known scheduled completion becomes stale at 150 minutes even while another run is queued/running or GitHub is unavailable. If no completion has ever been observed, durable enablement time supplies the initial grace. Known failure is detectable at the next successful poll; scheduled silence is nominally detectable at two and a half hours plus 0–5 minutes when scheduled evidence is known. These are alarm targets, not scheduler guarantees. `completedAt` separately retains the highest verified completion from either event type as actual browser execution/availability evidence; scheduled-cadence freshness must derive from retained verified first-attempt native scheduled or receipted cron-dispatch completions only (`attempt: 1`). GitHub reruns retain the original trigger while incrementing the attempt, so a rerun can recover execution failure without refreshing scheduled cadence. Apply this rule to already-cached evidence as well as new reads; preserve original event/attempt metadata and immutable receipts. Newer run number/attempt owns outcome recovery; an older run finishing late cannot erase a newer failure. Verified later success recovers failure while preserving its history. An already-old discovered scheduled success remains stale.

One list request and at most two uncached attempt-job reads per normal poll mean nominally 12 requests/hour and at most 36/hour before retries. A single shared retry budget permits one additional request per poll only after a transport failure, including a response-body connection failure; the retry keeps the same fixed origin, credentials and redirect rejection. Persistent transport errors, HTTP refusals (including rate limits), malformed JSON and invalid evidence remain degraded. HTTP and parse failures do not consume retries. At configured cadence the maximum is 48 requests/hour, without pagination; overlapping/manual invocations and shared egress can consume more quota. Fetches retain 2.5-second aborts, bounding GitHub reads to 10 seconds within the route's 20-second allowance for concurrent cron work, persistence and a two-second Sentry flush. Rate limits retain bounded reset/retry backoff. Reads reuse the configured repository-scoped token as described above; no credential change is required. The retry is transport resilience, not browser-success evidence: only verified named-step completion can recover browser outcomes.

The ten-run window and immutable attempt cache bound work. More uncached jobs keep observer status unavailable while later polls resolve them. Immutable attempt cache entries also receipt notification processing: an older failure discovered while draining the cap still emits failure/recovery against the newest verified outcome, without regressing current health. Losing overlap with prior invocation emits a coverage-gap observation; a subsequent fully verified overlapping window establishes a new boundary and recovers observer availability. This does not reconstruct missing history or clear current browser failure; prior gap snapshots remain recorded. The runs request filters `branch=main`, and accepted evidence additionally requires `head_branch: main` plus the exact workflow path, so the reviewed feature-branch poisoning claim was not reproduced. No new provider/token or Checkly integration is needed; Checkly access remained unverified after controller reads returned 401.

`20260906100000_monitor_observation_state.sql` seeds enablement once and adds `append_monitor_state(text,bigint,jsonb)` over existing `operational_metrics`. Fixed state keys, numeric/boolean-only bounded payloads, a unique key/version index and a transaction advisory lock with expected-version check serialize append claims. Service role retains only SELECT/INSERT table privileges and exclusively receives RPC execution. Losers reload at most twice; unavailable/malformed state fails open. Every accepted observation appends aggregate state even when paging is suppressed. No metrics retention job currently exists; future retention must preserve the current highest-version snapshot per key, including immutable enablement time. Missing state cannot mint new grace.

Business alerts group by fixed incident category; cron outages group by job. New category, increased severity, any count rise relative to the last accepted observation (including 5→4→5), and recurrence after verified recovery notify. Unchanged incidents are suppressed. Successfully evaluated empty sections recover; failed, partially readable and unavailable sections retain unknown state. Google purchase recovery requires successful preflight and zero query errors, then only predicates reached by the priority-exclusive builder are known. Setup faults mask import/primary predicates; zero imports mask the primary-conversion predicate. A masked incident stays active until its own predicate is evaluated clear. Independent alert categories continue to classify separately. PostHog evaluations continue. Telegram uses the same accepted incident identity with separate successful-delivery or explicit acknowledgement evidence; its timed detail cooldowns are only an incident-store outage fallback, as described above. Claims precede capture: a crash between claim and capture/flush can lose delivery. Bounded Sentry flush is not a provider receipt or exactly-once guarantee.

### Plan 1 operating boundary and open acceptance

The [Plan 1 execution receipt](superpowers/plans/2026-09-06-01-checkout-data-reliability.md#execution-receipt) is the single ledger for exact schema application, source and CI heads, merges, READY deployments, smoke checks and dated monitoring observations. [SECURITY.md](SECURITY.md#historical-profile-repair--completed-2026-09-07) separately owns the aggregate production profile-data receipt. This runbook owns only the current operating rules, rollback boundary and outstanding acceptance work.

- Keep the additive monitor-state rows, service-role-only append RPC, SELECT/INSERT-only service-role table grants and browser denial. Application rollback restores the prior routes and workflow while retaining that evidence. A separately approved schema rollback may drop the append RPC and partial unique index only after old code is restored; preserve snapshot rows. No schema change or backfill is required for the dispatcher; its Actions token is a separate production credential gate.
- The Vercel browser dispatcher requests twelve runs daily at minute 17 of every even UTC hour, with the existing eight-minute GitHub timeout. The independent observer runs every five minutes and evaluates 150-minute scheduled-execution freshness. `browser-check` dispatch heartbeat is separately watched at 150 minutes; a healthy dispatch does not clear a stale or failed browser outcome. GitHub may delay or drop scheduled jobs, so requested cadence is never coverage proof.
- Unreceipted manual dispatch and reruns of older scheduled jobs prove browser execution only. It must not establish scheduled cadence, mint a scheduled-green state or automatically recover a retained stale incident. If the bounded retained evidence has completions but no known scheduled completion, cadence is unknown and the observer is unavailable until scheduled proof returns; a bounded `unavailableReason` may appear only in the authenticated health response and is never persisted. This acceptance rule is part of the approved follow-up and must not be claimed as released until focused source proof, required CI, deployment and sequential failure/recovery observation are complete.
- Only a durably persisted `patient_cooldown` with a future retry time is an expected review-request deferral; provider, persistence, policy, reconciliation and unknown failures remain unhealthy. The observed cooldown incident justifies that reporting contract. A successful source change cannot clear an older stored failure; a later normal scheduled outcome owns recovery.
- The identical-digest PostgreSQL registry mirror fallback remains justified by observed public ECR HTTP 429 failures; it may retry image retrieval, never a failed test or a different database image. The two expired Google Ads conversion targets remain actionable, and the March superseded-only certificate incident remains preserved and unsent.

Cadence acceptance still requires requested slots, actual scheduled invocations, named browser-step completions, completion gaps, observer availability and any failure/recovery or freshness transition across a full post-release window. Keep hosted Stripe payment proof and Sentry provider delivery separate. Do not invoke patient-send routes, reset historical heartbeat state or create a scheduled task to manufacture acceptance.
...
**Telegram carries request notifications plus bounded operational sends.** Automatic sends are: a newly paid request; a PHI-free Medical Director voice-message alert after a durable caller-confirmed record exists; an hourly count-only reminder while paid requests wait for review; an at-most-hourly count-only reminder while voice messages remain unresolved; and essential CRITICAL-severity business alerts using the same aggregate detail string as Sentry. A voice alert contains category, received time, and an authenticated admin link. Patient name, callback number when requested, and confirmed summary remain field-level encrypted in `medical_director_voice_messages` and are prohibited from Telegram. New medical-certificate titles stay neutral until the real routing outcome is known; approval, decline, or manual-review routing edits the original message instead of sending another. Those lifecycle edits are awaited, with five-second timeouts on both the intake lookup and outbound Telegram call, and remain fail-soft. Telegram is deliberately not a general second monitoring channel. Warnings, routine metrics, and cron watchdog failures stay in Sentry/admin surfaces. Patient identity, contact or government identifiers, medicines, free-text descriptions, symptoms, notes, payment details, and intake answers are prohibited. The support Inbox bridge is manual-only and disabled in production: there is no scheduled support-inbox cron or Gmail reader. The `CRON_SECRET`-protected `/api/internal/support-inbox-alert` receiver remains only for deliberate aggregate-count diagnostics and requires `TELEGRAM_SUPPORT_INBOX_ALERTS_ENABLED=1`; production keeps it `0`. If its Telegram send fails, the receiver records `delivery_failed` and returns HTTP 502.
...
| `/api/cron/health-check` | Critical cron heartbeats and independent browser freshness | Sentry capture when essential scheduled jobs are overdue; current state is also visible in the admin system-health surface. |
...
- **Twilio AI voice (default OFF)**: `TWILIO_AI_VOICE_ENABLED=false`, `TWILIO_AUTH_TOKEN` (the Account Auth Token used to validate `X-Twilio-Signature`, not an API-key secret), `TWILIO_VOICE_PUBLIC_BASE_URL=https://instantmed.com.au`, a random `TWILIO_VOICE_SESSION_SECRET` of at least 32 characters, and optional comma-separated `TWILIO_VOICE_BLOCKED_CALLER_HASHES`. Enabling also requires `OPENAI_API_KEY`, `PHI_MASTER_KEY`, all three PHI encryption flags set to `true`, and configured Upstash Redis for caller rate limiting and the four-call concurrency cap.
...
Lena opens exactly with `Hi, this is Lena from InstantMed support. How can I help?` She listens to the issue, collects the patient's full name when possible, asks no more than two short clarifying questions, reads back one concise summary, and saves it only after confirmation. She does not request or retain date of birth. She asks for a callback number only when the patient explicitly wants a return call. The application speaks the success line only after the encrypted database write succeeds. Lena never diagnoses, triages, changes an outcome, discloses patient information, or promises a fix, approval, refund, prescription, certificate, callback time, or outcome.

When the Medical Director returns a requested call, present `0495 049 555` as the outbound caller ID or use withheld caller ID. Never expose the retired private support number.

### Rollback

For the fastest kill, first repoint or disable the number's incoming-call handler in the Twilio Console. Then set `TWILIO_AI_VOICE_ENABLED=false` and redeploy so the application path also fails closed to the contact-page fallback. Existing encrypted voice messages remain available for Medical Director follow-up; rollback never deletes them.
...
| `0495 049 555` | 24/7 voice message support for the Medical Director |
...
Detects category-level breaches of the 24h max review SLA. Per `docs/REVENUE_MODEL.md` §5 the target is "median below 30 minutes, P95 below 2 hours across the 24/7 operating window." 2026-05-23 evening snapshot: **med cert work P95 = 165h (7x over 24h max); max = 14 days**.
...
### September 19 profit and auction-access tests

The owner directed execution after the Scripts sales-drop diagnosis in Codex task `01a0a806-83fa-7360-a710-a62995b4f70d`. Both changes used the existing immutable proposal, exact Codex decision receipt, fresh baseline, Google validate-only, atomic apply and provider read-back path. The local mutation switch was enabled only for these explicit CLI executions; hosted environment flags and automatic mutation behavior were not changed.

| Proposal | Resource and change | Applied UTC | Google request ID | Read-back UTC |
|---|---|---|---|---|
| ADS-20260919-01 | Scripts `23870042807`: target ROAS 1.35 to 1.50; AUD 120/day retained | 2026-09-18T14:23:05.198Z | `-SILrmzXlLuxcWqLUblyfg` | verified 2026-09-18T14:24:03.135Z |
| ADS-20260919-02 | Women's Health ad groups `202440288434` and `202440288474`: maximum CPC AUD 3 to AUD 4; AUD 50/day retained | 2026-09-18T14:25:03.117Z | `OsThBv1sacfbW6BOfhkw4g` | verified 2026-09-18T14:25:52.908Z |

The first Scripts apply attempt stopped at the disabled local mutation switch before an account write; the same approved proposal then applied successfully with the process-scoped switch. There was no duplicate mutation. The source run was the delivered GREEN September 17 report, with fresh cash/first-customer evidence at validation. Fifteen missing Stripe fee-cache rows were refreshed through the existing provider reader; no payment or refund was initiated.

Women's Health's three-day spend was AUD 64.30 against two first orders and AUD 33.32 first-order contribution. Its 30-day first-order contribution was AUD 377.47 from 18 first orders after AUD 490.12 advertising spend and AUD 21.46 actual fees. Main keywords were below first-page bid, inherited the AUD 3 ad-group limit, and recent daily rank-lost share was about 70–73% with zero budget-lost share. AUD 4 is a bounded auction-access test, not a predicted marginal profit. The service CPC policy now uses Women's Health's own ceiling for both group bids and Manual CPC strategy changes; ED and Hair Loss retain AUD 3. No keyword, audience, destination or ad copy changed.
...
Current certificate average daily budget is A$30. Actual single-day spend can exceed an average daily budget. The A$20 exposure lasted only about 20 minutes and cannot establish comparative performance. The owner subsequently superseded short-window reviews with 14–28 completed days: September 23–October 6 and September 23–October 20, excluding September 22 partial exposure. REVENUE_MODEL owns this correction. Evaluate CPA and campaign contribution over that window before preparing a routine inverse proposal; no automatic action is authorized. A$30 → A$20 rollback needs an exact new approved proposal. No recurring monitor was created and no keyword, ad, destination or clinical setting changed.

### September 22 certificate keyword allocation and conversion-job diagnosis

The owner approved the exact certificate keyword packets in Codex task `01a0bef4-906b-7c30-b208-151894b1aa61`. `ADS-20260922-08` paused `medical certificate online` PHRASE and `doctor certificate online` EXACT in ad group `197380155482`. Apply request `4BtCyp2bPp23WHKYE9U5-A` succeeded at 07:42:58.063 UTC; read-back verified at 07:44:35.430 UTC. The approved addition packet `ADS-20260922-09` was rebound byte-for-byte to the fresh account baseline as `ADS-20260922-10` after the pauses; the old packet was not applied. The new packet added enabled EXACT criteria for `no appointment medical certificate`, `get medical certificate online`, and `telehealth medical certificate` in the same ad group. Apply request `sqqgTg9NRjO2Pbo9bC5s7g` succeeded at 07:45:59.642 UTC; read-back verified at 07:47:08.006 UTC. Both apply operations used only the process-scoped mutation switch and fresh Google validation.

Independent final read-back confirmed both pauses, all three exact additions, the existing phrase winners still enabled, A$30/day budget and A$22 target CPA. August 23–September 21 cash attribution includes stored match type: the paused criteria lost A$68.47 after actual fees and spend; `doctor certificate online` PHRASE contributed A$61.53. `medical certificate today` PHRASE was approximately break-even; its already-paused exact variant explained most of the pooled loss. These are observed allocation candidates, not proof that exact variants create incremental sales. Follow the owner's 14–28 completed-day review rule; the September 22 keyword edits form part of the new test exposure. Rollback requires fresh inverse proposals to re-enable the two paused criteria and/or pause the three additions.

**Separate read-only conversion-job diagnosis, through September 22 07:45 UTC:** `google-ads-conversions` reports `partial_failure`. All 23 purchase-upload audit rows inspected since September 21 00:00 UTC were successful; this proves accepted uploads, not Google's final ad matching. One recent A$29.95 full-refund adjustment returns `CONVERSION_NOT_FOUND`, following an earlier Data Manager processing response. Its successful upload was September 22 03:06:58.863 UTC, so the application's 72-hour retry grace ends September 25 03:06:58.863 UTC. A direct local Data Manager status read was unavailable (`missing_env`); do not present eventual matching or adjustment success as verified.

The health view independently reports two expired adjustment targets: April refunds, A$19.95 each, exact retained cash zero, aged 161 and 172 days. Both lack adjustment claims; their latest legacy `CONVERSION_NOT_FOUND` evidence occurred within 72 hours of the last successful June 30 upload. One missed the post-grace threshold by about 13 minutes. They therefore do not qualify for the evidence-based historical-not-counted exemption and sit outside the cron's 54-day paid-date window. Maximum original value associated with these unresolved historical records is A$39.90; this is potential historical Ads reporting overstatement, not missing patient cash or proven Google-counted value. Unknown outcomes, expired reservations, stale pending claims and irreversible-zero discrepancies are all zero. No refund, claim, audit history, Google conversion or heartbeat was rewritten during this diagnosis. Keep the recent retry and the historical reconciliation exception distinct; do not clear the alert solely to make monitoring green.
...
### September 22 Google Ads adjustment recovery

At the owner's explicit request, the two expired April refund targets were reconciled through the canonical leased adjustment runner on September 22 at 08:10:51–08:10:53 UTC. Fresh Google Ads responses returned `CONVERSION_NOT_FOUND`, well past the original June uploads' 72-hour matching grace. Both durable claims now read `resolved_not_counted`, with the reversible one-cent Google target and exact retained cash zero. All five actionable claim-health counters read zero after recovery. These were operator-triggered backfills: audit metadata truthfully records `runtime_source=node` and `request_path=operator-reconciliation/2026-09-22`; no hosted-runtime impersonation, cash mutation, or fabricated success was used.

The recent A$29.95 refund remains retryable within its existing matching grace, ending September 25 at 03:06:58.863 UTC (13:06 Sydney). The hourly cron remains responsible for retrying it; do not shorten the grace or mark the adjustment successful. The monitoring follow-up returns `waiting_for_match` only for known Google `CONVERSION_NOT_FOUND` or Data Manager `PROCESSING` inside the existing 72-hour grace. Provider-failure audit rows and retryable claims are retained. Cron exposes `adjustment_waiting_for_match`; pending matching alone does not make an otherwise completed job an outage. Overdue processing, configuration/transport errors, uncertain outcomes and actionable claim-health counts continue to fail the job. Local Data Manager status lookup was unavailable; a fresh Vercel environment pull contains masked OAuth values, not usable credentials. That local `invalid_client` response does not establish a production credential failure. A fresh operator Google Ads adjustment attempt at 09:29 UTC still returned `CONVERSION_NOT_FOUND`. Purchase-upload acceptance is not proof of eventual Google matching.

Two preventive defects were reproduced and repaired: the due view's nullable legacy-zero exclusion hid targets with no prior claim, and cron passed the Ads floor instead of exact cash into adjustment audit metadata. Migration `20260922081547_google_ads_adjustment_exact_cash.sql` was applied and read back in production: it appends the exact cash column and makes the exclusion NULL-safe while retaining service-role-only access and uncertain-outcome guards. No recent uploaded adjustment targets lacked claims at diagnosis. One recent zero-cash target remains due. The application-side handoff and bounded waiting-state classification deployed with PR #597 at merge commit `c16df4cdeaf27d3568f13a4582e58440c4981edc`, verified against the ready production deployment serving instantmed.com.au. The September 22 10:45 UTC scheduled heartbeat is `ok`; the matching production audit records exact cash zero, the one-cent Ads floor, and the real unsuccessful provider response. This verifies healthy job execution with one pending adjustment, not adjustment success. A quiet six-hour follow-up tracks its final outcome and escalates if still unresolved after grace. Synthetic PostgreSQL tests cover full and partial refunds, first attempts without claims, completed/uncertain exclusions and access controls; CI runs them. Rollback: revert the application handoff if needed and leave the additive database column in place; any predicate rollback must be a new migration, never a rewrite of applied history. Historical audit rows are retained unchanged.


### September 22 request-entry performance repair

PR #597's mobile Lighthouse report attributed a roughly 95 ms task to the homepage entry while loading the certificate form. A fresh unchanged production build confirmed the initial HTML referenced `app/(marketing)/page-*.js`: the root layout imported the same client module used for marketing availability hooks. A layout-only `ServiceAvailabilityBoundary` now wraps the unchanged provider, giving the root a unique client reference without delaying or changing availability state or context.

Local production-build comparison, three Lighthouse runs per route with the existing mobile profile: certificate entry fell from 33 initial scripts to 17, with no homepage entry; total loaded JavaScript fell from 1,166,755 to 840,860 uncompressed bytes. Median simulated LCP fell from 4.82s to 3.47s; `/request` fell from 5.14s to 3.75s. Median TBT stayed small locally (certificate 7ms to 6.5ms; hub 6ms to 10ms), so this does not claim a measured CI TBT improvement. All six candidate runs passed unchanged assertions, including FCP, TBT, CLS and the warning-level LCP threshold. The existing production-format sticky-bar test now rejects homepage entry scripts and still advances certificate to symptoms. Browser checks confirmed mobile progression, zero horizontal overflow and no page errors; desktop service selection also rendered. This is lab evidence; CI and the new production deployment remain release gates. No threshold, clinical, pricing, consent or provider-availability behavior changed.


### September 24 Ads implementation controls

`GOOGLE_ADS_API_VERSION` production override is v24. A new deployment is required for an environment update to affect running functions. Authenticated `/api/internal/integration-readiness` reports the effective API version and release SHA; a mismatch fails readiness. This is configuration evidence, not provider acceptance. Verify hourly conversion/adjustment outcomes and the next daily brief independently; retain the 24-hour no-v22 evidence gate.

Browser Google tags and their conversion/funnel dispatch are disabled to contain health-context forwarding by connected tags. GA4 browser reports and Secondary browser purchase diagnostics stop collecting; server Primary purchase imports, refund restatements, first-party attribution and anonymous PostHog events remain. Do not roll privacy containment back merely to restore GA4 graphs. Cancellation capability URLs are excluded from external analytics, and Sentry strips Google-tag URL query payloads and resume tokens. Google documents that enhanced measurement can generate history events even with manual page views disabled: https://developers.google.com/analytics/devguides/collection/ga4/views .

Deep audit treats specialty budgets as ceilings, reports non-API/unknown-client edits for comparison against approval receipts, and flags truncated history. A provider `INTERNAL_TOOL` asset creation alone does not establish unauthorised spend. API-client edits still require receipt comparison; client type is not an actor allowlist. Exact resource/baseline validation continues to block drift at mutation time.

Campaign-negative proposals reject token/match-type coverage already supplied by an enabled campaign criterion, an enabled attached shared list, or all nonremoved ad groups. One ad group's exclusion does not prove campaign-wide coverage. Exact mutation verification and rollback still use exact resource state.

`pnpm ads:agent experiment:create --proposal=<key> --max-loss-cents=15000 --duration-days=14` records the verified launch time, explicit loss cap and duration. This command records a versioned sequential whole-campaign test; it does not create a Google split experiment. The metric is campaign contribution per retained order, not first-order contribution. Historical records with the former label remain readable without changing their cash. Existing overlap checks and mutation locks remain in force. Registering historical tests does not remove overlap, baseline contamination, or delayed-refund uncertainty; annotate those in the plan's evidence receipt before interpreting a result.
