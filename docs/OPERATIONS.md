...
2. **DONE (2026-05-29, expanded 2026-06-29 and 2026-07-01).** `/admin/ops` renders an "Integrity (weekly invariants)" strip for Review SLA backlog (Q1 proxy: paid intakes awaiting review past 24h), Paid + cancelled (charged but undelivered intakes still in reporting), Cert + refund orphans (Q2), Refund record anomalies (Q4), Cert timestamp drift (Q6, 14d), Cert missing record (recent approved/completed med-cert intakes without a generated certificate row), and Invariant query failures. Counts come from `getOperationalInvariants()` in `lib/admin/ops-invariants.ts` (service-role, seeded-E2E filtered, fail-soft). The paid-cancelled, cert-orphan, and cert-missing-record cards are critical on any non-zero count; SLA backlog goes critical at `SLA_BREACH_CRITICAL` (10); cert timestamp drift stays warning-only and links to the certificate delivery rescue panel.
