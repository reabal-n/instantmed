...
**Telegram critical-alert cooldown.** `lib/monitoring/critical-alert-cooldown.ts` fingerprints each critical detail independently (SHA-256, whitespace-trimmed), suppresses its repeat Telegram page for the default `CRITICAL_ALERT_COOLDOWN_HOURS` (4h), and receipts delivery in `audit_logs` under `critical_business_alert_telegram`. Terminal Google Ads refund-adjustment risk uses a targeted seven-day cooldown: the unchanged immutable failure set pages once during the same seven-day freshness window used by the alert builder, while the daily Ads brief continues to show RED tracking and block scaling. **Sentry uses separate incident state** (see monitoring section below); periodic metrics continue while unchanged Sentry incidents are suppressed. Telegram keeps its own cooldown and receipts. A changed count or a genuinely new alert has a new fingerprint and pages immediately without re-bundling unchanged historical signals. Both the check and the receipt are **fail-open** — a duplicate page is preferable to a swallowed critical. Patient delay emails use `patient_delay_email_hours` (default 2h), and Sentry severity becomes critical when 5+ intakes breach that threshold.
...
Business alerts group by fixed incident category; cron outages group by job. New category, increased severity, any count rise relative to the last accepted observation (including 5→4→5), and recurrence after verified recovery notify. Unchanged incidents are suppressed. Successfully evaluated empty sections recover; failed, partially readable and unavailable sections retain unknown state. Google purchase recovery requires successful preflight and zero query errors, then only predicates reached by the priority-exclusive builder are known. Setup faults mask import/primary predicates; zero imports mask the primary-conversion predicate. A masked incident stays active until its own predicate is evaluated clear. Independent alert categories continue to classify separately. Existing PostHog evaluations and independent Telegram four-hour/seven-day detail cooldowns remain intact. Claims precede capture: a crash between claim and capture/flush can lose delivery. Bounded Sentry flush is not a provider receipt or exactly-once guarantee.

**Production schema receipt — 2026-09-06:** after required CI passed on `f225c1d807997f91c28b6bb53f73d17934a57565`, the linked CLI applied the sole pending reviewed migration. Read-back at 11:08:06 UTC confirmed exact version `20260906100000` / name `monitor_observation_state`, two initial version-1 state rows, a valid/ready unique partial index and service-role-only empty-search-path RPC execution. Service-role table grants remain SELECT/INSERT without UPDATE; browser SELECT remains denied. The ACL checker returned zero violations, linked schema error lint passed, and the canonical migration-history checker reported aligned history. These checks establish schema application; deployed observer and cadence proof follow separately.

**Application release — 2026-09-06:** PR #527 passed exact-head CI `34027297774` on `f225c1d807997f91c28b6bb53f73d17934a57565` and merged as `f7b2e43aa3d126cb05020c0ea7b27e016feeb82d`. The primary production alias matched READY deployment `dpl_Foxpmwb6xNG5NeEQfbKWuSPYvkZw` at 11:12:08.402 UTC; post-deploy smoke `34029556180` passed. The initial read showed both seeded version-1 rows with no checked-at timestamp, which was not execution proof. Five scheduled polls at 11:15:08.115, 11:20:08.192, 11:25:07.825, 11:30:07.568 and 11:35:07.638 UTC then persisted versions 2–6, with 2/4/6/8/10 verified attempts. Observed poll gaps were 299.633–300.077 seconds. Enablement stayed at 11:07:55.608 UTC. The final state was available, without a coverage gap or active browser incident, and the startup-unavailable state recovered. The latest browser success was scheduled run `34024261028`, completed at 09:19:28 UTC; manual run `34021064088` remained distinct. A later read confirmed version 14 at 12:15:07.493 UTC with the same enablement, available observer and no active browser incident. This establishes observer runtime/freshness recovery while the new four-hour browser cadence still awaits its full-day measurement.

**Retained incidents:** the 11:30:08.585 UTC business poll persisted version 2 and one critical missing-current-certificate incident. A bounded aggregate check matched one March 2026 superseded-only history and zero missing-current-valid records from other periods; the March certificate remains unchanged and unsent. At 11:35 the combined health heartbeat remained partial_failure due to the separate `google-ads-conversions` and `review-request` jobs, whose unrecovered failed outcomes predate this release (10:45:23 UTC and 00:00:15 UTC). The healthy browser observer does not clear those jobs. Heartbeat metadata alone does not establish their root cause. The 12:00:09.211 UTC business poll retained the same single incident in version 3. No Ads mutation, outreach retry or status reset was performed. Preserve these operational follow-ups; do not use a source flush or quiet Vercel log window as Sentry delivery proof.

**Apply/rollback packet:** the reviewed additive migration has been applied through the normal engineering release protocol, before deploying observer code. The release controller owns production application under the existing Plan 1 authorization; historical profile repair was separately authorized, applied and verified on 2026-09-07; its execution receipt is in SECURITY.md. Confirm both initial state rows, function ACLs and partial unique index; do not reset existing state. Local proof uses disposable PostgreSQL. Application rollback restores prior health/business routes and workflow schedule; retain additive schema/state so evidence survives. If schema rollback is separately required after restoring old code, drop `public.append_monitor_state(text,bigint,jsonb)` and `public.operational_metrics_monitor_version_unique`; preserve snapshot rows. No environment changes or backfill are needed.
...
**Monitoring follow-up — released 2026-09-07; full cadence observation handed off:** the four-hour request crossed the six-hour freshness threshold twice: version 51 at 2026-09-06 15:20:07.697 UTC, recovered in version 53 at 15:30:07.738, and version 205 at 2026-09-07 04:10:07.494, with the observer still available and its latest verified browser completion at 2026-09-06 22:08:37 UTC. The three post-release scheduled runs observed through that read were `34042318145` (created 15:26:35; browser step 15:27:22–15:27:56), `34051180542` (created 18:16:21; step 18:17:14–18:17:48) and `34063034637` (created 22:06:24; step 22:08:03–22:08:37), all on 6 September. The earlier scheduled completion at 09:19:28 preceded deployment; manual runs are separate evidence. The two-hour request provides more scheduling opportunities: one missed nominal slot gives a four-hour gap instead of eight hours. It does not guarantee GitHub execution. The independent five-minute observer, 360-minute freshness threshold and durable enablement stay unchanged. PR #531 merged at 2026-09-07 05:18:53 UTC and its production deployment was READY at 05:21:27.386; required PR CI and post-deploy smoke passed. The [Plan 1 receipt](superpowers/plans/2026-09-06-01-checkout-data-reliability.md#september-7-monitoring-and-release-follow-ups) owns exact revisions and proof. Measure the two-hour window through September 8 at 05:18:53 UTC separately from the earlier interval, and read cooldown reporting after the next normal review-request run without invoking a patient-send route.

The same follow-up changes only review-request heartbeat classification. The 7 September 00:00 UTC batch recorded 13 sends and two transient blocks; a bounded receipt check matched both blocks to pending `patient_cooldown` rows with persisted future retry times. Only that durable expected outcome is excluded from heartbeat failures. Delivery gates, cooldown duration, pending retries and send behavior are unchanged. This source correction does not clear a stored failed heartbeat; only a subsequent scheduled run establishes runtime recovery. Google Ads conversion claim-health evidence and the separately retained certificate incident remain independent follow-ups.

**Cadence acceptance handoff (open):** Rey/operations owns a fresh 24-hour read at 2026-09-07 12:00 UTC (22:00 Sydney), more than 24 hours after the monitoring deployment reached READY at 11:12:08.402 UTC on 6 September. Later tooling/documentation releases do not reset the durable observer enablement. Record requested scheduled slots, actual scheduled invocations, named browser-step completions, min/median/max completion gaps, observer availability and any failure/recovery or freshness incident. Check deployed-state enablement survives release and manual runs remain distinguishable. One-off browser success does not close scheduled cadence acceptance. Sentry provider delivery remains unverified without restored issue access; flush/source receipts alone do not prove delivery. This is an explicit measurement handoff, not a new scheduled task.
