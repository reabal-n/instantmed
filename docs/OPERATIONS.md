...
**Telegram critical-alert cooldown.** `lib/monitoring/critical-alert-cooldown.ts` fingerprints the aggregate detail string (SHA-256, whitespace-trimmed) and suppresses a repeat Telegram page for `CRITICAL_ALERT_COOLDOWN_HOURS` (4h), receipted in `audit_logs` under `critical_business_alert_telegram`. **Sentry always fires** — only the Telegram page is cooled, so no recorded signal is lost. Escalation is never delayed: a changed count or an added alert type is a different fingerprint and pages immediately. Both the check and the receipt are **fail-open** — a duplicate page is preferable to a swallowed critical. Patient delay emails use `patient_delay_email_hours` (default 2h), and Sentry severity becomes critical when 5+ intakes breach that threshold.
