...
The restricted operation union supports creation of responsive search ads, positive exact/phrase keywords, and a complete specialty Search campaign in one atomic Google Ads request. A campaign packet contains one private budget, one enabled Manual CPC Search campaign, Australia-presence and English criteria, 1-5 enabled ad groups, one responsive search ad per group, and 1-20 enabled exact/phrase keywords per group. Search Partners, the Display Network, broad match, enhanced CPC, medicine-name targeting, and advertiser-curated health audiences are excluded. The complete campaign must fresh-read back exactly before the proposal is verified; rollback pauses the campaign through a new approval-gated proposal.
