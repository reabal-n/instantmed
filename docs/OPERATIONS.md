...
**Apply/rollback packet:** the reviewed additive migration has been applied through the normal engineering release protocol, before deploying observer code. The release controller owns production application under the existing Plan 1 authorization; historical profile repair remains separately approval-gated. Confirm both initial state rows, function ACLs and partial unique index; do not reset existing state. Local proof uses disposable PostgreSQL. Application rollback restores prior health/business routes and workflow schedule; retain additive schema/state so evidence survives. If schema rollback is separately required after restoring old code, drop `public.append_monitor_state(text,bigint,jsonb)` and `public.operational_metrics_monitor_version_unique`; preserve snapshot rows. No environment changes or backfill are needed.
