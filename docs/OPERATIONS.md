...
3. **The Google Ads re-page window keys off pageable failures only.** `latestPageableFailureAt` (terminal **and** click-attributed) drives the 7-day gate instead of `latestFailureAt`, which is a max across every adjustment failure. Previously any unrelated failure — for example a user-data-only retraction on a different order — re-armed the window and resurfaced a finding that had already gone quiet.
