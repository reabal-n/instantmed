...
**Telegram alerts** require `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` env vars. General system-alert helpers may silently skip when those credentials are absent. The support Inbox bridge is manual-only and disabled in production: there is no scheduled support-inbox cron or Gmail reader. The `CRON_SECRET`-protected `/api/internal/support-inbox-alert` receiver accepts only a bounded aggregate unread count for deliberate diagnostics. When its dedicated `TELEGRAM_SUPPORT_INBOX_ALERTS_ENABLED=1` flag is explicitly enabled but Telegram credentials are missing or delivery fails, it records `delivery_failed` and returns HTTP 502. It retains the distributed lock, four-hour same-count cooldown, and audit row, and must never receive sender, subject, snippet, message ID, thread ID, body, attachment, draft, or other identifiable email content. Paid request alerts remain independent through `/api/cron/telegram-notifications` and the `telegram_notifications_enabled` feature flag.
...
- **Alerts**: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_WEBHOOK_SECRET` (optional), `TELEGRAM_SUPPORT_INBOX_ALERTS_ENABLED` (`0` by default; retained only for deliberate aggregate-count diagnostics). Production support-inbox alerts stay off; paid-request notifications use the separate database feature flag `telegram_notifications_enabled`.
