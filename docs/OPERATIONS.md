...
| Post-auto-approval doctor review | Each auto-approved medical certificate must receive one individual doctor outcome within 24 hours | `med_cert_batch_review_overdue` |
...
**Post-auto-approval review runbook:** Open `/dashboard` and use **Review oldest certificate**. Review the issued certificate and clinical record, then either attest that no change is needed or revoke it with a clinical reason so the intake returns to manual review. Never bulk-update `batch_reviewed_at` or backfill it without individual review. The 24-hour threshold is an InstantMed governance control; the business-alerts cron sends an aggregate-only alert payload with pending count, overdue count, and oldest age, never patient or intake identifiers.
