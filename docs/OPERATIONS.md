...
**Kill switches:** `DISABLE_INTAKE_EVENTS=true` (disable event logging), `DISABLE_STUCK_INTAKE_SENTRY=true` (disable stuck intake Sentry warnings). The legacy `DISABLE_RECONCILIATION_SENTRY` flag is retired with the per-record warning emitter; aggregate cron integrity alerts remain enabled.
