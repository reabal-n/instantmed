...
3. Doctor review builds a doctor-facing prescription context from the intake answers. An authenticated server-only resolver may exact-match the patient wording against InstantMed's curated `medications` table and expose its generic name alone as a clipboard convenience. It makes no external medication lookup.
...
- The Parchment panel may copy only a locally verified generic medicine name. It must not copy strength, form, dose, directions, or the full request paragraph. Unknown or ambiguous names have no copy action; the doctor searches the patient entry manually.
