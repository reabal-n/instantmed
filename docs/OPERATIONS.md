...
6. For `review_request` and `partial_intake_recovery`, distinguish provider acceptance from terminal policy suppression. `*_email_sent_at` means durably recorded provider acceptance/send confirmation; `*_email_suppressed_at` means the current policy deliberately prevented delivery. Check the outbox `delivery_status` for webhook-confirmed delivery truth. Neither lifecycle marker is inferred from the other.
7. Partial-recovery rows may expose only `recovery_tracking_id` as business metadata. The draft `session_id`, service, and resume URL must not appear in plaintext outbox metadata or logs. A legacy row without an encrypted provider body fails terminally and alerts; corrupt ciphertext or a temporary policy/database read failure remains deferred on the same row.
8. If a provider send succeeded but its lifecycle marker is absent, do not retry the provider manually. The owning cron reconciles confirmed `sent` outbox proof to the marker without resending; investigate Sentry if reconciliation continues to fail.
...
| Partial Intake Recovery | `/api/cron/recover-partial-intakes` | Hourly (:15) | Anonymous unconverted-draft recovery, including review/checkout-step drafts. Initial eligibility is 60m-6h idle; submitted intakes remain owned by abandoned-checkout recovery. Provider retries revalidate authoritative draft, recipient, route, expiry, conversion, and suppression state before Resend |
...
| Review Request | `/api/cron/review-request` | `00:00` + `23:00` UTC; Sydney-hour guard sends once around 10 AM local | One neutral ProductReview ask, 48h after confirmed document/eScript fulfilment. Daily finder catches up for 120d (no 72h drop), dedupes once per request, applies a 30d patient cooldown, suppresses ineligible/delivery-suppressed recipients, and re-checks `canSendMarketingEmail` immediately before provider delivery |
| Review Request Backfill | `/api/cron/review-request-backfill` | Monthly (1st, 3 AM UTC) | Dry-run-only monitor over the same fulfilment-based 120d candidate path; a real send remains a manual `?dryRun=false` trigger (CRON_SECRET-gated) and reuses the same state, suppression, cooldown, consent, and outbox-idempotency gates |
...
**Escalation:** The `stale-queue` cron monitors paid requests still waiting for review through PostHog and Sentry, and sends an hourly aggregate-only Telegram reminder while one or more paid requests older than 30 minutes remain queued. Essential CRITICAL-severity business alerts also reach Telegram; warning-severity and per-case SLA detail remain in Sentry/admin surfaces. Patient delay emails use `patient_delay_email_hours` (default 2h), and Sentry severity becomes critical when 5+ intakes breach that threshold.
