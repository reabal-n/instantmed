...
| Stale Queue | `/api/cron/stale-queue` | Hourly | Alerts on paid intakes waiting > 4h (warning) or > 8h (critical). Also initiates generation 1 of the **priority breach refund** (operator decision 2026-08-03): a priority intake still `paid`/`in_review` 3h+ after `paid_at` gets a fee-only $9.95 Stripe refund attempt. Exact balance evidence stamps `priority_fee_refunded_at`; the dedicated refund-reconciliation cron owns same-key recovery and the one evidence-backed successor. A later decline tops up only the remaining cash. Core: `lib/stripe/priority-fee-refund.ts`. |
