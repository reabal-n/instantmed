...
One list request and at most two uncached attempt-job reads per normal poll mean nominally 12 requests/hour and at most 36/hour at configured cadence, without retries/pagination. Overlapping/manual invocations and shared egress can consume more quota; the public GitHub limit is not reserved for this observer. Fetches have 2.5-second aborts. Rate limits retain bounded reset/retry backoff; HTTP, timeout, malformed/future timestamp and persistence errors remain degraded. The route allowance is 20 seconds for up to 7.5 seconds of GitHub reads, concurrent cron work, persistence and two-second Sentry flush. The observer continues to use bounded public GitHub reads without credentials. Only the fixed dispatch endpoint receives the repository-scoped Actions token; it is never logged or sent to a caller-selected URL. An authenticated credential restricted to GitHub Actions read access is optional future resilience work, not a setup blocker, and no quota outage has been demonstrated.
