...
| Stale Queue | `/api/cron/stale-queue` | Hourly | Alerts on paid intakes waiting > 4h (warning) or > 8h (critical). Also runs the **priority breach auto-refund** (operator decision 2026-08-03): a priority intake still `paid`/`in_review` 3h+ after `paid_at` gets its $9.95 fee refunded via Stripe partial refund (idempotency key `priority_breach_<intakeId>`, once-only via `intakes.priority_fee_refunded_at`), a transactional `priority_fee_refunded` email, and `payment_status = partially_refunded`; the later approval email acknowledges the refund, a subsequent decline still tops up to a full refund, and the `charge.refunded` webhook suppresses its generic partial-refund email for these. Core: `lib/stripe/priority-fee-refund.ts`. |
...
| Google Ads Conversions | `/api/cron/google-ads-conversions` | Hourly (:45) | Backfill paid conversion uploads from Supabase truth using captured click IDs and/or hashed first-party identifiers plus the stored `paid_at` conversion time. When `GOOGLE_DATA_MANAGER_CONVERSIONS_ENABLED=true`, uploads use Google Data Manager API and audit the returned `request_id` / `upload_identifier`; otherwise the legacy Google Ads API uploader remains the fallback and audits `upload_job_id`. The configured purchase action must still preflight as an offline click-import `UPLOAD_CLICKS` action. The same cron also backfills `google_ads_conversion_adjustment` rows for **Net Retained Purchase Value**: full refunds and disputes retract the order, while partial refunds restate the retained AUD value. Expired click identifiers are dropped when enhanced user data is available; click-window expiry is terminal. `CONVERSION_NOT_FOUND` before the 72-hour upload grace remains retryable; once the upload passes grace that unresolved early miss blocks scaling until rechecked. A `CONVERSION_NOT_FOUND` response produced after grace means Google did not count that order/action and is receipted as `resolved_not_counted`, not paged as a mutation failure. A later success or resolution receipt supersedes earlier failed attempts. Local development runs skip `google_ads_conversion_upload` and `google_ads_conversion_adjustment` audit-log writes so a developer shell cannot contaminate production audit reconciliation. |
