...
| Stale Queue | `/api/cron/stale-queue` | Hourly | Alerts on paid intakes waiting > 4h (warning) or > 8h (critical). Also requests the **priority breach refund** (operator decision 2026-08-03): a priority intake still `paid`/`in_review` 3h+ after `paid_at` gets a fee-only $9.95 Stripe Refund request under a deterministic key. Creation stays `pending`; exact balance evidence stamps `priority_fee_refunded_at`, moves the payment to `partially_refunded`, and queues the patient notice. One durably failed Refund permits one bounded successor via `priority_fee_refund_retry_attempted_at`; a later decline tops up only the remaining cash. Core: `lib/stripe/priority-fee-refund.ts`. |
