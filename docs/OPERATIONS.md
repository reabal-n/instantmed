...
The owner approved medical certificates from AUD 20 to AUD 50/day under a AUD 22 target CPA in a Claude session (decision reference `codex-task:claude-48977fd4-5d38-4141-9a44-319f11a125ec`); the policy ceiling change ships in the same pull request as this record. Two exact packets follow the existing immutable proposal, decision receipt, fresh baseline, Google validate-only, atomic apply and read-back path: first `campaign_bidding` (Maximize Conversions with target CPA AUD 22, budget unchanged), then `campaign_budget` (AUD 20 to AUD 50/day). Receipts are appended below once each apply and read-back completes; until then the live account is unchanged. Review criteria are owned by `docs/REVENUE_MODEL.md`.
