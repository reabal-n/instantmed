...
|------|----------|----------------|---------------------------|
| 1 | **Truth and measurement gate** - reconcile canonical docs; repair public 24/7, automation, and 18+ contradictions; close the batch-review and synthetic-E2E boundaries; correct Google Ads dollar units; make optional-email lifecycle truth explicit and tracking aggregate-only. | **Complete 2026-07-12; communication lifecycle follow-up complete 2026-07-19.** The additive truth schema, review-request lifecycle, and partial-recovery candidate RPC are applied and verified. Partial-intake recovery now has authoritative initial/retry policy gates, encrypted bearer-only replay, all-status durable ownership before candidate limits, marker-first terminal suppression, and sent-only reconciliation. Production deployment `dpl_2Z2T5nEQLCojx9wup1Em7BW8Juky` for `main` merge `1f1251b0f60822eea309674b8654e447b37759e3` reached `READY` in `syd1`; `/api/health` and the homepage returned HTTP 200, with no production runtime errors or deployment error/fatal logs in the first-hour verification window. | Complete only when each named boundary has implementation plus focused proof. Re-open any closed sub-boundary when production evidence or an operator decision exposes drift. |
