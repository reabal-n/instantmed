...
>
> **Last refreshed:** 2026-07-19. Refresh whenever priority or status changes; perform a deliberate review at least monthly.
...
|------|----------|----------------|---------------------------|
| 1 | **Truth and measurement gate** - reconcile canonical docs; repair public 24/7, automation, and 18+ contradictions; close the batch-review and synthetic-E2E boundaries; correct Google Ads dollar units; make review-email consent fail closed and tracking aggregate-only. | **Complete 2026-07-12; analytics code boundary hardened 2026-07-19.** Canon ownership and public eligibility/protocol language are contract-guarded; Ads dollar semantics are corrected; the doctor batch-review queue is reconciled; paid E2E fixtures use deterministic excluded identities; email consent is default-on with enforced opt-outs and suppressions (operator decision 2026-07-17, superseding the fail-closed-on-missing-row posture); review redirect tracking is aggregate-only; PostHog captures are personless and replay-free while Google Ads enhanced conversions remain intact. Live PostHog IP, console, heatmap, recording, and 24-month retention settings remain a deployment gate because the connected integration is read-only. | Complete only when each named boundary has implementation plus focused proof. Re-open any closed sub-boundary when production evidence or an operator decision exposes drift. |
