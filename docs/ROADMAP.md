...
>
> **Last refreshed:** 2026-07-12. Refresh whenever priority or status changes; perform a deliberate review at least monthly.
...
- support Inbox unread counts through the aggregate-only Telegram bridge once its Gmail count source and schedule are activated
...
|------|----------|----------------|---------------------------|
| 1 | **Truth and measurement gate** - reconcile canonical docs; repair public 24/7, automation, and 18+ contradictions; close the batch-review and synthetic-E2E boundaries; correct Google Ads dollar units; make review-email consent fail closed and tracking aggregate-only. | **Partially complete 2026-07-12.** Canon ownership and public eligibility/protocol language are contract-guarded; Ads dollar semantics are corrected. Still open: doctor batch-review reconciliation, deterministic synthetic E2E patient linkage, fail-closed review-email consent, and aggregate-only review redirect tracking. | Complete only when each named boundary has implementation plus focused proof. Re-open any closed sub-boundary when production evidence or an operator decision exposes drift. |
| 2 | **Operator brief and support approval surface** - keep the Overview brief actionable, route aggregate support counts to Telegram, and preserve the approval boundary for Ads changes and customer replies. | **Receiver implemented; Gmail count source and schedule pending.** Overview shows the milestone, exceptions, and Ads performance; the protected receiver accepts only unread counts. Production activation remains gated by credential rotation and count-only connector verification. Identifiable Gmail summaries/drafts are blocked by the privacy-processor gate. | One calm decision surface, aggregate-only alerts, no copied mailbox, no PHI in analytics, and no send or Ads mutation without approval. |
