...
>
> **Last refreshed:** 2026-09-05. Refresh whenever priority or status changes; perform a deliberate review at least monthly.
...
**2026-09-05 execution order:** prescribing completion/recovery, draft-note persistence and truthful notification fixes are merged in PR #520; next recover medical-certificate acquisition/conversion using the refreshed [revenue diagnosis](audits/2026-09-04-scaling-audit.md#2026-09-05-revenue-leak-investigation). The completed seven-day window retained $1,901.65 versus $2,625.35; certificate orders fell 35 to 21 while scripts fell only 37 to 36. Keep the current Scripts $95/day step under its existing three-closed-day/ten-attributed-order gate. Both prescription pages passed Search Console Live Tests on 5 September; `/prescriptions` indexing was requested and the existing `/online-prescriptions` request was confirmed. Actual indexing remains pending Google, so check the indexed outcome rather than repeating submissions. Measure refill paid reorders, then retain the Sep 9 weight and Sep 11 hair checkpoints. No new Ads variable or deferred outreach is authorised by this diagnosis. This execution order does not certify the open revenue rung or change its gates.
