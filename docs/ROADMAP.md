...
| 2 | **Operator brief and support approval surface** - keep the Overview brief actionable, route aggregate support counts to Telegram, and preserve the approval boundary for Ads changes and customer replies. | **Count-only bridge active.** Overview shows the milestone, exceptions, and Ads performance; the protected receiver and hourly Gmail label-count automation are active. Identifiable Gmail summaries/drafts remain blocked by the privacy-processor gate. | One calm decision surface, aggregate-only alerts, no copied mailbox, no PHI in analytics, and no send or Ads mutation without approval. |
