...
>
> **Last refreshed:** 2026-08-15. Refresh whenever priority or status changes; perform a deliberate review at least monthly.
...
| 4 | **Prove paid contribution by service** - keep launched services live at low budgets, review performance daily, improve keywords/negatives/assets/sitelinks with approval, and scale only a service that passes the first-order contribution gate. | **Daily manager active in read-only shadow as of 2026-07-31.** The Scripts attribution investigation was resolved on 2026-08-15, but its separate seven-GREEN-day and operating-health gates remain. Fresh account truth also shows ED and Hair Loss at `$12/day` with Maximise Conversions while the code-owned pilot baseline remains `$7/day` Manual CPC. Hair Loss was contribution-negative in the closed 30-day window; ED was positive. No live change is authorised: Hair needs exact approval for the prepared pause packet, while ED needs an operator choice between restoring the bounded pilot configuration and deliberately revising policy. | Trusted measurement, positive first-order contribution, stable safety/refund/queue metrics, and operator approval for the exact change. |
...
| C | **Retrospective review of the complete historical soft-flagged auto-issued cohort.** | Between the attestation being removed and #442 enforcing the pre-issuance block, a soft-flagged certificate could issue without a durable human review surface. Eight older cases cannot be found through persisted dashboard flags and require a reportable `ai_audit_log` query on `metadata->softFlags`; the ninth has the persisted flag. **Refreshed 2026-08-14: 9 intakes (May 26 – Aug 9), all draft-lane, 3 of 9 batch-reviewed under the old regime, all 9 still standing approved; one issued after #428 and before #442, zero at or after #442.** Review all nine in authenticated source records; opening a record is audit-writing, so obtain explicit approval before starting. | Operator |
