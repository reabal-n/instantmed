...
Plan 1 is in progress: checkout recovery and the first encryption-tooling slice are released; reviewed monitoring is in required release checks. The historical repair is on hold for a newly identified approved-scope race in the CLI; the earlier apply packet is superseded pending its reviewed correction and fresh evidence. Its [execution receipt](superpowers/plans/2026-09-06-01-checkout-data-reliability.md#execution-receipt) owns completion evidence and blockers. Plans 2–5 remain unstarted. Execution receipts own task proof; Git/PR/deployment history owns releases. Due measurement reads remain part of the standing operating rhythm and are checked at each session start, so the build sequence does not postpone the Sep 9/11/19 checkpoints. Commercial observation may continue while independent internal UI builds proceed.
