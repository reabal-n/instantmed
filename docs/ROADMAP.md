...
**Owner operating decision — 2026-08-18, clarified 2026-08-28:** No staffing or rota project is active. Requests may wait for the owner-doctor within the existing public timing language and the internal 24-hour maximum; the owner will decide when workload requires another doctor. Keep measuring queue age and escalate only a patient-safety issue or a breach of that maximum. The current first-review P95 remains evidence to watch, but it does not block the approved acquisition tests. A privacy-safe support-ticket classification still must demonstrate fewer than five contacts per 100 orders before the `$5,000` rung is formally certified. The support-email agent remains deferred. Lena is a short voice secretary that takes one confirmed message for the Medical Director; she does not provide general answers, clinical intake, triage, authentication, autonomous adjustment, or a promise of resolution. The implementation remains default-OFF until deployment and controlled end-to-end call proof.
...
Google Ads mutations and customer replies require operator approval. The implementation workflows live in `docs/OPERATIONS.md`. Email support conversations stay in Gmail and are handled manually; the app does not store email bodies or PHI. Automatic Telegram paging for support Inbox mail is retired; paid-request alerts and PHI-free Medical Director voice-message pages are separate bounded operational channels. Email thread summaries and reply drafts remain deferred. The voice path stores only the encrypted caller-confirmed fields, no raw audio or full transcript, and stays default-OFF until deployed and proven by a controlled call.
