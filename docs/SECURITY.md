...
Apply repeats the bounded keyset traversal and uses a single atomic PostgreSQL conditional update per candidate. The predicates compare the ID, every plaintext/encrypted field snapshot, the fixture-classification email/name, and `phi_encrypted_at`; missing targets must still be exactly null or empty. Returned affected IDs own the row/field success counts. Both migration-status updates must also return exactly the expected status ID; a missing, wrong, or ambiguous receipt is `STATUS_WRITE_FAILED`, never successful completion. Zero-match updates are reread and classified as source/target/other changes or deletion, without an ID-only retry. A row write error is recorded while later candidates continue; read/key/receipt/status failures stop remaining work. Skips and errors return a nonzero exit and leave `completed_at` unset; later runs resume from actual missing fields. Output and migration-status errors contain bounded technical codes and aggregates only. Dry-run performs no status writes, including failure paths.
