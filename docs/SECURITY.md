...
Preparation interface (production approval remains on hold):

```sh
NODE_OPTIONS=--conditions=react-server corepack pnpm encrypt:backfill --dry --batch=50
# After approval, supply the exact reviewed expectation object in this private file:
...
**Production apply succeeded at 04:09:10.937–04:09:28.312 UTC:** 466 profiles updated, 773 encrypted copies added, zero skipped rows and zero errors. Apply's own preflight matched every approved aggregate and the fingerprint before any write. Only missing encrypted columns and `phi_encrypted_at` were written; the atomic source/target comparison preserved concurrent edits and existing ciphertext.
...
`append_monitor_state(text,bigint,jsonb)` is a service-role-only, fixed-empty-search-path SECURITY DEFINER function for two allowlisted state keys in `operational_metrics`. It accepts bounded numeric/boolean evidence and fixed schema keys; no patient/intake/contact identifiers, prose, credentials, URLs or logs. Run IDs are public GitHub operational evidence. The table retains SELECT/INSERT-only service-role grants and no browser grants. Transaction advisory lock plus version CAS prevents competing state claims from both winning. Missing state fails open to alerts, never a fresh healthy baseline. The release controller applies this reviewed additive migration through the normal engineering release protocol under the existing Plan 1 authorization. Local fixture proof does not establish production application; historical profile repair remains separately approval-gated.
