...
| `medical_director_voice_messages` | Patient name, confirmed summary, and callback number only when requested | Entire `payload_enc` envelope; no plaintext payload, raw audio, full transcript, raw caller ID, raw Call SID, or date of birth | Service role only; RLS enabled with no patient/staff direct policy; admin-only server surface |
...
**Historical production profile repair completed and verified on 2026-09-07 under explicit operator approval.** All 773 missing copies across 466 profiles were filled; post-verification authenticated all 1,107 eligible copies and found none missing. The guarded zero-change rerun made no writes. The receipt below separates source/CI, deployment and production-data proof.

**Phase 1 (profiles — runtime shipped; historical coverage verified 2026-09-07):** `profiles.medicare_number_encrypted`, `profiles.date_of_birth_encrypted`, `profiles.phone_encrypted` use the existing `ENCRYPTION_KEY` AES-256-GCM format. The production execution receipt below verifies historical coverage separately from runtime support.

**Profiles-only repair command (hardened 2026-09-06; historical repair verified 2026-09-07):** supply `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and the existing `ENCRYPTION_KEY` through the private operator environment. The script loads no `.env` files. Use `NODE_OPTIONS=--conditions=react-server corepack pnpm encrypt:backfill --dry --batch=50` for the read-only packet. No flags also means dry-run; `--dry-run` aliases `--dry`. Only an explicitly authorized `--apply --expect=/private/approved-profile-scope.json` can write. The file must contain exactly the `expectation` object emitted by the approved dry run; never generate or replace it automatically immediately before apply. Unguarded `--apply` fails with `APPROVAL_REQUIRED`, including a zero-change rerun. Missing/unreadable/malformed expectation files fail with `APPROVAL_INVALID` before client creation or database access. Unknown flags, conflicting/repeated mode flags, repeated batch/expectation flags, expectation flags in dry mode, and batch values outside integer `1..500` fail before data access. There is no `--table` option and no envelope/key-rotation mode.

The preflight privately scans all profiles up to a fixed ID high-water mark in bounded keyset pages. It excludes the canonical seeded patient/staff IDs and `isLikelyTestPatientIdentity` machine email/name patterns, not the narrower reportable-patient cohort. The machine exclusions include the exact `doctor-review-<UUID>@test.instantmed.com.au` and `weight-review-<UUID>@test.instantmed.com.au` fixture addresses; this rule does not exclude the entire domain or generated name prefixes. Excluded rows contribute neither compatibility evidence nor writes, even if fixture ciphertext is unreadable. It reports only aggregate scanned/excluded/eligible/candidate counts and per-field missing, existing, authenticated-decryptable, parity-mismatch, and decrypt-failure counts. At least one existing nonempty ciphertext must authenticate and **every** eligible existing ciphertext must decrypt; wrong keys, mixed-key/unreadable ciphertext, incomplete reads, and no compatibility evidence stop before any status or profile write. Authenticated ciphertext with a plaintext parity difference (including absent plaintext) is reported separately and preserved. Empty plaintext is not encrypted; null or empty ciphertext can be filled only when the nonempty source still matches.

Preflight freezes each eligible candidate's ID, intended fields, and complete source/target/classification/timestamp snapshots in memory. Dry-run emits a PHI-free `expectation` object: `version: 1`, `scopeFingerprint`, aggregate `scanned`, `excluded`, `eligible`, `candidates`, and the five per-field maps `missing`, `existing`, `decryptable`, `parityMismatch`, `decryptFailures`. Counts must be nonnegative safe integers, field maps contain exactly DOB/phone/Medicare keys, and the fingerprint is a 64-character lowercase hexadecimal digest. The strict JSON file accepts no extra/missing keys and is limited to 16 KiB. The domain-separated HMAC-SHA-256 uses the existing encryption key material and binds the target URL and deterministically ordered frozen IDs, intended fields, and all CAS snapshots; it never exposes the underlying identifiers, values, ciphertext, URL or key. JSON key order and batch size do not change this scope. Every approved aggregate and the opaque fingerprint must exactly match the new preflight before **any** status or profile write; equal-count candidate substitutions and changed approved snapshots fail with `APPROVAL_MISMATCH`. Even fixture-count drift requires a fresh reviewed packet.

Apply writes only the frozen preflight candidates, using a single atomic PostgreSQL conditional update per candidate. It performs no second discovery scan, so a later inserted profile or a former noncandidate acquiring a missing field cannot receive a PATCH in that run. The predicates compare ID, every plaintext/encrypted field snapshot, fixture-classification email/name, and `phi_encrypted_at`; missing targets must still be exactly null or empty. Returned affected IDs own row/field success counts. Both migration-status updates must also return exactly the expected status ID; a missing, wrong, or ambiguous receipt is `STATUS_WRITE_FAILED`. Zero-match updates are reread and classified as source/target/other changes or deletion, without an ID-only retry. A `PROFILE_WRITE_FAILED` row error or a CAS skip may allow later frozen candidates to continue; the resulting partial run is incomplete, returns nonzero, and leaves `completed_at` unset. Fatal read/key/receipt/status errors stop candidate work. Output and status errors contain bounded codes and aggregates only. Dry-run performs no status writes, including failures.

Before production apply, review an exact packet containing project identity, code SHA, timestamp, scope/exclusions, the emitted expectation object, compatibility and existing parity exceptions, guarded-write method, verification command, and stop conditions; obtain separate operator approval. The file is a scope guard, not proof of human authorization. After any nonzero result, CAS skip, or write/status error, **do not execute subsequent commands or reruns**: preserve confirmed writes and inspect the aggregate receipt, then obtain a fresh reviewed plan and authorization. Only a successful approved run may proceed to authorized read-only verification. Any explicitly authorized zero-change apply rerun also requires its own reviewed zero-candidate expectation file; that file refuses new work before any status/profile write. The preflight is a sequential scan, not a database-wide transaction snapshot. Concurrent changes to frozen candidates cause CAS skips; newly eligible work remains for separate review. Retain plaintext and both current keys until the separate reader audit and retirement decision. Local CLI and disposable PostgreSQL/PostgREST proof do not establish production execution or parity.

Future repair interface (every production apply remains exact-scope approval-gated):

```sh
NODE_OPTIONS=--conditions=react-server corepack pnpm encrypt:backfill --dry --batch=50
# For a separately approved future scope, supply its exact reviewed expectation object:
...
### Historical-profile repair — completed 2026-09-07

The operator explicitly authorized the production repair and verification on 7 September, with no patient contact. The corrected, frozen-scope CLI from [PR #528](https://github.com/reabal-n/instantmed/pull/528) was executed at exact reviewed pre-squash source head `6b8d8eb6fd6f18dba41efba50aa3e335101a9023`, retained by GitHub at `refs/pull/528/head`; that source maps to merged commit `b8951b8366814e60d895f264ec533798179ce5a5`. Its [required CI](https://github.com/reabal-n/instantmed/actions/runs/34029654838), independent review and 146 HTTP/PostgreSQL regression cases had passed. Script blob `33c3d84005130ad734ccc3187d1b678e73ada785` and fixture-classifier blob `866e75ca1db588bbf0a47963a774ad3c4529f676` were rechecked before execution.

The fresh production dry run at **04:06:13.936–04:06:14.978 UTC** scanned **599 profiles**, excluded **51** canonical fixtures and found **548 eligible profiles**. The current Vercel production environment, injected privately from an empty directory without local dotenv fallback, authenticated **334/334 existing copies**. Running function memory was not introspected. The scan still found **466 candidates / 773 missing copies**, but its fingerprint differed from the September 6 packet. Unchanged totals do not establish unchanged candidate identities or values. The old artifact was preserved; a separate [current expectation](superpowers/receipts/2026-09-07-profile-encryption-scope.json) was reviewed before apply (SHA-256 `987fa2aa33232ec96bac18e16803ce3d8a2254405387689cc55e34008d762831`).

**Production apply succeeded at 04:09:10.937–04:09:28.312 UTC:** 466 profiles updated, 773 encrypted copies added, zero skipped rows and zero errors. Apply's own preflight matched every approved aggregate and the fingerprint before any write. Only missing encrypted columns and `phi_encrypted_at` were selected by the script; the existing `profiles_updated_at` trigger also advanced `profiles.updated_at` on those 466 ordinary row updates. The atomic source/target comparison preserved concurrent edits and existing ciphertext.

| Field | Copies added | Existing / authenticated after repair | Preserved parity differences | Decrypt failures |
|---|---:|---:|---:|---:|
| Date of birth | 452 | 520 / 520 | 0 | 0 |
| Phone | 300 | 359 / 359 | 1 | 0 |
| Medicare | 21 | 228 / 228 | 3 | 0 |

The read-only verification at **04:11:01.856–04:11:02.818 UTC** found **zero candidates or missing copies**, with **1,107/1,107 copies authenticated**. The four existing parity exceptions remained unchanged: one phone country-format difference, one Medicare plaintext-absent case and two Medicare content differences. No private values or patient identifiers are retained in the [execution receipt](superpowers/receipts/2026-09-07-profile-encryption-repair.json).

The separate [zero-candidate expectation](superpowers/receipts/2026-09-07-profile-encryption-zero-scope.json) was independently reviewed (SHA-256 `eb132e74badb45e7e857ebfd7d26b558078e1028e3cffad0a6ce87580e88f3ac`). The authorized conditional apply rerun processed and changed **zero profiles / zero fields**, with zero skips or errors. Production read-back still showed exactly one completed migration-status row: 466/466 records, zero errors, and the same completion/update timestamp. This verifies idempotence separately from the first successful apply.

**Preservation/rollback:** retain plaintext, current keys, existing ciphertext and verified new copies. No key rotation, plaintext retirement, bulk deletion, clinical outcome change or patient contact occurred. The plaintext-null block retained in the squashed baseline is historical migration SQL, not runtime cleanup performed by this repair. Any future unexpected result requires a fresh reviewed repair; reverting application code does not undo this verified data repair. The already-released tooling deployment and this production execution are separate evidence.
...
The Phase 2 PHI fields (added March 2026) are in dual-write mode — plaintext and encrypted columns are both written on every write, and reads prefer the encrypted column with a plaintext fallback. Post-launch, if a plaintext cutover is desired for non-medicare fields, create a new plan at that time.

**Authored empty clinical notes:** `prepareDoctorNotesWrite` preserves an explicitly saved empty string as an authored note through the same doctor-notes encryption wrapper. `null` means no saved note; historical nulls remain unchanged. Existing encryption flags, dual-write/read behavior and fail-closed encryption errors apply to empty strings too. The editor distinguishes empty from absent when reopening; this change requires no migration or backfill.

**Fail-closed writes + alarm (2026-07-24):** with the PHI flags enabled, an encrypt failure in any `lib/security/phi-field-wrappers.ts` `prepare*Write` helper or in `buildAnswersInsertColumns` (`lib/data/intake-answers.ts`) now throws `PhiEncryptionWriteError` instead of silently storing plaintext-only, and fires one fatal, stably-fingerprinted Sentry event per field via `lib/security/phi-encryption-alarm.ts` (`["phi-encryption-failure", field, operation]` — same alarm class as a misconfigured Stripe price). Decrypt failures alarm at `error` level and keep the plaintext read fallback for availability during dual-write. `instrumentation.ts` still verifies key material at boot, so a runtime encrypt throw means the keys changed underneath a live deploy.

**2026-07-24 checkout bypass fix:** the two live checkout persistence paths (`lib/stripe/checkout/persistence.ts`, `lib/stripe/guest-checkout.ts`) had been inserting `intake_answers.answers` directly, bypassing encryption entirely — `answers_encrypted` had zero rows in production while the encrypted data layer (`saveIntakeAnswers`) had zero callers. Both paths now insert through `buildAnswersInsertColumns`, the single encryption seam for answers writes. Three `doctor_notes` writers (decline, clinical-note sync, Telegram approve) that updated the plaintext column directly now go through `prepareDoctorNotesWrite`. Historical plaintext-only rows are backfilled via `scripts/encrypt-phi-backfill.ts` (profiles, ENCRYPTION_KEY system) and `scripts/encrypt-phi-backfill-envelope.ts` (intake_answers + doctor_notes, PHI_MASTER_KEY envelope system; key-match preflight, `--verify` parity mode, `--repair-mismatches` for envelopes gone stale behind a bypass writer); the plaintext columns stay until a reader audit plus backfill parity check allow retirement.
...
**Review-request measurement:** The one-use review-link capability is random and non-PHI, but it is still treated as a secret. Its raw value may exist only in memory and the encrypted frozen email payload; plaintext outbox metadata stores only a validated SHA-256 hash. The consume and aggregate funnel RPCs are `SECURITY INVOKER`, fixed-search-path, and executable only by `service_role`. The funnel returns aggregate lifecycle counts only, evaluates every mutable signal against one bounded `as_of`, and fails closed in the application unless its mutually exclusive primary buckets reconcile exactly to eligible. It never returns a patient, intake, recipient, provider message, capability, hash, or clinical field. The redirect emits only allowlisted aggregate campaign dimensions after the first successful consume; the raw key, hash, URL, patient/intake identifiers, recipient, service, and clinical context are prohibited from PostHog, logs, Sentry, and `operational_metrics`. Manually recorded external review totals contain a fixed metric name, numeric total, fixed source dimensions, and timestamp only.
...
**Authenticated helper boundary (applied; verified 2026-08-28):** migration `20260825073433_scope_profiles_realtime_policy_to_authenticated.sql` scopes `profiles_select_own_or_doctor` (`SELECT` on `public.profiles`), `doctors_manage_documents` (`ALL` on `public.documents`), and `Doctors can insert verifications` (`INSERT` on `public.document_verifications`) to `TO authenticated`, matching the roles allowed to execute `is_doctor()`. Linked migration history contains `20260825073433`; live `pg_policies` metadata confirmed all three policies exist on those commands and tables with roles exactly `{authenticated}`. Its apply-time `pg_policies` check fails closed if any policy is absent or retains a broader role.

**Specialty attribution integrity (applied; verified 2026-08-28):** migration `20260828090000_specialty_experience_attribution.sql` adds nullable, non-clinical `text` columns `public.partial_intakes.growth_experience_version` and `public.intakes.growth_experience_version` with no defaults. Live metadata confirmed the bounded-format constraints `partial_intakes_growth_experience_version_check` and `intakes_growth_experience_version_check` are present and validated; triggers `trg_partial_intakes_preserve_growth_experience` and `trg_intakes_growth_experience_immutable` are enabled; and trigger functions `preserve_partial_intake_growth_experience()` and `enforce_intake_growth_experience_immutable()` are `SECURITY INVOKER` with `EXECUTE` denied to `PUBLIC`, `anon`, and `authenticated`. Linked migration history aligned through `20260828090000`; the linked DB lint error gate and SECURITY DEFINER ACL check passed, and the post-DDL advisors reported no migration-specific security or performance finding.
...
- **Owner-only legacy helpers (7 at the time of the ACL migration):** production-only `is_doctor_or_admin` plus six replay-only functions (`create_clinical_summaries_table`, `create_notification`, `get_patient_notes`, `grant_consent`, `log_audit_event`, and `record_admin_action`) have no current repository caller or RLS dependency. The migration revokes every app-role grant when each object exists without recreating it where absent.
- **Trigger-only functions (10 at the time of the ACL migration):** seven production functions plus replay-only `audit_intake_created`, `audit_intake_status_change`, and `notify_on_intake_status_change` have no direct app-role execution grant.

Migration `20260814113000_retire_replay_only_intake_triggers.sql` removes the three replay-only intake triggers/functions and their legacy `log_audit_event` dependency after a clean baseline replay. The squashed baseline replaces the original `audit_log` table with a read-only compatibility view; retaining the old trigger writers made clean-reset intake inserts and status changes target columns that no longer exist. The same replay can recreate `archive_old_audit_logs(integer)` without the legacy `audit_logs.request_id`, `archived_at`, or `retention_tier` source shape its body requires. The migration removes that RPC only when it exists and its archive table or a required source column is absent; production has the complete legacy archive shape and retains the valid function unchanged. Every drop is conditional and non-cascading, so production keeps the same runtime semantics while local resets match the executable production inventory.
...
Patient self-service closure is **not** a clinical-record deletion. `deleteAccount()` in `app/actions/account.ts` calls the service-role-only `close_patient_account` RPC, which atomically blocks closure while active clinical work exists, writes a durable `closed_auth_accounts` tombstone keyed by the Supabase Auth user ID, clears `profiles.auth_user_id`, sets `profiles.account_closed_at` / `account_closure_reason`, and removes non-essential profile/contact PHI from plaintext and encrypted profile columns. Clearing the profile auth link immediately removes direct PostgREST/RLS access; the separate tombstone has no `auth.users` foreign key or authenticated grants and prevents the same Auth user from silently creating or relinking a fresh profile. Post-sign-in and every profile-creation fallback check the tombstone before writing. Retained clinical, payment, document, and audit tables stay linked by `profiles.id`.

Closed profiles are rejected by the central auth helpers (`getAuthenticatedUserWithProfile()`, `getOrCreateAuthenticatedUser()`, `getApiAuth()`, `getUserProfile()`) and are excluded from guest-profile relinking. Do not delete the Supabase auth user for this flow: global sign-out revokes its refresh sessions, while retaining the Auth identity keeps future sign-ins tied to the durable tombstone. Deleting and recreating Auth would produce a new ID outside that closure record; deleting before the atomic detach also risks historical `auth.users` cascade behavior.
...
| **Twilio Voice WebSocket** | `app/api/webhooks/twilio/voice/stream` | Validates `X-Twilio-Signature` against the exact `wss://` URL before accepting the upgrade, then validates a short-lived AES-256-GCM session token and exact `CallSid` before relaying audio or ConversationRelay text. Payloads, pre-open buffers and transient text history are bounded. Provider frames, raw audio and full transcripts are not logged or persisted. |
...
**Optional tracker account access:** `POST /track/request/access-link` requires the standard CSRF header and an empty body. Keeping it beneath `/track` preserves the existing HttpOnly cookie scope. The server verifies the capability, rejects closed/merged/non-patient profiles and linked Auth mailbox mismatches, and applies separate hashed IP (`auth`) and capability (`sensitive`) rate-limit buckets. All capability, rate-limit, and provider outcomes return only `{ accepted: true }`; CSRF failures use the standard 403 retry protocol. The request-local Supabase SSR client requests a magic link to the server-resolved email with account creation enabled only after the patient chooses the button. The production auth-email hook places its one-time confirmation credentials and return destination in the URL fragment. The post-sign-in destination is the exact `/patient/intakes/[id]` page, so opening the email in another browser does not depend on the original `/track` cookie. The intake ID selects a destination only; authenticated ownership still protects the request, documents and replies. The tracker capability and mailbox never enter the return URL. The standard provider callback remains supported. Provider/database errors are not logged at this boundary.
...
| `support` | support | Non-clinical ops. Initial scope (Phase 1, 2026-05-11): `/admin/ops`, `/admin/webhook-dlq`, `/admin/ops/parchment`, `/admin/ops/prescribing-identity`. Phase 8 (2026-05-20) opened `/admin/intakes` (intake ledger) and the intake refund action to support. Refund attempts are capped at $100 and 3 distinct attempts per rolling 24h by the atomic `reserve_support_refund_attempt` RPC; only `service_role` may execute it, and its RLS-protected receipt table grants no direct role access. Support still sees only ledger metadata (no clinical answers); Email delivery, settings, full patient ledgers, clinical approvals, and direct prescribing remain admin/doctor-only. |
| `patient` | patient | Patient portal only |

**Internal server modules:** Operations queries, intake-event helpers, email transport/outbox/recovery, notifications and draft generators use `server-only`, not `use server`. Only deliberate guarded actions may be registered. Every production build checks both Node and Edge action maps for these internal modules; missing classification metadata fails the build. The isolated compiled-action suite checks direct action dispatch with synthetic role fixtures, including denial before privileged reads or writes. It is application-boundary evidence, not hosted RLS proof.
...
| Supabase Auth email verification | The send-email hook plans branded messages through `lib/auth/auth-email-message-planner.ts`. One-time `token_hash` values live only in the `/auth/confirm` URL fragment, are scrubbed from PostHog/Sentry URL telemetry, and are exchanged by browser `verifyOtp()` only after an explicit patient click. Email scanners can load the interstitial without consuming the token. Secure email change sends the documented current-address and new-address hashes separately. |
| Password recovery | `/auth/forgot-password` calls `resetPasswordForEmail()`. The branded recovery message opens `/auth/confirm`; explicit browser verification establishes the recovery session and then navigates directly to `/auth/reset-password`. |
...
**2026-08-16 cleanup:** the legacy `/cert-preview` page and duplicate `/api/med-cert/preview` and `/api/med-cert/render` handlers are retired. Staff preview now uses the authenticated server action in `app/doctor/intakes/[id]/document/actions.ts`, while final issuance remains exclusively in the atomic approval pipeline. The middleware deny rule for `/cert-preview` remains fail-closed so the path cannot be accidentally reintroduced without an explicit review.
...
The `/dashboard` active-request search, `/admin/intakes` Ledger search, and shared `/admin/patients` / `/doctor/patients` directory search keep staff-entered identity or request terms in browser memory. Each browser sends its normalized term only through an authenticated, rate-limited Server Action POST boundary; admin-wide, doctor-touched, and support-masked scopes are derived again on the server and cannot be expanded by client input. The actions necessarily forward the normalized lookup predicate to Supabase/PostgREST, but provider failures are reduced to stable application messages before they reach the client or logs. Search terms are never persisted in staff URLs, history, referrers, sort links, or pagination links; an incoming legacy `q` parameter is stripped before any of these surfaces queries or renders.
...
Payment recovery does not weaken that route exclusion. A domain-separated, seven-day HMAC proof bound to the intake must verify server-side before an explicit guest-continue or authenticated patient retry-button action may write `intakes.recovery_email_engaged_at`. Guest GET and authenticated page load are read-only, so scanner traversal and browser prefetch cannot record engagement or start checkout. Applied and verified in production on 2026-09-04, migration `20260903120000_recovery_email_engagement.sql` adds a database trigger that rejects initial marker writes from non-service roles and preserves the first server value. The timestamp contains no URL, capability, patient/contact identifier, clinical data, campaign text, or acquisition click identifier. Aggregate reporting combines this forward-only server marker with already-persisted public `/request` recovery UTMs; it never emits row-level recovery evidence and labels pre-migration fallback totals non-exhaustive.
...
| `DISABLE_RECONCILIATION_SENTRY` (retired) | Per-record warnings were removed September 24; this legacy flag does not disable aggregate cron integrity alerts. |
...
`append_monitor_state(text,bigint,jsonb)` is a service-role-only, fixed-empty-search-path SECURITY DEFINER function for two allowlisted state keys in `operational_metrics`. It accepts bounded numeric/boolean evidence and fixed schema keys; no patient/intake/contact identifiers, prose, credentials, URLs or logs. Run IDs are public GitHub operational evidence. The table retains SELECT/INSERT-only service-role grants and no browser grants. Transaction advisory lock plus version CAS prevents competing state claims from both winning. Missing state fails open to alerts, never a fresh healthy baseline. The reviewed additive migration is applied and production observation is recorded in the [Plan 1 execution receipt](superpowers/plans/2026-09-06-01-checkout-data-reliability.md#execution-receipt); local fixture proof alone would not establish either. The historical profile repair was separately authorized, applied and verified in the aggregate receipt above. Neither completed operation authorizes a repair rerun or a new production-data scope.
