...
| Supabase Auth email verification | The send-email hook receives the secret as `email_data.token_hash`, but GoTrue's direct `/auth/v1/verify` URL must pass that value as `token=...`. `token_hash=...` is reserved for an app-owned `verifyOtp()` exchange and causes `validation_failed` on the direct endpoint. Pinned by `lib/__tests__/password-reset-flow-contract.test.ts`. |
