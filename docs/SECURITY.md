...
**Review-request measurement:** The one-use review-link capability is random and non-PHI, but it is still treated as a secret. Its raw value may exist only in memory and the encrypted frozen email payload; plaintext outbox metadata stores only a validated SHA-256 hash. The consume and aggregate funnel RPCs are `SECURITY INVOKER`, fixed-search-path, and executable only by `service_role`. The redirect emits only allowlisted aggregate campaign dimensions after the first successful consume; the raw key, hash, URL, patient/intake identifiers, recipient, service, and clinical context are prohibited from PostHog, logs, Sentry, and `operational_metrics`. Manually recorded external review totals contain a fixed metric name, numeric total, fixed source dimensions, and timestamp only.
