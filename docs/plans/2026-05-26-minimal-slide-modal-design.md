...
2. The doctor slide-over cockpit is dense: three tabs (Request / Notes / History), a 3×2 identity grid, a wrapped "Recommended plan" card, and a draft-note disclosure. For a 2-day cold cert the doctor only needs to read 1–2 lines of patient story and click Approve.
3. The ED slide leaves prescribing as "Doctor to select agent, strength, directions, quantity and repeats in Parchment after final review." Patient already told us their preference (daily / PRN / doctor decides) — we should map straight to a real medication + dose + directions so the doctor can copy-paste into Parchment.
...
- ED prescriptions ship a real recommendation: medication name, strength, quantity, repeats, directions — derived from the patient's stated `treatment_preference`.
