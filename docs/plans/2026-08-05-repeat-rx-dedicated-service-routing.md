...
- **D2 (recommended default, implemented in this plan): weight-loss-class medicines are flag-only** — a `gated_service_medication` doctor flag, no steer (there is no live destination), no block. Rationale: Ozempic/Victoza are also legitimate **type-2 diabetes** repeats — a hard block would wall diabetic patients; `docs/CLINICAL.md` line 197 already permits weight-loss meds under "manual review only"; and the 6 existing orders are revenue the doctor chose to serve. **Operator flip:** to hard-block instead, change the `gated` branch in Task 4 to return `{ valid: false, requiresConsult: true, error: <copy> }` and add the mirrored client terminal card — but do not do this without an explicit operator decision recorded here, because it declines diabetic repeats too.
...
- **Existing in-queue intakes** are handled by the doctor in the queue — decline with auto full refund and redirect, or prescribe with context. No backfill of old intakes. (Never identify patients in this repository: it is public, and a name beside a medicine or condition is PHI. Refer to cases by truncated intake id only.)
