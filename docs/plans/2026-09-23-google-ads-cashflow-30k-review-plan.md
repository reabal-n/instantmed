...
|---|---|---|---|
| 1 | Repair the reported reconciliation alerts: synthetic fixture filtering, separate clinical waiting from payment failure, one aggregate cron alert owner with a 26-hour overlapping window and retained recovery-page integrity warnings, preserve overdue-review detection | Regression tests, CI, production release, next natural cron result | Code/release work authorised September 24; never mark a patient delivered to silence an alert |
