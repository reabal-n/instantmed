...
**Operator, ~1 h total, each send approved individually at the moment of sending:** MediCompare email (`info@medicompare.com.au`), Finder partner form, Trustpilot claim-and-hold. The clinical process copy in `docs/audits/2026-07-09-comparison-surface-submission-kit.md` was repaired on 2026-08-12. The kit remains **not send-ready** until prices, certification, destination status, and the exact payload are freshly verified and separately approved. Do not re-research the landscape.
...
**Clinical source repair completed 2026-08-12.** The comparison kit and NHSD runbook now use the branch-aware `clinical_review_sequence`. This closes the copy contradiction only; no external send or listing mutation is authorised without a fresh read and separate approval of the exact payload.
