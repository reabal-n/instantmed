...
15. **`components/operator/`** — `OperatorShell`, bounded staff pages, split panes, panels, and local quick-action palettes. All admin/operator/support pages use this shell; do not rebuild separate admin/doctor mode shells.
16. **`lib/auth/staff-capabilities.ts`** — Role + capability helpers (`hasAdminAccess`, `hasDoctorAccess`, `hasSupportAccess`, `hasStaffAccess`, `doctorHasCapability`). Three staff roles since 2026-05-11: `admin` (owner-operator), `doctor`, `support`. Use these instead of raw `role === "admin"` checks.
