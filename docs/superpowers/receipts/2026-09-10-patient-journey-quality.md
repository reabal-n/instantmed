...
PR #549's three CI retries were independently inspected. The certificate preview was still loading during a 21-second server response; the concise-review request reached the application's 12-second timeout; and the prescribing case first timed out in test login, then on a full-record load taking 29.9 seconds. The later attempts passed, with no confirmed wrong-data or completion-state regression. They remain reliability/evidence limits: a whole-test retry does not prove the displayed in-place Retry action recovered the observed timeout.
