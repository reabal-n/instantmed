...
The current clinical contract deliberately permits certain codeine-combination brands to reach individual doctor review. Panadeine receives a pre-payment likely-decline warning with a persisted acknowledgement that checkout revalidates; it is not an unsupported-payment hard block. A consistent service decision not to offer these requests would justify a bounded clinical-policy and pre-payment enforcement change across UI, server checkout and retry paths. The operator's explanation does not by itself supply an approved blanket drug exclusion. Refund cash and acquisition/processing costs stay in the economics; neither approval-rate optimisation nor reclassifying refunds is an acceptable repair.
...
This addendum authorises no new runtime change, campaign, budget increase, external message, reminder, or Plan 3 execution. The selected two-keyword repair and its exact approval/read-back remain the only unfinished commercial intervention in Plan 2.
