...
| 2026-09-03 | Definitive exact-SHA Task 6 verification | Tested pre-receipt implementation `1fef5e542807c0e793357fa02f79aa6728ba0f64`; replaces every earlier Task 6 receipt | PASS: the exact candidate closes the material findings from the superseded review: ambiguous encoded capability paths fail closed and recovery proofs are scrubbed; GSC Search Analytics paginates in 25,000-row batches until empty with a 250,000-row safety ceiling and an explicit top-rows-only limitation; recovery reporting uses durable outbox sends plus a forward-only private-route engagement marker; and all 42 deep-city records are constrained by rendered-copy contracts to the approved service, eligibility, review, delivery, timing, and refund claims. Exact-tree gates passed: 19 focused files / 269 tests; 10 documentation specs / 122 tests / 123 Markdown docs; route-conflict check; lint with zero warnings or errors; typecheck; base-to-candidate `git diff --check`; full 707 files / 6,550 tests; and `SENTRY_AUTH_TOKEN= corepack pnpm build`, which compiled successfully and generated 482/482 pages/routes without a Sentry release or source-map upload. The exact production build passed seven indexed-city HTTP 200/self-canonical/H1/scoped-copy checks, 35/35 valid city-bound JSON-LD payloads, all 13 exact HTTP 308 redirects, and the complete sitemap policy: 67 root, 60 blog, 15 condition, 7 location, 1 compare, and 1 symptom URLs (151 unique total), with empty `/for`, `/guides`, and `/intent` sitemaps, no Gold Coast sitemap owner, all indexed-location and `/telehealth-australia` `lastmod` values dated 2026-09-03, and every other root-route date matching policy. A corrected 32-view matrix across `/`, `/medical-certificate`, `/prescriptions`, `/telehealth-australia`, Brisbane, Canberra, Hobart, and `/intent/medical-certificate-for-mental-health-day` at desktop/mobile and light/dark passed HTTP 200, exact canonical, one non-empty H1, all JSON-LD parsing, expected `index, follow` or `noindex, follow`, complete four-region footer `data-nosnippet` coverage, 81/81 internal destinations returning local 2xx, correct theme, zero horizontal overflow, broken images, invalid links, hidden substantive sections, failed/pending requests, console errors, or page errors. Visual review passed all 74 active captures: 16 desktop full pages, 14 non-telehealth mobile full pages, and 44 mobile telehealth clips covering both themes through the 17,435px page bottom; 96 invalid or superseded captures were quarantined. The live read-only GSC audit generated at 2026-09-03T11:38:05.010Z for 2026-06-02 to 2026-08-31 found six live sitemap endpoints at HTTP 200, 151 unique live URLs, 79 returned performance pages, 35 with clicks, and 15 inspections with 10 PASS, 5 NEUTRAL, and zero errors; because Google does not guarantee every Search Analytics row, absent-row and weak-group findings are top-rows-only, not proven zero traffic. The aggregate 30-day audit generated at 2026-09-03T11:38:11.995Z for 2026-08-04T11:38:11.995Z to 2026-09-03T11:38:11.995Z found 306 reportable intakes, 284 paid orders, A$9,338.55 gross, A$304.25 refunded, A$9,034.30 net retained, and A$31.81 net AOV. PostHog returned HTTP 200 with 766 intake starts, 468 checkout views, 116 client purchases, 285 server purchases, and 283 Google Ads server conversions. Google Ads returned HTTP 200 with A$3,204.71 spend, 749 clicks, 153 local orders, A$5,156.25 local net revenue, A$20.95 local CAC, and 1.609 local ROAS. The retained seven-day draft snapshot had 302 rows updated in-window, 34 with email, and 7 converted markers; durable outbox evidence recorded 101 partial-recovery sends and 25 abandoned-checkout sends, while public recovery UTMs observed 16 recovered paid orders and A$449.10 recovered net. The private-route marker migration is not applied, so recovered contribution is explicitly forward-only and non-exhaustive. All six aggregate artifacts passed both the writer assertion and a zero-match privacy scan for patient-level rows, names, emails, Australian phones, UUIDs, payment/customer/click/certificate IDs, bearer/JWT values, auth tokens, capability paths, and sensitive field keys. All active and superseded evidence, probes, audit artifacts, and the exact build cache were moved recoverably to `/Users/rey/.Trash/instantmed-1fef-verification.7JvfCU`; the temporary environment symlink was removed; port 3060 and agent-browser sessions were clear; the isolated tree was clean; and the main worktree's pre-existing user changes were unchanged. No push, deployment, indexing request, campaign mutation, or other external write occurred. Exact-SHA hosting, migration apply/replay and linked production history, post-ship Google recrawl/index/snippet or sitelink change, complete Search Analytics coverage, 1-day/7-day closed-window comparisons, exhaustive pre-migration recovery attribution, cross-system event parity, Firefox/WebKit and authenticated transactional browser flows, and revenue causality remain unverified. | Independent base-to-receipt review before any PR or production action |
