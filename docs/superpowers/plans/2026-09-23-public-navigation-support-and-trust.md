...
The subsequent Opus review supplied the four previously unspecified findings. This follow-up addresses those, both font issues, the documentation contradictions and the existing mobile-menu overlap:

- Regenerated the money-page Jakarta subset from the full Latin variable face for the actual certificate and prescription headings. It now covers all their characters, including `A`, `W`, `Y` and `g`, while shrinking from 5,024 to 4,880 bytes. A Chromium font-coverage regression loads the committed file alone and checks that no headline glyph falls back to another face.
- Added a complete system monospace stack to the certificate specimen reference. It uses no additional web font; browser font inspection verifies a local monospace face.
- Unavailable service heroes omit reviews and refund reassurance and offer Contact instead of a new request. The homepage keeps platform-wide reviews when only certificates are disabled, while certificate timing is separately gated. The browser check covers all six services.
- Separated assessment and delivery in `/how-it-works`, including its example-card wording. Moved the business-page timing margin onto the counter itself so a suppressed counter leaves no empty wrapper.
- Removed the duplicate timing-query exclusion. The canonical seeded-patient filter now explicitly retains NULL patient IDs while excluding the same five test identities. Reporting exclusions, staff synthetic isolation and admin opt-in remain intact; affected tests exercise the updated query predicate and isolation boundaries.
- Corrected the remaining Google-review references and public-copy notes in `CLAUDE.md`, its generated mirror, `docs/PRIMITIVES.md` and the claims registry. Preserved the approved public wording, ProductReview stars and equivalent accessible rating.
- Placed the mobile drawer above page chrome and hid the sticky purchase bar while it is open. Closing the drawer restores the bar and focus to the menu button. This fixes the pre-existing overlap as part of the navigation cleanup.

The new browser checks reproduced the missing-font and overlapping-menu failures on the previous build. The disabled-service check also exposed the certificate hero's unchanged request link; the shared availability gate now provides its Contact fallback. Final verification and refreshed screenshots are recorded with this follow-up in the PR description and `.superpowers/front-door/final-*` artifacts.

Final local verification: **8,491 unit tests passed, 122 skipped; 91 Chromium checks passed without retries**. Full lint, TypeScript, documentation audit, dead-code checks, production build (84-second compilation), server-action boundaries and bundle gates passed. `/request` remains 177 kB. Visual inspection confirmed uniform headline glyphs in desktop light and mobile dark mode, the specimen reference and the open mobile menu. The refreshed pack contains 53 current captures: 44 desktop/mobile, seven tablet heroes, weight-management steps and the open menu. Provider delivery, Safari/Firefox runs, physical devices and production release are outside this evidence. Font-attribution checks explicitly target Chromium; the other browser checks remain portable. GitHub CI must finish on the pushed revision before merging.
