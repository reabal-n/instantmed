...
### CI follow-up — September 16

CI exposed an outdated PostgreSQL recovery fixture; it now loads the actual receipt migration, uses current profile fields and authoritative answer reads, and asserts one receipt for repeated pending submissions and none for paid/terminal recovery. All 49 database cases pass locally and that CI stage passed after correction. The browser signed-resume gate then exposed consent evidence being checked before high-stakes safety classification. Recovery now preserves clinical classification and durable holds before requesting reconfirmation; current consent still gates payable URLs and new sessions. Regression coverage includes missing receipts alongside high-stakes rules, required-field holds and payment races. CI run `34983871855` passed build, E2E and Lighthouse at `189379f29`; the additional fixes below require a new matching-head CI run.

Local production verification also reproduced intermittent patient-shell hydration failures. A separate lifecycle regression proved that initial auth resolution remounted the entire page and erased an edited child; the document generation now stays stable during bootstrap and changes after an established session ends. Staff session invalidation and scoped metadata clearing remain intact. The remaining hydration failure was traced to the pinned Next renderer replaying a host element after streamed child suspension without restoring its hydration cursor, matching [React upstream #35494](https://github.com/react/react/pull/35494). Instrumented production traces identify both root `main-content` and patient content wrappers. A DOM-free client function boundary moves streamed-child reconciliation off the host element. A deterministic browser reproduction against Next's bundled renderer fails in 10/10 direct-host controls and passes in 10/10 real-component cases, preserving the original DOM and an edit made before hydration. The final clean-source production recovery run passes all 50 checks (ten repetitions, zero retries), covering desktop light and mobile dark recovery surfaces with the original zero-error assertion. Matching-head CI and production release verification remain pending. Notification-loading and auth-update-priority experiments were reverted; the recovery error assertion remains unchanged.

Monitoring follow-up: Sentry `INSTANTMED-CF` is a single sample notification already resolved. The latest active `INSTANTMED-BV` event records a GitHub workflow-list HTTP 403; its recovery event is September 15 at 21:00 UTC. Observer reads now use the existing bounded GitHub token, preserve no-redirect behavior and classify exhausted quota correctly. All 70 observer/dispatch tests pass. Production verification is still required; historical unavailable events are not all attributed to this one cause.
