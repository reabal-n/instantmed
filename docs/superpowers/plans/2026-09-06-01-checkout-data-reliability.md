...
**Status:** In progress on 2026-09-06. Checkout recovery and the profile-repair tooling are released. Monitoring passed required CI and is released with its additive migration verified; first observer execution is being checked. The approved-scope race is corrected and independently reviewed in a follow-up draft; the historical production repair awaits final CI and operator approval of the new frozen-scope packet. The earlier aggregate-only packet remains superseded. Plan 2 has not started; see the execution receipt below for separate source, CI and production evidence.
...
**Approved-scope correction complete locally; production apply still held.** A final authorization check found that the released CLI rescans candidates after preflight. A new eligible profile below the UUID high-water, or a previously noncandidate row gaining a missing field, can then receive a copy despite being absent from the approved scan. The independent reviewer confirmed this scope-control gap. The earlier approval packet is superseded. The follow-up freezes the candidate set, binds an opaque approval fingerprint and requires a guarded zero-change expectation; local tests and independent review are complete. No production apply occurred. The follow-up in [PR #528](https://github.com/reabal-n/instantmed/pull/528) now passes 146 HTTP/PostgreSQL cases and independent review. Both original races were captured before adding the new CLI flag. The fresh 10:34 UTC read-only packet at integration head `4d23c5118c45ff3d41a91eb9f2ae9194ec8b1092` matched 466 candidates / 773 missing copies / 319 authenticated existing copies, with four preserved parity exceptions and zero writes. Its strict aggregate/HMAC expectation is preserved separately in SECURITY.md and the linked receipt JSON; final main-based CI and applicable authorization remain pending.
...
- **Local/review:** 72 HTTP and actual PostgreSQL/PostgREST backfill cases passed, including wrong/mixed keys, null/empty fields, concurrent edits, partial failures, resumability, idempotency, privacy, missing status receipts and fixture exclusion from compatibility checks and writes. Positive/negative classifier tests also passed. A fresh dry run exposed two exact CI identity patterns missing from canonical exclusions; the narrow correction was reproduced and independently reviewed. Lint, typecheck, documentation and actual dead-code gates passed. The initial released script blob was `118039e39037003a0c93e87e7e952421d062bc30`, subsequently superseded by the frozen-scope correction; corrected classifier blob is `866e75ca1db588bbf0a47963a774ad3c4529f676`.
...
Read-only migration preflight found 78 existing operational metrics rows and no new state/index/RPC. An isolated linked-CLI dry run listed exactly `20260906100000_monitor_observation_state.sql`; a fresh dry run before release preparation confirmed the same sole pending migration. After exact-head CI passed, the linked CLI applied only that reviewed file. Production read-back at 11:08:06 UTC confirmed the exact migration, both initial state rows, valid/ready unique index, fixed empty search path and service-role-only RPC access; table SELECT/INSERT-only grants were unchanged. The ACL checker returned zero violations, schema error lint passed, and canonical migration history aligned through `20260906100000`. Existing observer-host activity was verified separately: health heartbeat 08:45:07 UTC and 288 HTTP 200 health responses over the preceding 24 hours. First observer execution/read-back and the new cadence measurement remain pending; required CI, schema application and deployment/smoke are complete.

### Outstanding work and handoff boundary

- Complete monitoring first observer read-back and final receipts, then record observed freshness and the remaining cadence measurement honestly.
- Finish final main-based CI and release of the reviewed scope correction, compare a fresh production dry run to its preserved expectation, resolve applicable apply authorization, then verify the authorized repair and zero-change rerun. The earlier apply question is on hold.
