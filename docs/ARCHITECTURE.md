...
**Medication entry:** One screen (`components/request/steps/medication-step.tsx`) — the patient types one medication name per request (with optional strength/form) or describes it, then answers when it was last prescribed, their dose & frequency, whether the regimen changed, what it treats, and side effects. The separate `medication-history` step was **merged in on 2026-07-17** (P2.1): the second screen was never independently answerable, so it only cost a page turn on the weakest paid path. Answer keys were unchanged by the merge; the "who prescribed it last" field (optional, desktop-only, read by nothing) was dropped. The PBS reference combobox + `GET /api/medications/search` lookup (and `lib/clinical/pbs-client.ts`) were **retired 2026-06-28** (#208 + dead-code sweep): the lookup was slow and read as a hard "search and select from a list" gate, while the doctor confirms the exact medicine in Parchment/MIMS at prescribing time anyway.

**Repeat-Rx required answers:** dose & frequency (`currentDose`, mirrored to `dosageInstructions`), `indication`, and the unchanged-regimen attestation (`doseChanged`) are mandatory (operator decision 2026-06-26); `hasSideEffects` is a required safety field at checkout. Editing the medicine or the dose text clears `doseChanged` — the attestation belongs to the exact regimen the patient reviewed (`lib/clinical/repeat-rx-attestation.ts`). Checkout re-validates the merged step through **both** `validateMedicationStep` and `validateMedicationHistoryStep` (`lib/request/unified-checkout.ts`); the merge was composition-only and did not collapse those schemas.
