...
| `supabase/migrations/` | 101 SQL migration files (1 squashed baseline + 100 incremental). Most recent: `20260719164324_privacy_safe_flow_instance_attribution.sql` (adds nullable UUID `flow_instance_id` columns to `partial_intakes` and `intakes` for personless attempt-level conversion attribution; applied and verified in production 2026-07-20 with both columns nullable, typed as UUID, and documented as non-clinical metadata). |
