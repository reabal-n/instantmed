...
**Confirmed-payment finalization:** `lib/stripe/confirmed-payment-finalization.ts` is the sole owner of the exact-current Checkout Session -> paid compare-and-swap. Both paid Stripe webhook handlers and the authenticated `/api/stripe/verify-payment` fallback delegate to it. The finalizer requires Stripe-confirmed payment, exact `intakes.payment_id = session.id`, a retryable intake/payment state, and a numeric `session.amount_total`; one guarded write reconciles paid state, Stripe identifiers, `checkout_error`, and the charged amount. A zero-row result is re-read and classified as a concurrent paid winner, stale Session, non-retryable state, or retryable update failure instead of being guessed from transport context. The same module owns idempotent completion work (review/draft start, referral reconciliation, PostHog, Google Ads, request-received outbox send, in-app notification, and ledgered Telegram alert), so a browser fallback that settles first cannot leave a clinically paid request without the webhook-owned side effects. Webhook adapters translate typed failures into the DLQ; the authenticated fallback returns a bounded patient-safe error and lets Stripe retry through the webhook.
...
| `supabase/migrations/` | 134 SQL migration files (1 squashed baseline + 133 incremental). Latest on disk: `20260903120000_recovery_email_engagement.sql`; it adds a nullable, non-PHI, first-write-wins recovery-email engagement marker to `intakes`. Latest applied and verified production migration: `20260828090000_specialty_experience_attribution.sql`. On-disk presence is not deployment proof; linked migration history and post-apply receipts own production status. |
