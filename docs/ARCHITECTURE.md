...
**`amount_cents` is the source of truth for refund math** and must equal what Stripe actually charged. The paid-transition webhook handlers (`checkout-session-completed.ts`, `checkout-session-async-payment-succeeded.ts`) reconcile `amount_cents = session.amount_total` so referral-credit coupons (a Stripe `amount_off` coupon) and the Express priority fee (an extra line item) don't desync the stored amount from the charge. Seeding the list price instead made `issueRefundAction()` refund **more** than the charge for coupon customers (Stripe rejects "Refund amount > charge amount") and **less** for Express customers (short by $9.95). Note: orders paid before 2026-06-07 may still carry the list price in `amount_cents`; reconcile those from Stripe if a legacy refund fails.
...
| **Misc** (14) | Various | `/api/draft`, `/api/certificates/[id]/download`, `/api/health`, `/api/medications/search`, `/api/og`, `/api/verify`, `/api/unsubscribe`, `/api/search`, `/api/profile/ensure`, `/api/attribution/heard` (token-authed self-reported "how did you hear about us?" write; POST in-app + GET one-click for email) |
...
| `app/api/` | API routes (87 route files) | `stripe/webhook/`, `cron/`, `health/`, `certificates/`, `intakes/` |
