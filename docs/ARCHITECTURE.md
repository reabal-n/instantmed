...
| `supabase/migrations/` | 130 SQL migration files (1 squashed baseline + 129 incremental). Latest: `20260819055501_allow_partially_refunded_fulfilment.sql`. The current release tranche spans Stripe dispute/refund recovery, cron outcomes, future-review retry, support-refund role casts, exact Codex Ads approval, and partial-refund fulfilment from `20260814182000` through `20260819055501`. On-disk presence is not deployment proof; linked migration history and post-apply receipts own production status. |
