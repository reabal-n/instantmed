...
| **Cron** (27) | `/api/cron/*` | See OPERATIONS.md for full cron table |
...
| `lib/notifications/` | Alerts | `telegram.ts` + `paid-request-telegram.ts` (request-only Telegram notifications), `service.ts` (payment notifications), `support-inbox-alert.ts` + processor (manual aggregate-only support count diagnostics) |
...
| File | Role |
|------|------|
| `lib/clinical/auto-approval-state.ts` | Atomic CAS state transitions and Sentry/PostHog observability |
