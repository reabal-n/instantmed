...
**Template types:** `BaseEmail` is the only React email shell. Legacy retry reconstruction supports core transactional and lifecycle types including `med_cert_patient`, `med_cert_employer`, `script_sent`, `request_declined`, `consult_approved`, `still_reviewing`, and `review_request`; new outbox rows replay their frozen encrypted provider body instead. `partial_intake_recovery` also retries from that frozen body because its bearer resume URL is intentionally not reconstructable from plaintext metadata; a legacy row without the encrypted body fails terminally and alerts. `ReviewRequestEmail` is the only patient email that asks for a public review. Medical-certificate, prescription, and consult outcome templates are transactional only and contain no review, attribution, or referral block. (The dead specialty `*_approved`, `welcome`, `verification_code`, `intake_submitted`, `payment_retry`, `referral_credit`, and reusable email referral/review blocks were removed — real prescription approvals notify via `script_sent`.) Supabase Auth send-email templates cover `magiclink`, `signup`, and `recovery` via `app/api/webhooks/supabase-auth/route.ts`. Marketing/engagement types are capped by warmup limits; transactional clinical/payment sends are not.

**Review-request delivery:** The daily route is invoked at both UTC offsets that can correspond to 10:00 Australia/Sydney and a timezone guard selects the single valid run across AEST/AEDT. Eligibility begins 48 hours after confirmed patient-visible fulfilment (`document_sent_at` for medical certificates; `script_sent_at` for prescription/consult) and remains catch-up eligible for 120 days, so a missed run or an aged-past-72-hour cohort is not dropped. Only paid, approved/completed requests with no prior marker qualify. Initial selection excludes requests already owned by pending/sending outbox rows, leaving those retries to the dispatcher without starving newer catch-up work. The outbox idempotency key enforces one ask per request; a 30-day patient cooldown spans different requests. Candidate selection and dispatcher retry both re-check state, bounce/complaint suppression, and `canSendMarketingEmail` immediately before the provider call. The encrypted frozen provider body's sole recipient must also match the current authoritative recipient before every provider attempt. The lifecycle marker is written after durable provider confirmation or a terminal policy suppression; marking suppressed requests prevents an oldest-first catch-up batch from repeatedly selecting the same unsendable cohort, and does not count those rows as delivered.

**Partial-intake recovery delivery:** The hourly finder owns all unconverted anonymous drafts, including drafts currently on review or checkout steps; abandoned-checkout recovery owns only submitted intakes. Initial candidates are inclusively 60 minutes to 6 hours idle, using one captured clock value, and must have neither a sent nor suppression marker. The sender then re-reads the authoritative draft by non-bearer `recovery_tracking_id` and verifies the exact candidate timestamp, recipient, conversion/expiry state, resumable service/subtype route, bounce state, and email suppression immediately before every provider attempt. Dispatcher retries repeat those checks without the initial six-hour age ceiling. The explicit outbox key is `partial-intake-recovery:<recovery_tracking_id>`, so two drafts at one address remain distinct. Plaintext outbox business metadata contains only that tracking ID; the bearer `session_id` and resume URL exist only in memory and inside the encrypted frozen provider body. Before Resend, the decrypted body's sole recipient and exact resume URL must match the current authoritative draft. A changed recipient/route fails closed without replaying stale bearer content. `recovery_email_sent_at` is written only after the outbox `sent` state is durable; terminal policy decisions use the separate `recovery_email_suppressed_at` marker. A service-role reconciler uses a read-only RPC to find and heal missing sent markers from confirmed outbox proof without resending.
...
| `lib/email/send-email.ts` | 2,235 lines | Central provider boundary, durable outbox send, and dispatcher retry orchestration |
