...
| **Misc** (14) | Various | `/api/draft`, `/api/certificates/[id]/download`, `/api/health`, `/api/og`, `/api/verify`, `/api/unsubscribe`, `/api/search`, `/api/profile/ensure`, `/api/attribution/heard` (token-authed self-reported "how did you hear about us?" write; POST in-app + GET one-click for email), `/api/last-reviewed` (public PHI-free freshness beacon: timestamp of the latest completed review, 60s cache, seeded-E2E rows excluded; feeds the homepage "Last reviewed" signal) |
...
| `supabase/migrations/` | 120 SQL migration files (1 squashed baseline + 119 incremental). Most recent: `20260814181000_align_manual_delivery_reconciliation_lock_order.sql` — forward-only reconciliation hardening that aligns the manual-delivery receipt with the intake-then-certificate lock order used by certificate revocation and preserves the actor on the append-only ledger with a restrictive foreign key; it changes no existing attestation, delivery, provider, intake, or certificate state. Before it, `20260814180000_historical_auto_issued_review_lane.sql` — fixed nine-case, admin-only Medical Director retrospective with same-actor contextual-open evidence and an append-only no-correction receipt bound to the exact current certificate storage version; correction continues through the existing transactional revoke path, and neither outcome is inferred or bulk-written. Before it, `20260814171919_harden_audit_function_search_paths.sql` — pins the ten functions identified by the security advisor without changing their bodies or grants, and indexes the manual-delivery ledger's nullable actor foreign key. Before it, `20260814163645_reconcile_manual_certificate_delivery.sql` — append-only, admin-attested legacy manual-delivery evidence bound to the exact current certificate storage version; it clears false resend alerts without rewriting provider email state, the intake delivery mirror, an unknown historical delivery time, or certificate validity. Before it, `20260814123000_restore_profiles_consent_myhr.sql` — forward-only clean-replay repair restoring the production-shaped nullable `profiles.consent_myhr BOOLEAN DEFAULT FALSE` column omitted from the migration squash, with no row rewrite or backfill. Before it, `20260814120000_review_request_funnel_lifecycle_truth.sql` — aggregate-only, point-in-time review-request lifecycle reconciliation with service-role-only access. Before it, `20260814113000_retire_replay_only_intake_triggers.sql` — non-cascading replay hygiene that removes obsolete baseline-only intake audit/notification writers after their target table becomes a compatibility view while preserving valid production behavior. |
