...
| `supabase/migrations/` | 131 SQL migration files (1 squashed baseline + 130 incremental). Latest: `20260823101500_fix_partially_refunded_review_claim.sql`. The current release tranche spans Stripe dispute/refund recovery, cron outcomes, future-review retry, support-refund role casts, exact Codex Ads approval, partial-refund fulfilment, and the canonical doctor-claim repair from `20260814182000` through `20260823101500`. On-disk presence is not deployment proof; linked migration history and post-apply receipts own production status. |
