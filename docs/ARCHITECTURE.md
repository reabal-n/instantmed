...
| `supabase/migrations/` | 107 SQL migration files (1 squashed baseline + 106 incremental). Most recent: `20260727180000_google_ads_agent_control_plane.sql` (pending production application until the read-only shadow rollout; adds PHI-free run, proposal, and experiment state with RLS default-deny and service-role-only grants, plus the Stripe fee-cache fields required for fee-aware contribution). |
