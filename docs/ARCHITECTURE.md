...
**Guest request access:** `/track/[token]` is a server-only capability exchange. It validates a purpose-scoped, seven-day HMAC token, stores it in an HttpOnly, SameSite=Lax cookie scoped to `/track`, and redirects immediately to the clean `/track/request` URL. That page exposes only a read-only status projection and service label; it never exposes the token, doctor question, clinical reply surface, full intake row, Realtime subscription, or queue-position estimate. Closed, merged, non-patient, and otherwise invalid profiles fail closed. A signed-in exact owner is redirected to the canonical patient request; everyone else must sign in or create an account through a fixed return URL before reading clinical messages or responding. The capability surface is private/no-store, no-referrer, noindex, and excluded from client analytics and attribution.
...
| `e2e/` | 82 TypeScript files, including 70 specs and `helpers/` (seed/teardown, auth bypass, production-synthetic side-effect isolation). Focused paid-flow and ops smoke specs are the blocking CI gate. |
