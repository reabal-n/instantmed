...
| `supabase/migrations/` | 132 SQL migration files (1 squashed baseline + 131 incremental). Latest on disk: `20260825073433_scope_profiles_realtime_policy_to_authenticated.sql`; it limits all three RLS policies that invoke `is_doctor()` to authenticated clients and verifies their catalog role scope. The latest applied and verified production migration remains `20260823101500_fix_partially_refunded_review_claim.sql`. On-disk presence is not deployment proof; linked migration history and post-apply receipts own production status. |
