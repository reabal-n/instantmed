...
**Guest request access:** `/track/[token]` is a server-only capability exchange. It validates a purpose-scoped, seven-day HMAC token, stores it in an HttpOnly, SameSite=Lax cookie scoped to `/track`, and redirects immediately to the clean `/track/request` URL. That page exposes only a read-only status projection and service label; it never exposes the token, doctor question, clinical reply surface, full intake row, Realtime subscription, or queue-position estimate. Closed, merged, non-patient, and otherwise invalid profiles fail closed. A signed-in exact owner is redirected to the canonical patient request; everyone else may choose **Email me a secure access link** or use standard sign-in. The empty, CSRF-protected POST to `/track/request/access-link` resolves the owner mailbox only on the server and sends a real Supabase PKCE link through a fixed clean return URL. Account creation/linking remains optional; authenticated ownership is required before reading clinical messages or responding. The capability surface is private/no-store, no-referrer, noindex, and excluded from client analytics and attribution.
