...
**Staff cockpit architecture:** `/admin` is the primary combined operator surface. It uses `OperatorShell`, shared navigation from `lib/dashboard/staff-navigation.ts`, `StaffCommandPalette` for fast jumps, live sidebar counts from `lib/data/staff-nav-counts.ts`, and compact summaries from `lib/doctor/case-summary.ts`. Dedicated Scripts navigation deep-links to `/admin?status=scripts#doctor-queue`; Patients deep-links to `/admin/patients` and carries the active prescribing-identity blocker count. `/admin/intakes/[id]`, `/admin/patients`, `/admin/patients/[id]`, and `/doctor/intakes/[id]` share the same cockpit mental model: patient detail, request summary, blockers, and decision actions stay together instead of forcing a manual switch between admin and doctor views. The patient directory is sortable by most recent request, request type, last script, name, or joined date.
