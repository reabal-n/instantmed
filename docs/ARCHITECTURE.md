...
| `supabase/migrations/` | 114 SQL migration files (1 squashed baseline + 113 incremental). Most recent: `20260814113000_retire_replay_only_intake_triggers.sql` — non-cascading replay hygiene that removes obsolete baseline-only intake audit/notification writers after their target table becomes a compatibility view, and conditionally removes an archival RPC only when its required legacy schema is absent, while preserving live production behavior. Before it, `20260814110000_make_stuck_intakes_reportable.sql` — private `security_invoker` stuck-intake view repair that exposes reportability keys, binds medical-certificate delivery to the latest valid document, aligns paid/partially-refunded fulfilment obligations for System Health and Operations, clears delivery proof on current-certificate revocation, and makes both provider reconciliation and legacy mirror repair service-role-only, storage-version-locked transactions. Before it, `20260812154500_merge_intake_risk_flags_atomically.sql` — service-role-only row-locked merge that deduplicates doctor-visible intake flags and preserves the highest severity. |
