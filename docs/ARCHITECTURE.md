...
**Staff cockpit architecture:** `/admin` is the primary combined operator surface. It uses `OperatorShell`, shared navigation from `lib/dashboard/staff-navigation.ts`, `StaffCommandPalette` for fast jumps, live sidebar counts from `lib/data/staff-nav-counts.ts`, and compact summaries from `lib/doctor/case-summary.ts`. Dedicated Queue navigation deep-links to `/admin?status=review#doctor-queue`, Scripts navigation deep-links to `/admin?status=scripts#doctor-queue`, and Patients deep-links to `/admin/patients` with the active prescribing-identity blocker count. Owner-operator doctor setup lives inside the admin shell at `/admin/settings/doctor-identity`, reusing the doctor identity client for MFA, availability, certificate identity, signature, avatar, and Parchment linking without requiring a switch to the doctor portal. `/admin/intakes/[id]`, `/admin/patients`, `/admin/patients/[id]`, and `/doctor/intakes/[id]` share the same cockpit mental model: patient detail, request summary, blockers, and decision actions stay together instead of forcing a manual switch between admin and doctor views. The patient directory is sortable by most recent request, request type, last script, name, or joined date.
...
| `subscriptions` | Dormant/legacy Repeat Rx subscription compatibility, not active acquisition | `profiles.id` via `profile_id` |
...
profiles
  ...
  +-- subscriptions (profile_id, legacy compatibility)
```
...
| **Doctor** | `/api/doctor/*` | `assign-request`, `certificates/[intakeId]/download`, `drafts/[intakeId]`, `export`, `intakes/[id]/audit-view`, `intakes/[id]/lock`, `intakes/[id]/review-data`, `log-view-duration`, `monitoring-stats`, `onboarding-status`, `patients/[patientId]/health-profile`, `patients/[patientId]/summary`, `scripts`, `scripts/[id]` |
...
| **Stripe Portal** (1) | `/api/stripe/customer-portal` | POST → creates Stripe billing portal session for historical subscription/account management |
| **Webhooks** (5) | `/api/stripe/webhook`, `/api/stripe/verify-payment`, `/api/webhooks/resend`, `/api/webhooks/telegram`, `/api/webhooks/parchment` | Per-provider signature verification; subscription invoice/deletion handlers are compatibility-only while acquisition is inactive; includes `prescription.created` (Parchment) |
