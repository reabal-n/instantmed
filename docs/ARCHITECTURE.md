...
| `supabase/migrations/` | 99 SQL migration files (1 squashed baseline + 98 incremental). Most recent: `20260719101500_review_request_candidate_anti_join.sql` (moves durable-owner exclusion into a bounded service-role RPC so PostgREST row and URL limits cannot recycle owned review requests; applied and verified in production 2026-07-19: migration history aligned, schema lint clean, service-role execution succeeds, and anon execution is denied). |
