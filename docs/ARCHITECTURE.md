...
**Patient context:** `PatientProfilePanel` loads only after an explicit open and returns saved health-profile context, three prior relevant events, and aggregate request/note counts without echoing the active request. The canonical full record uses the existing Tabs primitive: Clinical for saved profile, identity/contact, and recent clinical activity; History for complete request/prescription/note chronology; Operations for Parchment and identity administration, full first/last acquisition attribution, email/audit activity, and duplicate tools. `PatientTimeline` remains the shared renderer, with callers supplying only the relevant channels and empty filters hidden.
