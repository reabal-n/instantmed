...
| `supabase/migrations/` | 100 SQL migration files (1 squashed baseline + 99 incremental). Most recent: `20260719113000_partial_recovery_candidate_anti_join.sql` (moves partial-recovery durable-owner exclusion into a bounded service-role RPC so every existing outbox status wins before the oldest-first candidate limit; applied and verified in production 2026-07-19: migration history aligned, schema lint clean, service-role execution succeeds, and anon execution is denied). |
