...
**Patient UX:** Plain input with a "describe it if you're unsure" prompt and recent-medication quick-pick chips. Clinician view: patient-requested medicine context plus a doctor-only generic-name copy action when `lib/clinical/generic-medication-resolver.ts` finds one exact, unambiguous match in InstantMed's curated `medications` table. If the direct lookup is unresolved, `lib/clinical/prior-medication-match.ts` may identify one narrowly bounded typo candidate among the same patient's non-cancelled prior prescriptions after the server action verifies the doctor/intake relationship; that prior name must still exact-resolve through the curated table. This is not general fuzzy catalog search. The authenticated server action reads only generic names and brand aliases plus the bounded same-patient history; it performs no external medication lookup and never trusts a client-supplied active-ingredient field. Unknown, ambiguous, unauthorised, or unsafe-looking results expose no copy action. Clipboard content is the verified generic name alone—never strength, form, dose, directions, or broader Parchment context. The resolver is reference-only and cannot affect eligibility, safety flags, status, or prescribing.
