...
| `supabase/migrations/` | 106 SQL migration files (1 squashed baseline + 105 incremental). Most recent: `20260723170000_create_operational_metrics.sql` (additive restoration of the aggregate-only metrics table omitted from the existing production database after migration squashing; RLS enabled, direct browser roles denied, service-role select/insert only; production application is release-gated). |
