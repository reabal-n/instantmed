...
**Repeat-Rx required answers:** concrete medicine strength (structured or reliably inferred inline), current amount/directions **and** frequency (`currentDose`, mirrored to `dosageInstructions`), `indication`, and the unchanged-regimen attestation (`doseChanged`) are mandatory; `hasSideEffects` is a required safety field at checkout. A frequency starter such as `Once daily` is not a dose and cannot satisfy the current-regimen field on its own. Editing the medicine or the dose text clears `doseChanged` — the attestation belongs to the exact regimen the patient reviewed (`lib/clinical/repeat-rx-attestation.ts`). Checkout re-validates the merged step through **both** `validateMedicationStep` and `validateMedicationHistoryStep` (`lib/request/unified-checkout.ts`) plus the canonical repeat payload validator. New submissions persist the versioned dose-contract marker owned by `lib/clinical/repeat-rx-dose-requirement.ts`; authenticated retry and signed guest resume apply the same two-field check only when that marker is present, preserving payment recovery for every request saved before the new code was active.
