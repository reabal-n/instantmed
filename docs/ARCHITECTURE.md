...
**Review-request delivery:** The daily route is invoked at both UTC offsets that can correspond to 10:00 Australia/Sydney and a timezone guard selects the single valid run across AEST/AEDT. Eligibility begins 48 hours after confirmed patient-visible fulfilment (`document_sent_at` for medical certificates; `script_sent_at` for prescription/consult) and remains catch-up eligible for 120 days, so a missed run or an aged-past-72-hour cohort is not dropped. Only paid, approved/completed requests with no prior marker qualify. The outbox idempotency key enforces one ask per request; a 30-day patient cooldown spans different requests. Candidate selection and dispatcher retry both re-check state, bounce/complaint suppression, and `canSendMarketingEmail` immediately before the provider call. The lifecycle marker is written after durable provider confirmation or a terminal policy suppression; marking suppressed requests prevents an oldest-first catch-up batch from repeatedly selecting the same unsendable cohort, and does not count those rows as delivered.
