-- Keep doctor Telegram alerts as "new request" alerts only.
-- Old paid rows with a missing sent marker should stay visible in ops/queue,
-- not suddenly ping the doctor hours or days later.
...
SET
  ...
  paid_request_telegram_error = 'Suppressed from Telegram retry because paid_at is older than the 30 minute new-request alert window.',
  updated_at = now()
...
BEGIN
  ...
  WHERE i.id = p_intake_id
    ...
    AND i.paid_at >= p_claimed_at - interval '30 minutes'
...
COMMENT ON FUNCTION public.claim_paid_request_telegram_notification(uuid, timestamptz, timestamptz) IS
  'Atomically claims a recent paid-request Telegram notification before the external Telegram API call.';
