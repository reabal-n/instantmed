-- Keep duplicate patient reconciliation complete after Parchment prescription sync.
-- The previous merge RPC predates local `prescriptions` rows, so synced scripts
-- could remain attached to the archived duplicate profile.
...
DECLARE
  ...
  legacy_consents_table regclass;
BEGIN
  duplicate_ids := ARRAY(
  ...
  WHERE patient_id = ANY(duplicate_ids);
  GET DIAGNOSTICS moved_count = ROW_COUNT;
  reference_counts := reference_counts || jsonb_build_object('prescriptions', moved_count);
  ...
  legacy_consents_table := to_regclass('public.' || 'consents');
  IF legacy_consents_table IS NOT NULL THEN
    EXECUTE format('UPDATE %s SET patient_id = $1 WHERE patient_id = ANY($2)', legacy_consents_table)
