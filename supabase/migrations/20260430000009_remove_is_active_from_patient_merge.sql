-- Production drift fix: live profiles does not expose is_active. The archive
-- marker for merged duplicates is merged_into_profile_id, so do not depend on
-- the legacy is_active column.
