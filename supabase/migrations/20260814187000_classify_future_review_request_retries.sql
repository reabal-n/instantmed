-- Keep future-scheduled review-request retries out of the actionable backlog.
--
-- The lifecycle aggregate already treats a newly eligible request with no
-- outbox owner as awaiting the next scheduled run. A retry reservation can
-- also be deliberately deferred by the marketing warmup policy. Once that
-- pending/sending row exists, it is still expected work until scheduled_for;
-- it must not be presented as a missed send before that timestamp arrives.
  ...
  outbox_evidence as (
    select
      ...
      coalesce(bool_or(
        ...
        and outbox.scheduled_for > bounds.as_of
      ), false) as has_future_scheduled_owner
...
comment on function public.get_review_request_funnel(
  ...
  'Returns aggregate-only, point-in-time review-request lifecycle counts. Future-scheduled pending or sending owners remain awaiting work until their schedule arrives; primary buckets reconcile exactly to eligible.';
