      ...
      <div className={cn("space-y-6 animate-in fade-in duration-300", className)}>
        ...
        <div className="grid gap-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-20 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 animate-pulse"
  ...
  if (variant === "review") {
    return (
      <div className={cn("space-y-6 animate-in fade-in duration-300", className)}>
        ...
        <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-5 space-y-4">
          {[1, 2, 3, 4].map((i) => (
      ...
      <div className="space-y-5">
        {[1, 2, 3].map((i) => (
          <div key={i} className="space-y-2" style={{ animationDelay: `${i * 100}ms` }}>
            <div className="h-4 w-24 bg-slate-200 dark:bg-slate-700 rounded animate-pulse" />
            <div className="h-12 w-full bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 animate-pulse" />
          </div>
        ))}
      </div>
      
      {/* Button skeleton */}
      <div className="h-12 w-full bg-slate-200 dark:bg-slate-700 rounded-xl animate-pulse mt-8" />
    </div>
  )
}
