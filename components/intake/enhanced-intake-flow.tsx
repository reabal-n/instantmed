 ...
 * - Route: `/request?service=med-cert` or `/request?service=prescription`
 * 
...
export function EnhancedIntakeFlow({
  ...
  const updateField = useCallback(
    ...
    [isAuthenticated, validateField, state.service, state.certType]
  )
  ...
  const renderStep = () => {
    switch (step) {
      ...
      // ======= SAFETY CHECK =======
      // Uses shared EmergencyGate component for consistency across all flows
      case "safety":
        // If already confirmed, show compact confirmation
        if (state.safetyConfirmed) {
          return (
            <motion.div className="space-y-4">
              ...
              {/* Show symptom checker results if any symptoms were flagged */}
              {symptomCheckResult.severity !== "ok" && (
                <SymptomChecker
        ...
        // Show the shared EmergencyGate for initial acknowledgment
        return (
          <motion.div className="space-y-4">
            <EmergencyGate
              serviceName={
                state.service === "med-cert" ? "Medical certificate requests" :
                state.service === "repeat-script" ? "Repeat prescription requests" :
                state.service === "new-script" ? "New prescription requests" :
                "This service"
              }
              onAcknowledge={() => {
                updateField("safetyConfirmed", true)
              }}
            />
            
            {/* Show symptom checker if critical symptoms detected from earlier input */}
