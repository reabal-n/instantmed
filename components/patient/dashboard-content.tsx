...
export function DashboardContent({
  ...
  // NOTE: Cast to string for comparisons — some statuses exist in DB but not yet in TS enum
  const needsAction = displayRequests.filter(
    (r) => (r.payment_status as string) === "pending_payment" || (r.status as string) === "needs_follow_up"
  )
  const inProgress = displayRequests.filter(
    (r) => (r.status as string) === "pending" && (r.payment_status as string) !== "pending_payment"
  )
        ...
        <section aria-labelledby="needs-action-heading">
          ...
          >
            {needsAction.map((request, index) => (
              <li key={request.id}>
                <PremiumRequestCard
                  title={formatCategory(request.category) || "Request"}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                            {formatCategory(request.category) || "Request"}
                          </p>
