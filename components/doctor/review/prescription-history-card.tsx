...
export function PrescriptionHistoryCard({ patientId, history, reload }: PrescriptionHistoryCardProps) {
  ...
  async function refresh() {
    ...
    setMessage(null)
    try {
      const result = await refreshPatientParchmentPrescriptionsAction(patientId)
  ...
  return (
    <div className="space-y-2" data-testid="prescription-history">
      <PatientTimeline
        ...
        emptyLabel={history?.error || (history ? "No prescriptions recorded in InstantMed. Refresh from Parchment to check for additional records." : "Prescription history has not loaded.")}
      />
      <div className="flex flex-wrap items-center gap-2 px-1">
        <Button variant="outline" size="sm" onClick={refresh} disabled={refreshing}>
          <RefreshCw className="mr-2 h-3.5 w-3.5" aria-hidden />
          {refreshing ? "Refreshing prescriptions…" : "Refresh from Parchment"}
        </Button>
