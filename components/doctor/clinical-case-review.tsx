...
interface ClinicalCaseReviewProps {
   ...
   * current-request prescribing facts.
   */
...
export function ClinicalCaseReview({
  ...
  const summary = suppliedSummary ?? buildClinicalCaseSummary({
    category,
    subtype,
    serviceType,
    patientName,
  ...
  const visibleFacts = hideRequestFacts
    ? []
    : compact
      ? scannableFacts.slice(0, COMPACT_FACT_LIMIT)
      : scannableFacts
  ...
  const draftNoteLabel = needsRecordedScriptReconciliation
    ...
    : isEditableDraftNote
      ? "Draft note · Review required"
      : "Clinical note · Review required"
  const clinicalNoteSection = (
    <Collapsible
      open={draftNoteOpen}
      onOpenChange={onDraftNoteOpenChange}
      className="rounded-xl border border-border/60 bg-card shadow-sm shadow-primary/[0.035]"
    >
      <div
        className={cn(
      ...
      >
        <CollapsibleTrigger className="group flex min-w-0 items-center gap-2 text-left">
          ...
          <span className="truncate text-xs font-semibold text-foreground">{draftNoteLabel}</span>
          <ChevronDown
