...
const SERVICE_CONFIG: Record<string, { name: string; price: string; href: string; wait: string }> = {
  "/medical-certificate": { name: "Medical Certificate", price: `from ${PRICING_DISPLAY.MED_CERT}`, href: "/medical-certificate/new", wait: "Doctor reviewed" },
  "/prescriptions": { name: "Prescription", price: PRICING_DISPLAY.REPEAT_SCRIPT, href: "/prescriptions/new", wait: "Doctor reviewed" },
}

const DEFAULT_CONFIG = { name: "See a doctor", price: `from ${PRICING_DISPLAY.MED_CERT}`, href: "/request", wait: "Doctor reviewed" }
      ...
      <div className="bg-card dark:bg-card border-t border-border/10 dark:border-white/10 px-4 py-3 safe-area-inset-bottom shadow-2xl dark:shadow-none">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="min-h-[44px] min-w-[44px] flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors shrink-0"
            aria-label="Dismiss"
          >
            <X className="w-4 h-4" />
          </button>
