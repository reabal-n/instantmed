...
describe("Parchment ops dashboard and retry contract", () => {
  it("exposes a focused admin Parchment ops page with retryable webhook failures", () => {
    ...
    expect(pageSource).toContain("Actionable failures")
    expect(pageSource).toContain("Historical sandbox failures")
    ...
    expect(pageSource).toContain("actionableFailures.length > 0 || dashboard.stats.unlinkedPrescribers > 0")
