...
describe("Parchment ops dashboard and retry contract", () => {
  it("exposes a focused admin Parchment ops page with retryable webhook failures", () => {
    ...
    expect(pageSource).toContain("Actionable webhook failures")
    expect(pageSource).toContain("Historical sandbox webhook failures")
    expect(pageSource).toContain("Actionable failures 7d")
    expect(pageSource).toContain("Not synced yet")
    ...
    expect(pageSource).toContain("dashboard.stats.retryableFailures > 0 || dashboard.stats.unlinkedPrescribers > 0")
    ...
    expect(opsSource).toContain("NON_ACTIONABLE_SANDBOX_FAILURE_REASONS")
