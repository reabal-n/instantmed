...
describe("staff readiness panel contract", () => {
  it("surfaces staff role, owner doctor, Sentry, and dashboard smoke readiness in the admin dashboard", () => {
    ...
    expect(readinessDataSource).toContain("DASHBOARD_SMOKE_COOKIE_HEADER")
    expect(readinessDataSource).toContain("getDoctorOnboardingState")
    expect(panelSource).toContain("Release readiness")
    expect(panelSource).toContain("staff roles, owner doctor setup, Sentry, and authenticated dashboard smoke")
  })
