...
describe("ordered monitoring evidence", () => {
  ...
  it("suppresses unchanged alerts, accepts decreases, then pages 5 to 4 to 5", () => {
    ...
    expect(state.incidents[0].at).toBe(1)
    state = advanceIncidents(state.incidents, [{ metric: 0, severity: 1, count: 4 }], [0], 3)
    expect(state.events).toHaveLength(0)
    expect(state.incidents[0]).toMatchObject({ at: 1, count: 4 })
    const rebound = advanceIncidents(state.incidents, [{ metric: 0, severity: 1, count: 5 }], [0], 4)
    expect(rebound.events).toHaveLength(1)
    expect(rebound.incidents[0].at).toBe(4)
  })
