...
describe("business operating scorecard contract", () => {
  ...
  it("replaces the input-placeholder scorecard with delivered fee-aware evidence", () => {
    ...
    expect(pageSource).toContain("getLatestDeliveredAdsAgentRun")
    expect(pageSource).toContain("buildBusinessReadModel")
    expect(pageSource).not.toContain("getBusinessOperatingScorecard")
    expect(clientSource).toContain("30d net retained")
    expect(clientSource).toContain("First-order contribution")
    expect(clientSource).toContain("Ads-attributed net retained − Stripe fees − spend")
    expect(clientSource).toContain("Every Google Ads mutation still requires exact operator approval")
    expect(clientSource).not.toContain("Inputs still needed")
    expect(clientSource).not.toContain("Below-capacity contribution")
