...
describe("guest profile auth linking", () => {
  ...
  it("keeps shared auth paths on deterministic guest-linking logic", () => {
    const authHelperSource = readFileSync(join(process.cwd(), "lib/auth/helpers.ts"), "utf8")
    ...
    expect(postSignInSource).toContain("getPreferredGuestProfileIdForIntake")
  })
})
