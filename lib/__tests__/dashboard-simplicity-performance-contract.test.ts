...
describe("dashboard simplicity and runtime performance contracts", () => {
  ...
  it("keeps the doctor queue header focused on current operational pressure", () => {
    ...
    expect(queueClientSource).toContain("visibleNextIntakes")
    expect(queueClientSource).toContain("nextIntakes={filteredIntakes.slice(0, 3)}")
    expect(queueClientSource).toContain("nextCaseButtonLabel")
  ...
  it("keeps split-view loading and row chrome calm under interaction", () => {
    ...
    expect(reviewPanelSource).toContain("Loading patient record")
    ...
    expect(reviewPanelSource).toContain("Waiting for the patient's answers")
  ...
  it("keeps clinical action colours muted on the doctor surface", () => {
    ...
    expect(availabilityToggleSource).toContain("#6B8A76")
    ...
    expect(actionButtonsSource).toContain("Decline returns")
    expect(actionButtonsSource).toContain("Confirm first.")
    ...
    expect(actionButtonsSource).toContain("Pre-flight checks")
    ...
    expect(actionButtonsSource).toContain('completeLabel: "Note drafted"')
    expect(actionButtonsSource).toContain('`${check.label} done`')
    expect(actionButtonsSource).toContain("h-7")
    expect(actionButtonsSource).toContain("text-destructive")
    expect(actionButtonsSource).toContain("text-destructive/80")
  ...
  it("keeps the review pane decision controls visible before scrolling", () => {
    ...
    expect(actionSource).toContain("Approve & send")
    expect(actionSource).toContain('"Request symptoms"')
    expect(actionSource).toContain("Review & send")
    ...
    expect(clinicalCaseReviewSource).toContain("What the patient told us")
    ...
    expect(clinicalCaseReviewSource).toContain("focus:min-h-[220px]")
    expect(clinicalCaseReviewSource).toContain("max-h-[260px]")
  ...
  it("keeps complete patient states neutral while missing states stay prominent", () => {
    ...
    expect(decisionStripSource).toContain('compact ? "text-xs text-slate-400"')
  ...
  it("keeps the patient profile drawer as a fast reference surface", () => {
    ...
    expect(source).toContain('Tabs defaultValue="identity"')
    expect(source).toContain('value="identity"')
    expect(source).toContain('value="contact"')
