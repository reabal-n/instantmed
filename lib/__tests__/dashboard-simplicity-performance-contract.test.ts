...
describe("dashboard simplicity and runtime performance contracts", () => {
  ...
  it("keeps the doctor queue header focused on current operational pressure", () => {
    ...
    expect(queueFiltersSource).toContain("Case open")
    expect(queueFiltersSource).toContain("hasOpenCase")
    expect(queueFiltersSource).toContain('variant={hasOpenCase ? "outline" : "default"}')
    expect(queueFiltersSource).toContain("border-border/70 bg-muted/30 text-muted-foreground")
    expect(queueFiltersSource).toContain("Finish or close the current case before opening the next one.")
  ...
  it("keeps split-view loading and row chrome calm under interaction", () => {
    ...
    expect(read("components/doctor/intake-review-panel.tsx")).toContain("review-pane-in_220ms")
    ...
    expect(read("app/globals.css")).toContain("transform: translateY(4px)")
  ...
  it("keeps clinical action colours muted on the doctor surface", () => {
    ...
    expect(actionButtonsSource).toContain("Declining refunds")
    ...
    expect(actionButtonsSource).toContain("text-rose-700/80")
    expect(actionButtonsSource).toContain('variant="ghost"')
    expect(actionButtonsSource).toContain('tone="tertiary"')
    expect(actionButtonsSource).toContain("Decline & refund ${patientFirstName}")
