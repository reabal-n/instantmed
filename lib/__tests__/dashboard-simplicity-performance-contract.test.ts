...
describe("dashboard simplicity and runtime performance contracts", () => {
  ...
  it("avoids high-frequency whole-queue redraws in the staff cockpit", () => {
    ...
    expect(queueUtilsSource).toContain("postHourCadenceMs")
    expect(queueUtilsSource).toContain("return Math.max(1_000, nextBoundaryMs)")
  ...
  it("keeps the doctor queue header focused on current operational pressure", () => {
    ...
    expect(queuePressureSource).toContain("calculateLiveWaitTime")
    expect(queuePressureSource).toContain("postHourCadenceMs")
    expect(queuePressureSource).not.toContain("liveSecondsLabel")
    expect(queuePressureSource).not.toContain("setInterval(() => setNowMs(Date.now()), 1000)")
    ...
    expect(queueFiltersSource).toContain('mobileLabel: "Scripts"')
    expect(queueFiltersSource).toContain('desktopLabel: "Scripts to write"')
    ...
    expect(queueClientSource).toContain("searchQuery={searchQuery}")
    expect(queueClientSource).toContain("searchDoctorQueueAction")
    expect(queueClientSource).not.toContain("initialSearchQuery")
  ...
  it("keeps clinical action colours muted on the doctor surface", () => {
    ...
    expect(actionButtonsSource).toContain("data-action-readiness-state")
    expect(actionButtonsSource).toContain("data-action-readiness-summary")
    expect(actionButtonsSource).toContain("auditTrailLabel")
    expect(actionButtonsSource).not.toContain("DecisionWaitSignal")
    expect(actionButtonsSource).not.toContain("data-decision-wait-signal")
    expect(actionButtonsSource).toContain("const hasRecordedPrescription")
    expect(actionButtonsSource).toContain("const showPreSendSignals")
    expect(actionButtonsSource).toContain("!hasRecordedPrescription")
    expect(actionButtonsSource.match(/\{showPreSendSignals \? \(/g)).toHaveLength(1)
    expect(actionButtonsSource).not.toContain("QUEUE_WAIT_TARGET_MINUTES")
    expect(actionButtonsSource).not.toContain("getQueueClockTickDelayMs")
    expect(actionButtonsSource).not.toContain("DECISION_WAIT_SIGNAL_CADENCE_MS")
    expect(actionButtonsSource).toContain("Certificate details ready for review.")
    ...
    expect(actionButtonsSource).toMatch(/const canDecline =\s*intake\.script_sent !== true/)
    expect(actionButtonsSource).toContain("sm:order-first sm:mr-auto")
    expect(actionButtonsSource).toContain("border-border/60")
    expect(actionButtonsSource).toContain("Responses captured. Review before you send.")
    expect(actionButtonsSource).toContain("Ready for Parchment · confirm strength there")
    expect(actionButtonsSource).toContain("reviewPacket.advisories[0]")
    expect(actionButtonsSource).toContain("reviewPacket.safety.gaps.length === 0")
    expect(actionButtonsSource).toContain("const incompleteChecks")
    expect(actionButtonsSource).toContain("data-readiness-check")
    expect(actionButtonsSource).toContain('completeLabel: "Identity details captured"')
    expect(actionButtonsSource).toContain('completeLabel: "Safety responses captured"')
    expect(actionButtonsSource).not.toContain("No flags detected")
    ...
    expect(intakePanelSource).toContain("{!inline ? (")
  ...
  it("keeps the review pane decision controls visible before scrolling", () => {
    ...
    expect(requestInfoSource).toContain("packet.facts.find")
    expect(requestInfoSource).toContain("supportingFacts.map")
  })
  ...
  it("keeps complete patient states neutral while missing states stay prominent", () => {
    ...
    expect(decisionStripSource).toContain("snapshot.ageDobLabel")
    expect(decisionStripSource).toContain("snapshot.sex.label")
    expect(decisionStripSource).toContain('label: "Location"')
    expect(decisionStripSource).toContain('label: "Phone"')
    expect(decisionStripSource).not.toContain("First visit")
    expect(decisionStripSource).not.toContain("AttributionChip")
    expect(decisionStripSource).not.toContain("previousIntakeCount")
