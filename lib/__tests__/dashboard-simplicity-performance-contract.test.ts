...
describe("dashboard simplicity and runtime performance contracts", () => {
  ...
  it("keeps split-view loading and row chrome calm under interaction", () => {
    ...
    expect(queueTableSource).toContain("Waiting {waitLabel}")
  ...
  it("keeps clinical action colours muted on the doctor surface", () => {
    ...
    expect(actionButtonsSource).toContain("motion-safe:hover:-translate-y-0.5")
    ...
    expect(actionButtonsSource).toContain("Refunds {refundLabel ?? \"the patient\"} automatically")
  ...
  it("keeps the review pane decision controls visible before scrolling", () => {
    ...
    expect(cockpitSource).toContain("caseReadAcknowledged")
    expect(cockpitSource).toContain("onFocusCapture")
    expect(cockpitSource).toContain("const approvalReadRequired = isApprovalReviewState && !caseReadAcknowledged")
    ...
    expect(actionSource).toContain("approvalReadRequired")
    expect(actionSource).toContain("requiresClinicalDetail")
    expect(actionSource).toContain("Review the case before approving.")
    ...
    expect(clinicalCaseReviewSource).toContain("Auto-saves as you type.")
    expect(clinicalCaseReviewSource).toContain("Only you can see this note until you send.")
  ...
  it("keeps complete patient states neutral while missing states stay prominent", () => {
    ...
    expect(decisionStripSource).toContain("Identity and notes")
