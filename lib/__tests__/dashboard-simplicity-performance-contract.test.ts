...
describe("dashboard simplicity and runtime performance contracts", () => {
  ...
  it("keeps the doctor queue header focused on current operational pressure", () => {
    ...
    expect(queueClientSource).toContain("No cases finished yet today.")
    expect(queueClientSource).toContain("Next: ${nextPatientName")
  ...
  it("keeps clinical action colours muted on the doctor surface", () => {
    ...
    expect(actionButtonsSource).toContain("All checks passed. Note ready to send.")
    ...
    expect(actionButtonsSource).toContain("border-transparent")
