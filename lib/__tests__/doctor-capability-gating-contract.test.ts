...
describe("doctor capability gating contract", () => {
  ...
  it("admin (owner-operator) always has all clinical capabilities by default", () => {
    ...
    // S8 defaults false even for admin until explicitly granted.
    expect(doctorHasCapability(admin, "prescribe_s8")).toBe(false)
  })
