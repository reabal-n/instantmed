...
describe("doctor add patient Parchment contract", () => {
  it("exposes an Add patient CTA on the patient directory and selects Parchment sync state", () => {
    ...
    expect(listSource).toContain('aria-label="Search patients"')
    expect(listSource).toContain('aria-label="Active patient exceptions on this page"')
  ...
  it("lets doctors retry/open Parchment from an existing patient record", () => {
    ...
    expect(detailSource).toContain("Identity and contact")
    expect(detailSource).toContain(">Prescribing</span>")
