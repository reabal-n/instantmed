...
import { editSoapSection, parseSoapDraft } from "@/lib/doctor/soap-note"
...
describe("lossless SOAP editing", () => {
  it("preserves typing spaces, blank lines and every untouched byte", () => {
    ...
    expect(editSoapSection(note, parsed, "S", "Patient reports symptoms  \n\nNext word ").note).toBe(
      "S: Patient reports symptoms  \n\nNext word \nO:\tRecorded answers\nA: Review pending\nP: Review required\n\n",
    )
    expect(editSoapSection(note, parsed, "P", parsed.P.value).note).toBe(note)
  })
  it.each([
  ...
  it("retains marker-like text inserted inside a section", () => {
    const note = "S: First\nO: Observed\nA: Assessment\nP: Plan"
    const edited = editSoapSection(note, parseSoapDraft(note)!, "S", "First\nP: quoted text\nStill subjective ").note
...
describe("editing an embedded marker without changing editor fields", () => {
  it("keeps section boundaries stable for continued typing after a marker line", () => {
