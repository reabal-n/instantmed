...
describe("patient directory duplicate review UX", () => {
  ...
  it("keeps the directory scalable without making search page-local", () => {
    ...
    expect(patientsListSource).toContain("createPatientDirectoryNavigationCoordinator")
    expect(patientsListSource).toContain("directoryNavigation.scheduleSearch")
  ...
  it("offers only URL-mirrored global newest and name sorting", () => {
    ...
    expect(patientsListSource).toContain("sort: currentSort")
    expect(patientsListSource).toContain("directoryNavigation.cancelPendingSearch()")
