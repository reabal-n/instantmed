...
describe("patient directory duplicate review UX", () => {
  ...
  it("keeps the directory scalable without making search page-local", () => {
    ...
    expect(patientsListSource).toContain("searchPatientDirectoryAction")
    expect(patientsListSource).toContain("searchRequestSequenceRef")
    expect(patientsListSource).toContain("useDebounce(searchQuery, 350)")
  ...
  it("offers only URL-mirrored global newest and name sorting", () => {
    ...
    expect(patientsListSource).toContain("sequence !== searchRequestSequenceRef.current")
    expect(patientsListSource).not.toContain('params.set("q"')
