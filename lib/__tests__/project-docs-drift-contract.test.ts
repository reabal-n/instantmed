...
describe("project docs drift contract", () => {
  ...
  it("keeps root migration canon aligned with the on-disk release tranche", () => {
    ...
    expect(wikiArchitecture).toContain(
      "Linked migration history is aligned through `20260915131638`",
    )
    expect(wikiArchitecture).toContain(
  ...
  it("keeps growth policy and prepared submission packets aligned with active service scope", () => {
    ...
    expect(roadmap).toContain("rungs are all closed by explicit operator decision")
    expect(roadmap).not.toContain("formal rung remains open")
    expect(roadmap).toContain("no second doctor is hired")
    expect(roadmap).toContain("`$10,831.70` net-retained revenue")
