...
describe("portfolio money-page art direction", () => {
  it("uses a catalog-derived card chooser instead of the split-panel route diagram", () => {
    ...
    expect(homepage).toMatch(/Australian\s+adults 18\+/)
  ...
  it("gives the women's-health overview two bounded lanes and one review boundary", () => {
    ...
    expect(landing).toContain("<WomensHealthDecisionFork isDisabled={isDisabled} />")
    ...
    expect(fork).toContain('href={isDisabled ? "/contact" : "/request?service=consult&subtype=womens_health&intent=uti"}')
