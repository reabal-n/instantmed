...
describe("email delivery ops contract", () => {
  ...
  it("does not expose manual retries for cron-owned audit-only email failures", () => {
    ...
    expect(emailHubClientSource).toContain("isProviderTerminalDeliveryStatus")
    expect(emailHubClientSource).toContain("New attempt required")
    ...
    expect(emailRetrySource).toContain("id, email_type, status, delivery_status, retry_count, certificate_id, error_message")
    expect(emailRetrySource).toContain("terminal and is not retryable")
    expect(emailRetrySource).toContain("isProviderTerminalDeliveryStatus")
    expect(emailRetrySource).toContain("start a new audited send attempt")
  })
})
