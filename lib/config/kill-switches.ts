 ...
 * Kill Switches
 *
 * Environment variable-based kill switches for disabling critical flows.
 * All flags are server-side only to prevent exposure of internal config.
 *
 * NOTE: This is separate from the DB-based feature flags in lib/feature-flags.ts.
 * DB flags are toggled at runtime via /admin/features.
 * Kill switches here require an env var change + redeploy.
 *
 * Usage:
 *   import { flags, isCheckoutDisabled, isConsultSubtypeDisabled } from "@/lib/config/kill-switches"
 *
...
const logger = createLogger("kill-switches")
