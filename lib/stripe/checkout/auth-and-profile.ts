...
export async function runAuthAndProfile(
  ...
  try {
    ...
    return stepFail("auth_or_session", "Authentication failed. Please sign in and try again.")
  }
  if (!authUser) {
    logger.warn("User not authenticated when attempting checkout")
    return stepFail(
      "auth_or_session",
      "You must be logged in to submit a request. Please sign in and try again.",
    )
  }
  ...
  if (authUser.profile?.id) {
    ...
    if (profileError || !profileId) {
      return stepFail(
        "persistence",
        `Profile creation failed: ${profileError || "Unknown error"}`,
      )
    }
    patientId = profileId
  }
  ...
  if (!dobForAgeCheck) {
    ...
    return stepFail("clinical_or_input_validation", AGE_REQUIREMENT_ERROR)
  }
  ...
  if (age < 18) {
    ...
    return stepFail("clinical_or_input_validation", UNDER_18_ERROR)
  }
