...
"use client";
/**
 * Patched next-themes for React 19 compatibility.
 *
 * React 19 throws "Encountered a script tag while rendering React component"
 * (TypeError: nextResource.createElement is not a function) when next-themes'
 * ThemeScript component renders React.createElement("script", ...).
 *
 * This patch replaces the ThemeScript component with one that runs the same
 * theme-init function via useInsertionEffect instead — no <script> DOM node,
 * same synchronous-before-paint execution order on the client.
 */
...
var b = ["light", "dark"],
  ...
  // Patched: use useInsertionEffect instead of rendering <script>
  _ = t.memo(
    ({
      forcedTheme: e,
      storageKey: i,
      ...
      (t.useInsertionEffect || t.useLayoutEffect || t.useEffect)(() => {
        M(s, i, a, e, h, l, u, m);
      }, []);
      return null;
    }
