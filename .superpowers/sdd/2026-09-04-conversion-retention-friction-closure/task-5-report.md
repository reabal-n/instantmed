...
- Added a server-only PostHog start-cohort reader for valid RFC UUID v4 flow IDs. It reports separate starts, checkout initiations, server purchases, repeat-Rx medication progression, mobile viewed/completed counts and rate, validation blocks, unresolved validation blocks, service steers, and clinical hard blocks.
...
- Added strict CLI arguments, a package invocation that loads `.env.local`, recursive sensitive-key/value rejection, and atomic mode-0600 JSON output. `--release-at` is mandatory and visible in every invocation; future boundaries are rejected. `.env.example` and `docs/OPERATIONS.md` document deploy/promote, capture SHA and READY time, set Production env, redeploy, and verify.
