...
function CockpitIntakeDetailClient({
  ...
  const { lockState } = useIntakeLock(intake.id, Boolean(reviewData.viewerActionAccess?.canReviewService && isReviewLockableStatus(reviewData.intake.status)))
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (reviewData.intake.doctor_notes == null && reviewData.aiDrafts) {
      ...
      if (clinicalDraft) {
        ...
        actions.setInitialNotes(formatted || "", formatted || "", false)
      } else {
        actions.setInitialNotes("", "", false)
      ...
      <IntakeReviewProvider value={contextValue}>
        <div className="flex h-[calc(100dvh-4rem)] min-h-0 flex-col gap-3 overflow-hidden">
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
            <Button variant="ghost" size="sm" asChild className="-ml-2 text-muted-foreground">
              <Link href={backHref} onClick={async (event) => {
                ...
                if (await actions.flushNotes()) router.push(backHref)
              }}>{backLabel}</Link>
            </Button>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden pr-1">
            ...
            />
            <IntakeReviewCockpit className="min-h-0 flex-1" historicalReviewActions={historicalReviewActions} />
          </div>
        </div>
      </IntakeReviewProvider>
