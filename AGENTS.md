...
- **Cockpit v2 ledger (2026-05-20)**: `/admin/intakes` renders the new `AdminIntakesLedgerV2` (in `app/admin/intakes/intakes-ledger-v2.tsx`) when the `cockpit_v2` feature flag is on, otherwise the legacy `AdminIntakesLedgerClient`. Default sort is `created_at DESC`. The historical client-side status-priority sort (at `intakes-ledger-client.tsx:204`) is now an opt-in "Smart sort" toggle, not the surprise default. **Do not add an unconditional status-priority client re-sort to any new staff list surface** — make work-priority an explicit operator choice, not the silent default. The v2 ledger keeps the existing `IntakeReviewPanel` slide-over: a plain left-click on a row opens it, Cmd/Ctrl/middle-click follows the row's `href` to the case detail page. Sort and quick-filter state mirror to the URL so views are shareable. Use `STAFF_LEDGER_HREF` for any `router.replace` targeting the ledger.
