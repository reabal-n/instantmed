...
**Staff dashboard:** Admin, doctor, and support work are intentionally unified into one staff cockpit. `/dashboard` is the canonical URL for the staff dashboard (Phase 1 of remaster, 2026-05-11) and forwards by role until Phase 2 lands the consolidated queue surface here. Today the actual cockpit lives at `/admin` (admin doctor) and `/doctor/dashboard` (clinical doctor) and uses `OperatorShell`, surfacing the next clinical/admin actions without separate "admin mode" vs "doctor mode" switching. Staff pages should use `components/operator/*`, shared navigation from `lib/dashboard/staff-navigation.ts`, compact case summaries from `lib/doctor/case-summary.ts`, and the central `revalidateStaff()` helper from `lib/dashboard/revalidate-staff.ts` for cache busting. Use the new `STAFF_*_HREF` constants from `lib/dashboard/routes.ts` for new code; legacy `ADMIN_*_HREF` / `DOCTOR_*_HREF` constants are aliases kept until Phase 2 consolidates the file tree. Keep dashboards bounded and scannable: primary desktop staff pages should avoid whole-page scrolling by using split panes, short action lists, tabs, and detail drawers/panels. If a staff page has more controls than fit in one viewport, keep the page itself fixed-height and put secondary detail inside internal panes.
