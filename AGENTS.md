...
- **Patient profile (Phase 4 remaster, 2026-05-12)**: One canonical patient profile at `/doctor/patients/[id]`. `/admin/patients/[id]` is a redirect-only stub — the duplicate 471-line admin patient page (4-stat tile row + 3-card layout) was retired. Admin-specific actions (patient merge audit, refunds) live on the operator nav, not duplicated on each profile.
