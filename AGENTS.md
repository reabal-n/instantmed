...
- **Supabase migrations**: Squashed to 1 baseline, then layered fixes on top. Current count on disk: **75 migration files** (1 baseline + 74 incremental). Most recent: `20260524090000_allow_forward_dated_med_certs.sql` (widens the AEST-aware cert `start_date` constraint to allow forward-dating up to 14 days; paired with PR #57's UI chip widening). Prior to that, `20260524080000_aest_aware_cert_start_date_constraint.sql` (replaces UTC `CURRENT_DATE` with AEST-anchored `(now() AT TIME ZONE 'Australia/Sydney')::date` on the cert `start_date` constraint — see Pinned Bug Class #2 for the AEST/UTC disagreement pattern) and `20260521010000_add_paid_request_telegram_message_id.sql`. Account closure state lives in `20260502003000_add_account_closure_to_profiles.sql`; archived audit rows are locked by `20260504193100_enable_rls_on_audit_logs_archive.sql`. 182 pre-squash files were removed on 2026-04-21 after a drift audit found they were never in the tracker and would break fresh clones via `supabase db push`. Full historical audit at `docs/audits/2026-04-21-migration-drift-audit.md`; current migration/conversion audit at `docs/audits/2026-05-03-supabase-conversion-audit.md`. Use `supabase db push` and `pnpm db:check-migrations` after linked migration changes. Note: the tracker still has a duplicate-name row for `add_delay_notification_sent_at` (both `20260403000001` and `20260403062235`); cosmetic only, clean up with `supabase migration repair` when convenient.
