...
- **`support` role**: Non-clinical operations role. Added to `user_role` enum on 2026-05-11. Phase 7 of dashboard remaster (2026-05-12) bounds support to `/admin/ops`, `/admin/webhook-dlq`, `/admin/ops/parchment`, and `/admin/ops/prescribing-identity` only. Support sees masked/redacted data, can work Stripe webhook recovery, can triage Parchment/identity blockers, and cannot access patient ledgers, email hub, settings, finance, analytics, clinical actions, approvals, or prescribing. Application-layer checks via `hasSupportAccess(profile)` from `lib/auth/staff-capabilities.ts`.
