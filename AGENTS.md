...
- **Branch protection on `main`: NOT SET as of 2026-05-23 evening** — verified via `gh api repos/reabal-n/instantmed/branches/main/protection` returning 404. Root cause of "CI red on main for 2 days without anyone noticing": no enforcement. Apply via `gh api -X PUT ...` one-liner in the doc cleanup session log when ready. Recommended: require `build` + `Doc surface audit` checks, no admin enforcement (so the owner-operator can still bypass for emergencies), no PR review requirement (solo operator).
