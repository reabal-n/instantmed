...
|------|---------------|
| `app/` | 573 files |
| Route-like files under `app/` | 251 |
| API route files under `app/api/` | 96 |
| Cron route files under `app/api/cron/` | 30 |
| `components/` | 411 files |
| `lib/` | 1,478 files |
| E2E TypeScript files under `e2e/` | 93 (78 specs) |
| Health guide MDX files under `content/blog/` | 107 |
| SQL migrations under `supabase/migrations/` | 147 |

Newest on-disk migration is `20260922081547_google_ads_adjustment_exact_cash.sql` (exact cash and NULL-safe first adjustment attempts; applied and verified in production on 2026-09-22). Latest applied production migration is `20260922081547_google_ads_adjustment_exact_cash.sql` (service-only adjustment view; one recent target remains due). Previous applied production migration is `20260914105558_restore_weight_management_checkout_catalog.sql` (canonical weight-management catalog restored at 8,995 cents and age 18; inactive legacy row preserved; no patient records changed). The preceding applied production migration is `20260913080413_refine_request_closures_and_retire_priority_refunds.sql` (administrative closure recovery and priority refund retirement; applied and verified in production on 2026-09-13). Live read-back matched both reviewed refund function bodies, confirmed service-role-only execution and recovery-view access, a valid administrative-closure recovery index, aligned migration history, zero SECURITY DEFINER ACL violations and zero schema-lint errors. The earlier applied production migration is `20260906100000_monitor_observation_state.sql`. Live read-back confirmed two initial state rows, a valid unique partial version index, fixed-empty-search-path service-role-only RPC execution, unchanged SELECT/INSERT-only table grants, aligned migration history and zero SECURITY DEFINER ACL violations. The preceding migration is `20260905130000_google_ads_adjustment_health_reconciliation.sql`. It corrects read-time Ads health classification with no payment, claim, or audit backfill; live metadata confirms service-role-only invoker access. Prior `20260905120001_converge_delivery_tracking_runtime.sql` converges the legacy delivery-tracking shape without rewriting rows; live schema lint passes. The preceding `20260905120000_refill_reminder_funnel.sql` contains only aggregate refill cohorts and indexes. Runtime-schema convergence, preference ordering (`1100`), shared Resend receipts (`1150`), and refill reporting (`1200`) were applied in order on 2026-09-05. Live metadata confirmed nullable columns, enabled preference triggers, invoker RPCs executable only by service_role, and zero SECURITY DEFINER ACL violations. Linked migration history is aligned through `20260915131638`. These schema migrations performed no historical profile repair, identity-trigger replacement, or patient send. Earlier recovery-email migration `20260903120000_recovery_email_engagement.sql` remains applied. The immediately preceding applied migration is `20260902090000_converge_fraud_flag_review_state.sql`, which converges legacy boolean fraud-review state to the app-owned `open|reviewed|dismissed` model; it is also applied and verified. Live recovery metadata confirmed a nullable `timestamptz` column with no default, its enabled preservation trigger, and its `SECURITY INVOKER` function with `EXECUTE` denied to `PUBLIC`, `anon`, and `authenticated`; the migration backfilled no markers. The prior `20260828090000_specialty_experience_attribution.sql` adds bounded, nullable, non-clinical `growth_experience_version` columns with set-once draft and immutable intake semantics. Live metadata confirmed both nullable `text` columns, both validated constraints, both enabled triggers, and both `SECURITY INVOKER` trigger functions with `EXECUTE` denied to `PUBLIC`, `anon`, and `authenticated`. The immediately preceding `20260825073433_scope_profiles_realtime_policy_to_authenticated.sql` is also applied; all three `is_doctor()` policies have roles exactly `{authenticated}`. The additive `20260827210500_twilio_voice_callback_requests.sql` is applied and verified in production: the encrypted queue was empty at release, service-role access succeeded, `anon` table/RPC access was denied, and the live SECURITY DEFINER ACL checker returned zero violations. The earlier refund-recovery migration, `20260816101752_harden_stripe_refund_recovery.sql`, also passed its linked DB lint and ACL gates; detailed database receipts live in `docs/ARCHITECTURE.md`.
