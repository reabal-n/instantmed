...
- **Supabase migrations**: Squashed to 1 baseline, then layered fixes on top. Current count: **199 migration files** (baseline + 198 incremental). Most recent: `20260411000003_parchment_integration.sql` (sex, parchment_user_id, parchment_patient_id on profiles). Use `supabase db push`. May need `supabase migration repair` for drift. Note: Supabase's MCP `list_migrations` only shows what's been pushed via the migration tracker — not everything on disk.
