...
- **Certificate UI labels (Phase 3 remaster)**: `Edit & Reissue` was renamed to `Correct certificate` (same reference number, replaces the PDF in-place). The reissue-vs-regenerate distinction is "Correct certificate" (same reference, max 3 reissues) vs "Replace certificate" (new reference + supersedes old; tracked by `regenerate-certificate.ts`). Do not relabel without updating both surfaces.
