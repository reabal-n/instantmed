...
- **Supabase migrations**: Squashed to 1 baseline, then layered fixes on top. Current count: **198 migration files** (baseline + 197 incremental). Most recent: `20260411000002_service_waitlist.sql` (service_waitlist table for coming-soon email capture). Use `supabase db push`. May need `supabase migration repair` for drift. Note: Supabase's MCP `list_migrations` only shows what's been pushed via the migration tracker — not everything on disk.
