...
- **Supabase migrations**: Squashed to 1 baseline, then layered fixes on top. Do not trust stale migration-count notes; use `supabase migration list` before pushing. Most recent local migration: `20260501060000_guard_med_cert_future_start_dates.sql` (CHECK guard preventing future-dated issued certificate `start_date` values). Prior safety migration `20260428000001_lock_cert_status.sql` adds the BEFORE UPDATE trigger that only permits `valid → revoked` transitions on `issued_certificates`; defence-in-depth so no future cron can silently mass-mutate cert status. 182 pre-squash files were removed on 2026-04-21 after a drift audit found they were never in the tracker and would break fresh clones via `supabase db push`. Full audit at `docs/audits/2026-04-21-migration-drift-audit.md`. Use `supabase db push` after confirming the pending list. Note: the tracker still has a duplicate-name row for `add_delay_notification_sent_at` (both `20260403000001` and `20260403062235`); cosmetic only, clean up with `supabase migration repair` when convenient.
- **Med certs do not expire**: The retired `expire-certificates` cron must stay absent from routes, `vercel.json`, heartbeat monitoring, and tests. Verification only treats `revoked` as inauthentic; legacy `superseded` rows return a "request the current version" message; everything else verifies. Don't reintroduce expiry logic — the DB trigger above will reject it.
- **Med cert language and dates are locked**: `lib/pdf/template-renderer.ts` `getBodyText` / `getReturnText` use conservative consultation-statement language only. Do NOT add "medically unfit", exam-deferral support, fitness-for-driving / firearm / court / flying / custody language, or any modality disclosure ("asynchronous telehealth") in the cert body. Marketing copy in `components/marketing/sections/certificate-type-selector.tsx` must not promise outcomes ("Accepted by all X", "Supports Y"). High-stakes use cases (exam, court, fitness-to-X, workers comp, NDIS, TAC) are blocked at intake by `checkHighStakesUseCase` in `lib/clinical/intake-validation.ts` and again at auto-approval by `HIGH_STAKES_USE_CASE_KEYWORDS` in `lib/clinical/auto-approval.ts`. Hard duration cap: `MAX_MED_CERT_DURATION_DAYS = 3`. Start dates must be today or in the past; use `lib/medical-certificates/date-policy.ts` for every checkout, approval, in-place reissue/date correction, render, or preview path. DB migration `20260501060000_guard_med_cert_future_start_dates.sql` adds the table-level guard.
- **Med cert credentials stay out of telemetry**: Verification codes, certificate refs, and certificate numbers may appear in the patient-facing certificate/email body where clinically required, but not in analytics properties, Sentry breadcrumbs/extras/tags, email metadata, or non-essential logs. Regression coverage lives in `lib/__tests__/medical-certificate-policy.test.ts`, `lib/__tests__/sentry-phi-scrubber.test.ts`, and `lib/__tests__/logger-sanitization.test.ts`.
