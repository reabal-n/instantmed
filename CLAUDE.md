...
- **Staff dashboard URL (Phase 2 remaster, 2026-05-12)**: `/dashboard` is the live staff cockpit. `/admin`, `/doctor`, `/doctor/dashboard`, and `/doctor/queue` redirect here from `next.config.mjs`. New code references `STAFF_DASHBOARD_HREF` / `buildStaffDashboardHref(...)` from `lib/dashboard/routes.ts` rather than the legacy `ADMIN_DASHBOARD_HREF` / `DOCTOR_DASHBOARD_HREF` aliases. The dashboard page is role-aware: admin renders the full Admin layer + setup card + queue; doctor renders queue + KPI; support is forwarded to the bounded support ops cockpit at `/admin/ops`.
...
- **Retired dashboard components (Phase 2 remaster)**: `AdminPulse`, `lib/data/admin-pulse.ts`, `yesterday-widget*`, the non-compact `AdminHubZones` branch, and `IntakeMonitor` were deleted. `/admin/analytics` reads the queue-health helpers directly; do not reintroduce a separate live-monitor widget or polling API.
