...
- **Supabase migrations**: Squashed to 1 baseline, then layered fixes on top. Current count: **196 migration files** (baseline + 195 incremental). Most recent: `20260410000002_doctor_telegram_alert.sql` (doctor_telegram_alert_sent_at column on intakes + 3 notification feature flags seeded). Use `supabase db push`. May need `supabase migration repair` for drift. Note: Supabase's MCP `list_migrations` only shows what's been pushed via the migration tracker — not everything on disk.
