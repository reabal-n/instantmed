...
- **Retired dashboard components (Phase 2 remaster)**: `AdminPulse`, `lib/data/admin-pulse.ts`, `yesterday-widget*`, and the non-compact `AdminHubZones` branch were deleted. `IntakeMonitor` is still live on `/admin/analytics` (where stats belong) but never on the dashboard. Do not reintroduce them.
