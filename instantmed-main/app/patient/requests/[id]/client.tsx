...
export default function PatientRequestDetailPageClient({
  ...
  document,
  showRetryPrompt,
}: PatientRequestDetailPageClientProps) {
  ...
  const statusConfig = getStatusConfig(request.status, request.payment_status)
          ...
          {request.category && (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{formatCategory(request.category)}</p>
              </div>
            </div>
          )}
          <div className="flex items-start gap-3">
