...
test.describe("Unified Request Flow - Draft Persistence", () => {
  ...
  for (const { width, theme } of [{ width: 390, theme: "light" }, { width: 1280, theme: "dark" }] as const) {
      test(`recovered prescription checkout can correct identity at ${width}px in ${theme} mode`, async ({ page }, testInfo) => {
        ...
        await page.screenshot({ path: testInfo.outputPath("checkout-identity-error.png"), fullPage: true })
        await alert.getByRole("button", { name: "Edit your details", exact: true }).click()
        ...
        await page.screenshot({ path: testInfo.outputPath("checkout-identity-details.png"), fullPage: true })
